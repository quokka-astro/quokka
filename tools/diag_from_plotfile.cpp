...
{
	std::cout << "Usage:\n"
		  << "  quokka_diag_from_plotfile --inputs <inputs.in> --plotfile <plt> [options]\n\n"
		  << "Options:\n"
		  << "  --inputs <file>     Quokka inputs file (required)\n"
		  << "  --plotfile <dir>    AMReX plotfile directory (required)\n"
		  << "  --force             Run diagnostics regardless of interval settings\n"
		  << "  --step <n>          Override plotfile step number\n"
		  << "  --time <t>          Override plotfile time\n"
		  << "  param=value         Any AMReX/Quokka ParmParse overrides\n";
}

auto ParseArgs(int argc, char **argv, std::string &inputs, std::string &plotfile, bool &force,
	       std::optional<int> &stepOverride, std::optional<amrex::Real> &timeOverride,
...
auto BuildAmrexArgv(char **argv, const std::string &inputs, const std::vector<std::string> &overrides,
		    std::vector<std::string> &amrexArgsStr, std::vector<char *> &amrexArgs) -> void
{
	amrexArgsStr.clear();
	amrexArgs.clear();
...
template <>
void DiagFramePlane::processDiag<OfflineProblem>(int a_nstep, const amrex::Real &a_time)
{
...
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
		...
		for (amrex::MFIter mfi(planeData[lev], amrex::TilingIfNotGPU()); mfi.isValid(); ++mfi) {
			...
			if (m_normal == 0) {
				amrex::ParallelFor(bx, m_fieldNames.size(), [=] AMREX_GPU_DEVICE(int i, int j, int k, int n) noexcept {
					...
					plane(i, j, k, n) = intwgt[0] * state(p0m1, i, j, stIdx) + intwgt[1] * state(p0, i, j, stIdx) +
								intwgt[2] * state(p0p1, i, j, stIdx);
				});
			} else if (m_normal == 1) {
				amrex::ParallelFor(bx, m_fieldNames.size(), [=] AMREX_GPU_DEVICE(int i, int j, int k, int n) noexcept {
					int const stIdx = idx_d_p[n]; // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
					plane(i, j, k, n) = intwgt[0] * state(i, p0m1, j, stIdx) + intwgt[1] * state(i, p0, j, stIdx) +
								intwgt[2] * state(i, p0p1, j, stIdx);
				});
			} else if (m_normal == 2) {
				amrex::ParallelFor(bx, m_fieldNames.size(), [=] AMREX_GPU_DEVICE(int i, int j, int k, int n) noexcept {
					int const stIdx = idx_d_p[n]; // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
					plane(i, j, k, n) = intwgt[0] * state(i, j, p0m1, stIdx) + intwgt[1] * state(i, j, p0, stIdx) +
	...
	if (nlevs > 0) {
		...
		Write2DMultiLevelPlotfile(diagfile, nlevs, GetVecOfConstPtrs(planeData), m_fieldNames, pltGeoms, a_time, step_array, ref_ratio, simulationMetadata);
	}
}
...
auto main(int argc, char **argv) -> int
{
	...
	if (!ParseArgs(argc, argv, inputs, plotfile, force, stepOverride, timeOverride, amrexOverrides)) {
		PrintUsage();
		return 1;
	}
	...
	int exitCode = 0;
	{
		...
		std::unordered_set<std::string> supportedTypes = {"DiagFramePlane", "DiagPDF"};
#if AMREX_SPACEDIM == 3
		...
		for (auto const &diagName : diagNames) {
			...
			if (!supportedTypes.contains(diagType)) {
				amrex::Print() << "Skipping diagnostic '" << diagName << "' (type " << diagType
						   << ") - not supported by offline tool.\n";
				continue;
			}
		...
		for (auto const &var : diagVars) {
			if (std::find(pfVars.begin(), pfVars.end(), var) == pfVars.end()) {
				amrex::Abort("Required diagnostic variable missing from plotfile: " + var);
			}
		}
		...
		for (int lev = 0; lev < nlevels; ++lev) {
			diagMFVec[lev] = std::make_unique<amrex::MultiFab>(grids[lev], dmaps[lev], static_cast<int>(diagVars.size()), nGrow);
		...
		for (auto &diag : diagnostics) {
			...
			diag->setDiagData<OfflineProblem>(nullptr, &diagMFVecPtr, &diagVars, &geoms, &refRatio, &metadata);

			bool willDo = force || diag->doDiag(time, nstep);
			if (!willDo) {
				amrex::Print() << "Skipping diagnostic (interval not matched). Use --force to override.\n";
				continue;
			}

			if (auto *framePlane = dynamic_cast<DiagFramePlane *>(diag.get())) {
				if (!framePlane->getParticleTypes().empty()) {
					amrex::Print() << "DiagFramePlane requests particles; offline tool ignores particle output.\n";
				}
