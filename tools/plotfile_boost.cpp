...
auto FindComponentIndex(amrex::Vector<std::string> const &names, std::string const &target) -> int
{
...
void PrintUsage()
{
	amrex::Print() << "Usage: plotfile_boost <plotfile_in> <plotfile_out> <vx> <vy> <vz>\n";
}
...
auto main(int argc, char **argv) -> int
{
	amrex::Initialize(argc, argv);
	{
		...
		const amrex::Real boost_v2 = (boost_vx * boost_vx) + (boost_vy * boost_vy) + (boost_vz * boost_vz);
		...
		if (rho_index < 0 || px_index < 0 || py_index < 0 || pz_index < 0 || energy_index < 0) {
			amrex::Abort("Required plotfile variables not found (gasDensity/x-GasMomentum/y-GasMomentum/z-GasMomentum/gasEnergy)");
		}
		...
		for (int lev = 0; lev <= finest_level; ++lev) {
			...
			geom[lev] = amrex::Geometry(plotfile.probDomain(lev), &real_box, plotfile.coordSys(), is_per.data());
			if (lev < finest_level) {
			...
			for (amrex::MFIter mfi(state[lev]); mfi.isValid(); ++mfi) {
				...
				amrex::ParallelFor(box, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
					...
					const amrex::Real energy_new =
					    energy - (boost_vx * px + boost_vy * py + boost_vz * pz) + static_cast<amrex::Real>(0.5) * rho * boost_v2;
		...
		amrex::WriteMultiLevelPlotfile(plotfile_out, finest_level + 1, mf_ptrs, var_names, geom, plotfile.time(), level_steps, ref_ratio);
	}
	amrex::Finalize();
	return 0;
}
