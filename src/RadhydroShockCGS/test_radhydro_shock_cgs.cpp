...
constexpr double c_v =
    k_B / (mu * (gamma_gas - 1.0)); // specific heat [erg g^-1 K^-1]
...
constexpr double shock_position =
    0.0130; // 0.0132; // cm (shock position drifts to the right slightly during
            // the simulation, so we initialize slightly to the left...)
...
auto problem_main() -> int {
  ...
  if (amrex::ParallelDescriptor::IOProcessor()) {
    ...
    if (fstream.is_open()) {
      ...
      matplotlibcpp::scatter(strided_vector_from(xs_exact_scaled, s),
                             strided_vector_from(Trad_exact, s), 10.0,
                             Trad_exact_args);
    }
    ...
    if (fstream.is_open()) {
      ...
      matplotlibcpp::scatter(strided_vector_from(xs_exact_scaled, s),
                             strided_vector_from(Tmat_exact, s), 10.0,
                             Tgas_exact_args);
    }
