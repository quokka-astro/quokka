...
// Specialization for Protostar particles
template <> struct ParticleCreationTraits<ParticleType::Protostar> {
	...
	template <typename problem_t> struct ParticleCreator {
			   ...
			   amrex::RandomEngine const & /*engine*/) const
		{
			...
			for (int p_idx = 0; p_idx < num_particles; ++p_idx) {
				...
				// Set birth time
				if (birth_time_index >= 0 && birth_time_index < ParticleType::NReal) {
					p.rdata(birth_time_index) = current_time;
				}
				// Set initial luminosity to zero if applicable
				if constexpr (Physics_Traits<problem_t>::is_radiation_enabled) {
					constexpr int nGroups = Physics_Traits<problem_t>::nGroups;
					// Assuming luminosity follows birth_time directly
					int lum_index = birth_time_index + 1;
					for (int g = 0; g < nGroups; ++g) {
						if (lum_index + g < ParticleType::NReal) {
							p.rdata(lum_index + g) = 0.0;
						}
					}
				}
			}
	...
	{
		// Use the common implementation with our checker and creator types
		ParticleCreationImpl::createParticlesImpl<problem_t, ContainerType, ParticleCreationTraits<ParticleType::Protostar>::template ParticleChecker,
							  ParticleCreationTraits<ParticleType::Protostar>::template ParticleCreator>(
		    container, mass_idx, state, accretion_rate, lev, current_time, dt, evolution_stage_index, birth_time_index, state_fc);
	}
};
