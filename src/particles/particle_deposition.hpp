...
//-------------------- Deterministic Mass Deposition (AMR-aware) --------------------
...
void deterministicDepositSingleLevel(const ContainerType* container, amrex::MultiFab &buffer_rhs, int lev, amrex::Real Gconst, int mass_idx)
{
	...
	for (amrex::MFIter mfi(buffer_rhs); mfi.isValid(); ++mfi) {
		const auto &ptile = container->ParticlesAt(lev, mfi);
                                ...
                                int lev_min, int finest_lev, amrex::Real Gconst, int mass_idx)
{
	...
	for (int lev = 0; lev <= finest_lev; ++lev) {
		mf_part[lev].define(container.ParticleBoxArray(lev), container.ParticleDistributionMap(lev), 1, nGrow);
		mf_tmp[lev].define(container.ParticleBoxArray(lev), container.ParticleDistributionMap(lev), 1, 0);
	...
	for (int lev = finest_lev - 1; lev >= lev_min; --lev) {
		amrex::average_down(mf_part[lev + 1], mf_part[lev], 0, 1, container.GetParGDB()->refRatio(lev));
	}

	// Step 6: Copy results back to target MultiFabs
	for (int lev = lev_min; lev <= finest_lev; ++lev) {
		// Check if grids match - if not, need ParallelCopy
		if (container.OnSameGrids(lev, *rhs[lev])) {
