...
namespace StellarPhysics
{
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE auto radius_ZAMS(amrex::Real mass) -> amrex::Real
    {
        ...
        amrex::Real denominator = StellarTables::Tout96::nu + StellarTables::Tout96::xi * std::pow(m_sol, 2.0) + StellarTables::Tout96::upsilon * std::pow(m_sol, 8.5) +
                                 std::pow(m_sol, 18.5) + StellarTables::Tout96::gpi * std::pow(m_sol, 19.5);
