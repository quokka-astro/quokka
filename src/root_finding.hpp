...
public:
   ...
   AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE eps_tolerance(amrex::Real eps_)
   {
      eps = eps_;
   }
   AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE eps_tolerance(unsigned bits)
   {
...
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, const T& fax, const T& fbx, Tol tol, int& max_iter)
{
   //
...
template <class F, class T, class Tol>
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, Tol tol, int& max_iter)
{
   max_iter -= 2;
...
template <class F, class T, class Tol>
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
std::pair<T, T> bracket_and_solve_root(F f, const T& guess, T factor, bool rising, Tol tol, int& max_iter)
{
   BOOST_MATH_STD_USING
   ...
   if((fa < 0) == (guess < 0 ? !rising : rising))
   {
      ...
      {
         if(count == 0)
            AMREX_ALWAYS_ASSERT_WITH_MESSAGE(false, "Unable to bracket root, last nearest value was %1%");
         //
