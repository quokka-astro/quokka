...
public:
  AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE eps_tolerance() {
  ...
  AMREX_FORCE_INLINE explicit eps_tolerance(unsigned bits) {
    ...
    eps = (std::max)(T(ldexp(1.0F, 1 - bits)),
                     T(4 * std::numeric_limits<T>::epsilon()));
  }
  AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE bool operator()(const T &a,
                                                           const T &b) {
    BOOST_MATH_STD_USING
...
bracket(F f, T &a, T &b, T c, T &fa, T &fb, T &d, T &fd) {
  //
...
template <class T>
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE T safe_div(T num, T denom, T r) {
  //
...
template <class T>
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE T
quadratic_interpolate(const T &a, const T &b, T const &d, const T &fa,
...
template <class T>
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE T
cubic_interpolate(const T &a, const T &b, const T &d, const T &e, const T &fa,
  ...
  T d21 = (b - d) * fd / (fd - fb);
  T d31 = (a - b) * fb / (fb - fa);
  BOOST_MATH_INSTRUMENT_CODE("q11 = " << q11 << " q21 = " << q21
                                      << " q31 = " << q31 << " d21 = " << d21
                                      << " d31 = " << d31);
  ...
  T c = q31 + q32 + q33 + a;
  BOOST_MATH_INSTRUMENT_CODE("q22 = " << q22 << " q32 = " << q32
                                      << " d32 = " << d32 << " q33 = " << q33
                                      << " c = " << c);
...
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE std::pair<T, T>
toms748_solve(F f, const T &ax, const T &bx, const T &fax, const T &fbx,
              Tol tol, int &max_iter) {
  //
  ...
  if (sgn(fa) * sgn(fb) > 0) {
    AMREX_ALWAYS_ASSERT_WITH_MESSAGE(
        false, "Parameters a and b do not bracket the root!");
  }
...
template <class F, class T, class Tol>
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE std::pair<T, T>
toms748_solve(F f, const T &ax, const T &bx, Tol tol, int &max_iter) {
  max_iter -= 2;
...
template <class F, class T, class Tol>
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE std::pair<T, T>
bracket_and_solve_root(F f, const T &guess, T factor, bool rising, Tol tol,
                       int &max_iter) {
  BOOST_MATH_STD_USING
  ...
  if ((fa < 0) == (guess < 0 ? !rising : rising)) {
    ...
    while ((sgn)(fb) == (sgn)(fa)) {
      if (count == 0) {
        AMREX_ALWAYS_ASSERT_WITH_MESSAGE(
            false, "Unable to bracket root, last nearest value was %1%");
      }
      //
