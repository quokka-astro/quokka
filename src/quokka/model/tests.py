...
from quokka.tools.cmake import ctest_root_testfile_path
...
def discover_problems(build_dir: Path, command: str, profile: Optional[str]) -> List[str]:
    ...
    receipts = __import__('quokka.project.state', fromlist=['artifact_receipts_dir']).artifact_receipts_dir(build_dir)
    if receipts.exists():
