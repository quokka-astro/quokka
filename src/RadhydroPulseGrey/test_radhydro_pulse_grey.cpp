...
/// \brief Defines a test problem for radiation in the advecting diffusion regime with rapidly varying opacity.
///
/// Logs
/// With n=128, the curve is not smooth, but the error is still small.
...
auto problem_main() -> int
{
	...
	// const long int max_timesteps = 1;
	...
	matplotlibcpp::save("./radhydro_pulse_grey_temperature-" + t_str + ".pdf");
	...
	matplotlibcpp::title(fmt::format("time t = {:.4g}", initial_time + sim.tNew_[0]));
	matplotlibcpp::tight_layout();
	matplotlibcpp::save("./radhydro_pulse_grey_density-" + t_str + ".pdf");
	...
	vgas_args["linestyle"] = "-";
	vgas_args["color"] = "k";
	matplotlibcpp::plot(xs, V0vec, vgas_args);
	matplotlibcpp::legend();
	matplotlibcpp::title(fmt::format("time t = {:.4g}", initial_time + sim.tNew_[0]));
	matplotlibcpp::tight_layout();
	matplotlibcpp::save("./radhydro_pulse_grey_velocity-" + t_str + ".pdf");
