...
auto problem_main() -> int
{
	...
	for (int i = 0; i < nx; ++i) {
		...
		double trad_exact = T_equilibrium;
	...
	if (fstream.is_open()) {
		...
		interpolate_arrays(bin_center.data(), F_interp.data(), static_cast<int>(bin_center.size()), nu_exact.data(), Fnu_exact.data(), static_cast<int>(nu_exact.size()));
	...
	// interpolate_arrays(nu_exact.data(), F_interp.data(), static_cast<int>(nu_exact.size()), bin_center.data(), F_r_spec.data(), static_cast<int>(bin_center.size()));
