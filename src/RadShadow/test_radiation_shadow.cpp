...
RadSystem<ShadowProblem>::ComputeRosselandOpacity(const double rho,
...
AMRSimulation<ShadowProblem>::setCustomBoundaryConditions(
  ...
  if (i < lo[0]) {
    ...
    const double Egas =
        consVar(lo[0], j, k, RadSystem<ShadowProblem>::gasEnergy_index);
    const double rho =
        consVar(lo[0], j, k, RadSystem<ShadowProblem>::gasDensity_index);
    const double px =
...
template <>
void RadhydroSimulation<ShadowProblem>::ErrorEst(int lev,
...
auto problem_main() -> int {
  ...
  for (int n = 0; n < nvars; ++n) {
    boundaryConditions[n].setLo(0,
                                amrex::BCType::ext_dir); // left x1 -- streaming
  ...
  // (if epsilon is smaller than tolerance, there can be unphysical radiation
  // shocks.)
  ...
  const auto epsilon = 4.0 * a_rad * (T_initial * T_initial * T_initial) *
                       sigma0 * (c * dt_CFL) / c_v;
  amrex::Print() << "radiation epsilon (stiffness parameter) = " << epsilon
                 << "\n";

  // radiation-matter implicit solver must have a tolerance near machine
  // precision!
