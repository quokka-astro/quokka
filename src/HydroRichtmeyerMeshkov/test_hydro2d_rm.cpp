...
template <>
void RadhydroSimulation<RichtmeyerMeshkovProblem>::computeAfterTimestep() {
...
auto problem_main() -> int {
  ...
  auto isNormalComp = [=](int n, int dim) {
    ...
    if ((n == HydroSystem<RichtmeyerMeshkovProblem>::x2Momentum_index) &&
        (dim == 1)) {
      return true;
    }
    if ((n == HydroSystem<RichtmeyerMeshkovProblem>::x3Momentum_index) &&
        (dim == 2)) {
      return true;
    }
    return false;
