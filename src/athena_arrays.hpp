...
template <typename T> class AthenaArray
{
	...
	AthenaArray()
	    : pdata_(nullptr), nx1_(0), nx2_(0), nx3_(0), nx4_(0), nx5_(0),
	      nx6_(0), state_(DataStatus::empty)
	{
	}
	// ctor overloads: set expected size of unallocated container, maybe
	// allocate (default)
	explicit AthenaArray(int nx1, DataStatus init = DataStatus::allocated)
	    : pdata_(nullptr), nx1_(nx1), nx2_(1), nx3_(1), nx4_(1), nx5_(1),
	      nx6_(1), state_(init)
	{
		AllocateData();
	}
	...
	AthenaArray(int nx3, int nx2, int nx1,
	    ...
	    : pdata_(nullptr), nx1_(nx1), nx2_(nx2), nx3_(nx3), nx4_(1),
	      nx5_(1), nx6_(1), state_(init)
	{
		AllocateData();
	}
	AthenaArray(int nx4, int nx3, int nx2, int nx1,
		    DataStatus init = DataStatus::allocated)
	    : pdata_(nullptr), nx1_(nx1), nx2_(nx2), nx3_(nx3), nx4_(nx4),
	      nx5_(1), nx6_(1), state_(init)
	{
		AllocateData();
	}
	...
	// TODO(felker): replace InitWithShallowSlice with ??? and remove
	// shallow_copy enum val
	// TODO(felker): replace raw pointer with std::vector + reshape (if
	// performance is same)
	...
	operator=(const AthenaArray<T> &t);
	...
	// TODO(felker): Replace this unrestricted "getter" with a limited,
	// safer alternative.
	// TODO(felker): Rename function. Conflicts with "AthenaArray<> data"
	// OutputData member.
	T *data() { return pdata_; }
	const T *data() const { return pdata_; }

	// overload "function call" operator() to access 1d-5d data
	// provides Fortran-like syntax for multidimensional arrays vs.
	// "subscript" operator[]
	...
	// "const variants" called for "const AthenaArray<T>" returns T by
	// value, since T is typically a built-in type (versus "const T &" to
	// avoid copying for general types)
	...
	// int l?, int o?
	T &operator()(const int p, const int m, const int n, const int k,
		      const int j, const int i)
	{
		return pdata_
		    [i +
		     nx1_ *
			 (j + nx2_ * (k + nx3_ * (n + nx4_ * (m + nx5_ * p))))];
	}
	T operator()(const int p, const int m, const int n, const int k,
...
__attribute__((nothrow)) AthenaArray<T>::AthenaArray(const AthenaArray<T> &src)
{
	...
	nx6_ = src.nx6_;
	if (src.pdata_) {
		std::size_t size = (src.nx1_) * (src.nx2_) * (src.nx3_) *
				   (src.nx4_) * (src.nx5_);
		pdata_ = new T[size]; // allocate memory for array data
		for (std::size_t i = 0; i < size; ++i) {
			pdata_[i] =
			    src.pdata_[i]; // copy data (not just addresses!)
					   // into new memory
		}
...
{
	if (this != &src) {
		// setting nxN_ is redundant given the above (unenforced)
		// constraint on allowed usage
		nx1_ = src.nx1_;
		nx2_ = src.nx2_;
		nx3_ = src.nx3_;
		nx4_ = src.nx4_;
...
__attribute__((nothrow)) AthenaArray<T>::AthenaArray(AthenaArray<T> &&src)
{
	...
	nx6_ = src.nx6_;
	if (src.pdata_) {
		// && (src.state_ != DataStatus::allocated){  // (if forbidden
		// to move shallow slices)
		//  ---- >state_ = DataStatus::allocated;

		// Allowing src shallow-sliced AthenaArray to serve as move
		// constructor argument
		state_ = src.state_;
		pdata_ = src.pdata_;
		// remove ownership of data from src to prevent it from free'ing
		// the resources
		src.pdata_ = nullptr;
							     ...
							     int nx1)
{
	...
	nx6_ = 1;
	pdata_ =
	    new T[nx1 * nx2 * nx3](); // allocate memory and initialize to zero
}
							     ...
							     int nx2, int nx1)
{
	...
	pdata_ = new T[nx1 * nx2 * nx3 *
		       nx4](); // allocate memory and initialize to zero
}
...
template <typename T> void AthenaArray<T>::AllocateData()
{
	switch (state_) {
		case DataStatus::empty:
		case DataStatus::shallow_slice: // init=shallow_slice should
						// never be passed to ctor
			break;
		case DataStatus::allocated:
			// allocate memory and initialize to zero
			pdata_ =
			    new T[nx1_ * nx2_ * nx3_ * nx4_ * nx5_ * nx6_]();
			break;
	}
}
