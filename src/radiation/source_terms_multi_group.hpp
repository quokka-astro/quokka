...
AMREX_GPU_HOST_DEVICE auto RadSystem<problem_t>::ComputeJacobianForPureGas(
    ...
    quokka::valarray<double, nGroups_> const &Src, double /*coeff_n*/, quokka::valarray<double, nGroups_> const &tau, double c_v, double cscale,
...
/*
template <typename problem_t>
AMREX_GPU_HOST_DEVICE void RadSystem<problem_t>::SolveMatterRadiationEnergyExchange(
	double Egas_guess, quokka::valarray<double, nGroups_> EradVec_guess,
	double rho,
	...
	)
{
	...
	int n = 0;
	for (; n < maxIter; ++n) {
		const auto T_gas = quokka::EOS<problem_t>::ComputeTgasFromEint(rho, Egas_guess, massScalars);
