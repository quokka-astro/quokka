...
template <typename problem_t>
auto RadhydroSimulation<problem_t>::ComputeProjections(const int /*dir*/) const -> std::unordered_map<std::string, amrex::BaseFab<amrex::Real>>
{
	// user should implement
							...
							amrex::YAFluxRegister *fr_as_fine)
{
	...
	for (int retry_count = 0; retry_count <= max_retries; ++retry_count) {
		...
		for (int substep = 0; substep < nsubsteps; ++substep) {
			...
			success = advanceHydroAtLevel(state_old_tmp, fr_as_crse, fr_as_fine, lev, time, dt_step);
			cur_time += dt_step;
							...
							amrex::Real dt_lev) -> bool
{
	...
	// Stage 2 of RK2-SSP
	{
		...
		fillBoundaryConditions(state_inter_, state_inter_, lev, time + dt_lev, PreInterpState,
					PostInterpState);

		// check intermediate state validity
		AMREX_ASSERT(!state_inter_.contains_nan(0, state_inter_.nComp()));
		AMREX_ASSERT(!state_inter_.contains_nan()); // check ghost zones

		auto const &stateOld = state_old_tmp;
		auto const &stateInter = state_inter_;
