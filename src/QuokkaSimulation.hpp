...
template <typename problem_t> void QuokkaSimulation<problem_t>::computeAfterEvolve(amrex::Vector<amrex::Real> & initSumCons)
{
	...
	template <typename problem_t>
	auto QuokkaSimulation<problem_t>::advanceHydroAtLevel(amrex::MultiFab & state_old_cc_tmp,
							      std::array<amrex::MultiFab, AMREX_SPACEDIM> & state_old_fc_tmp, amrex::FluxRegister * fr_as_crse,
							      amrex::FluxRegister * fr_as_fine, amrex::EdgeFluxRegister * emf_as_crse,
							      amrex::EdgeFluxRegister * emf_as_fine, int lev, amrex::Real time, amrex::Real dt_lev) -> bool
	{
		...
		if (integratorOrder_ == 2) {
			...
			if constexpr (Physics_Traits<problem_t>::is_mhd_enabled) {
				for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
					fillBoundaryConditions(state_inter_fc_[idim], state_inter_fc_[idim], lev, time, quokka::centering::fc,
							       quokka::direction{idim}, AMRSimulation<problem_t>::InterpHookNone,
							       AMRSimulation<problem_t>::InterpHookNone, FillPatchType::fillpatch_function);
				}
			}
	...
	template <typename problem_t>
	template <FluxDir DIR>
	void QuokkaSimulation<problem_t>::hydroFluxFunction(
	    amrex::MultiFab & primVar_mf, amrex::MultiFab & cc_bfield_perp_comps_mf, amrex::MultiFab & leftState, amrex::MultiFab & rightState,
	    amrex::MultiFab & leftState_bfield, amrex::MultiFab & rightState_bfield, amrex::MultiFab & flux, amrex::MultiFab & faceVel,
	    amrex::MultiFab & x1FSpds, std::array<amrex::MultiFab, AMREX_SPACEDIM> const &consVar_fc, amrex::MultiFab const &x1Flat,
	    amrex::MultiFab const &x2Flat, amrex::MultiFab const &x3Flat, const int ng_reconstruct, const int nvars)
	{
		...
		if (reconstructionOrder_ == 5) {
			...
			HyperbolicSystem<problem_t>::template ReconstructStatesPLM<DIR, SlopeLimiter::minmod>(primVar_mf, leftState, rightState, ng_reconstruct,
													      nvars);
			if constexpr (Physics_Traits<problem_t>::is_mhd_enabled) {
		...
		if constexpr (Physics_Traits<problem_t>::is_mhd_enabled) {
			...
			HydroSystem<problem_t>::template ComputeFluxes<RiemannSolver::HLLC, DIR>(flux, faceVel, leftState, rightState, leftState_bfield,
												 rightState_bfield, primVar_mf, artificialViscosityK_, nullptr,
												 nullptr, nghost_vel_);
		}
	}
	...
	template <typename problem_t>
	template <FluxDir DIR>
	void QuokkaSimulation<problem_t>::hydroFOFluxFunction(
	    amrex::MultiFab & primVar_mf, amrex::MultiFab & cc_bfield_perp_comps_mf, amrex::MultiFab & leftState, amrex::MultiFab & rightState,
	    amrex::MultiFab & leftState_bfield, amrex::MultiFab & rightState_bfield, amrex::MultiFab & flux, amrex::MultiFab & faceVel,
	    amrex::MultiFab & x1FSpds, std::array<amrex::MultiFab, AMREX_SPACEDIM> const &x1ConsVar_fc_mf, const int ng_reconstruct, const int nvars)
	{
		if constexpr (Physics_Traits<problem_t>::is_mhd_enabled) {
		...
		if constexpr (Physics_Traits<problem_t>::is_mhd_enabled) {
			HydroSystem<problem_t>::template ComputeFluxes<RiemannSolver::LLF_MHD, DIR>(
			    flux, faceVel, leftState, rightState, leftState_bfield, rightState_bfield, primVar_mf, artificialViscosityK_, &x1FSpds,
								      ...
								      int const /*nsubsteps*/, amrex::FluxRegister *fr_as_crse, amrex::FluxRegister *fr_as_fine)
	{
		...
		fillBoundaryConditions(state_new_cc_[lev], state_new_cc_[lev], lev, (time + dt_radiation), quokka::centering::cc, quokka::direction::na,
				       PreInterpState, PostInterpState);
						       ...
						       amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> dx)
	{
		...
		if (radiationReconstructionOrder_ == 5) {
			...
			HyperbolicSystem<problem_t>::template ReconstructStatesPLM<DIR, SlopeLimiter::MC>(primVar.array(), x1LeftState.array(),
		...
		RadSystem<problem_t>::template ComputeFluxes<DIR>(x1Flux.array(), x1FluxDiffusive.array(), x1LeftState.array(), x1RightState.array(),
								  x1FluxRange, consState, dx, use_wavespeed_correction_); // watch out for argument order!!
	}
