						      ...
						      amrex::Real dt_lev) -> bool
{
	...
	if constexpr (Physics_Traits<problem_t>::is_mhd_enabled) {
		...
		MHDSystem<problem_t>::ComputeEMF(ec_emf_components_fo, state_old_cc_tmp, FOfaceVel, state_old_fc_tmp, FOfast_mhd_wavespeeds, emfReconstructionOrder_, emfAveragingType_, emf_scheme_);
	}
	...
	// Stage 1 of RK2-SSP
	{
		...
		if constexpr (Physics_Traits<problem_t>::is_mhd_enabled) {
			...
			MHDSystem<problem_t>::ComputeEMF(ec_emf_components_rk_stage1, stateOld_cc, faceVel, stateOld_fc, fast_mhd_wavespeeds, emfReconstructionOrder_, emfAveragingType_, emf_scheme_);
		}
	...
	if (integratorOrder_ == 2) {
		...
		if constexpr (Physics_Traits<problem_t>::is_mhd_enabled) {
			...
			MHDSystem<problem_t>::ComputeEMF(ec_emf_components_rk_stage2, stateInter_cc, faceVel, stateInter_fc, fast_mhd_wavespeeds, emfReconstructionOrder_, emfAveragingType_, emf_scheme_);
		}
