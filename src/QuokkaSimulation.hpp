...
template <typename problem_t> class QuokkaSimulation : public AMRSimulation<problem_t>
{
	...
	void initialize()
	{
		...
		void addFluxArrays(std::array<amrex::MultiFab, AMREX_SPACEDIM> & dstfluxes, std::array<amrex::MultiFab, AMREX_SPACEDIM> & srcfluxes,
		...
		auto addStrangSplitSourcesWithBuiltin(amrex::MultiFab & state, std::array<amrex::MultiFab, AMREX_SPACEDIM> & state_fc,
						      std::array<amrex::MultiFab, AMREX_SPACEDIM> * recal_fluxes, int lev, amrex::Real time, amrex::Real dt_lev)
		    -> bool;
		...
		void advanceRadiationForwardEuler(int lev, amrex::Real time, amrex::Real dt_radiation, int iter_count, int nsubsteps,
						  amrex::FluxRegister *fr_as_crse, amrex::FluxRegister *fr_as_fine, amrex::MultiFab &state_out);
		void advanceRadiationMidpointRK2(int lev, amrex::Real time, amrex::Real dt_radiation, int iter_count, int nsubsteps,
						 amrex::FluxRegister *fr_as_crse, amrex::FluxRegister *fr_as_fine, amrex::MultiFab &state_inter);
		...
		auto computeHydroFluxes(amrex::MultiFab const &consVar_cc, std::array<amrex::MultiFab, AMREX_SPACEDIM> const &consVar_fc, int nvars,
		...
		template <FluxDir DIR>
		void hydroFluxFunction(amrex::MultiFab & primVar, amrex::MultiFab & cc_bfield_perp_comps_mf, amrex::MultiFab & leftState,
		...
		void hydroFOFluxFunction(amrex::MultiFab & primVar, amrex::MultiFab & cc_bfield_perp_comps_mf, amrex::MultiFab & leftState,
					 amrex::MultiFab & rightState, amrex::MultiFab & leftState_bfield, amrex::MultiFab & rightState_bfield,
					 amrex::MultiFab & x1Flux, amrex::MultiFab & x1FaceVel, amrex::MultiFab & x1FSpds,
					 std::array<amrex::MultiFab, AMREX_SPACEDIM> const &x1ConsVar_fc_mf, int ng_reconstruct_total, int nvars,
					 int nghost_Riemann, amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> dx);
	...
	template <typename problem_t> void AMRSimulation<problem_t>::initializeSimulationMetadata()
	{
		if constexpr (Physics_Traits<problem_t>::unit_system == UnitSystem::CONSTANTS) {
			// if unit system is CONSTANTS, the units are not well defined unless all four constants, G, k_B, c, and a_rad, are defined. However, in
			// a hydro simulation, only k_B is defined. In a radiation-hydrodynamics simulation, only k_B, c, and a_rad are defined. Besides,
			// CONSTANTS is only used for testing purposes, so we don't care about the units in that case.
			...
			if constexpr (Physics_Traits<problem_t>::unit_system == UnitSystem::CGS) {
				...
				k_B = C::k_B /
				      (Physics_Traits<problem_t>::unit_length * Physics_Traits<problem_t>::unit_length * Physics_Traits<problem_t>::unit_mass /
				       ...
				       Physics_Traits<problem_t>::unit_temperature);
			}
	...
	template <typename problem_t> void QuokkaSimulation<problem_t>::readParmParse()
	{
		...
		// set MHD runtime parameters
		{
			...
			if constexpr (Physics_Traits<problem_t>::resistivity_model == ResistivityModel::constant) {
				...
				AMREX_ALWAYS_ASSERT_WITH_MESSAGE(!resistivity_key_present,
								 "mhd.resistivity is set in the input file, but this problem's "
		...
		// set photochemistry runtime parameters
		{
			...
			hpp.query("min_density_allowed",
				  min_density_allowed); // don't do photochemistry in cells with densities below the minimum density specified
		}
#endif
		...
		bool cooling_table_include_pe = false;
		{
			...
			if constexpr (quokka::EOS<problem_t>::is_tabulated) {
				...
				if ((enablePhotoChemistry_ == 1) && (enableCooling_ == 1)) {
					amrex::Abort(
					    "photochemistry.enabled = 1 and cooling.enabled = 1 cannot be used together with the EOSTabulated EOS backend. "
		...
		// set photoelectric heating runtime parameters
		{
			...
			if (use_sfh_based_pe_heating_) {
				AMREX_ALWAYS_ASSERT_WITH_MESSAGE(!cooling_table_include_pe,
								 "When use_sfh_based_pe_heating is set to true, please use a Grackle cooling table that does "
								 "NOT include photoelectric heating.");
				AMREX_ALWAYS_ASSERT_WITH_MESSAGE(
				...
				if (const_sfr_Msun_per_year_per_kpc2_ < 0.0) {
					AMREX_ALWAYS_ASSERT_WITH_MESSAGE(
					    (sfh_interval_ > 0) || (sfh_time_interval_ > 0),
					...
					AMREX_ALWAYS_ASSERT_WITH_MESSAGE(
					    sf_area_kpc2_ > 0.0, "When use_sfh_based_pe_heating is set to true, sf_area_kpc2 must be set to a positive value");
				} else {
	...
	template <typename problem_t> void QuokkaSimulation<problem_t>::createInitialStochasticStellarPopParticles()
	{
		...
		// StochasticStellarPop particles are created on-the-fly from fluid cells. The user can optionally implement this function to create particles
		// at the beginning of the simulation. note: an implementation is only effective if StochasticStellarPop_particles are used
	}
									   ...
									   amrex::Real dt) -> bool
	{
		auto const applyDust = [&]() {
			if constexpr (Physics_Traits<problem_t>::is_dust_enabled && Physics_Traits<problem_t>::is_mhd_enabled) {
				DustSources<problem_t>::computeDustDragAndLorentz(state, state_fc, dt, dust_omega_drag_, dust_omega_res_,
	...
	template <typename problem_t>
	void QuokkaSimulation<problem_t>::refineGrid(int /*lev*/, amrex::TagBoxArray & /*tags*/, amrex::Real /*time*/, int /*ngrow*/)
	{
		// default empty implementation
		// user should implement using problem-specific template specialization
	}
	...
	template <typename problem_t>
	void QuokkaSimulation<problem_t>::applyPoissonGravityAtLevel(amrex::MultiFab const &phi_mf, const int lev, const amrex::Real dt)
	{
#if (AMREX_SPACEDIM == 3)
	...
	template <typename problem_t> void QuokkaSimulation<problem_t>::updateInitialMagneticEnergyFromFaceField()
	{
		...
		for (int lev = 0; lev <= finest; ++lev) {
			...
			if (lev < static_cast<int>(state_old_cc_.size())) {
				...
				state_old_cc_[lev].ParallelCopy(state_new_cc_[lev], dest_comp, dest_comp, 1, state_old_cc_[lev].nGrow(),
									 ...
									 amrex::EdgeFluxRegister *emf_as_fine)
	{
		...
		auto restoreHydroState = [&]() {
			...
			if constexpr (Physics_Traits<problem_t>::is_mhd_enabled) {
				for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
					amrex::Copy(state_new_fc_[lev][idim], accepted_state_fc[idim], 0, 0, Physics_Indices<problem_t>::nvarPerDim_fc,
						    nghost_fc_);
				}
			}
		};
		...
		auto updateAcceptedHydroState = [&]() {
			...
			if constexpr (Physics_Traits<problem_t>::is_mhd_enabled) {
				for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
					amrex::Copy(accepted_state_fc[idim], state_new_fc_[lev][idim], 0, 0, Physics_Indices<problem_t>::nvarPerDim_fc,
						    nghost_fc_);
				}
			}
		};
		...
		while (completed_units < max_total_substeps && cur_retry_level <= max_retries) {
			...
			if (cur_retry_level > 0 && Verbose()) {
				amrex::Print() << "\t>> Re-trying hydro advance at level " << lev
					       << " with reduced timestep (remaining dt = " << dt_attempt_remaining << ", total substeps = " << total_substeps
					       << ", completed substeps = " << start_substep << ", dt_new = " << dt_substep << ")\n";
			}
	...
	auto QuokkaSimulation<problem_t>::advanceHydroAtLevel(amrex::MultiFab &state_old_cc_tmp, std::array<amrex::MultiFab, AMREX_SPACEDIM> &state_old_fc_tmp,
							      ...
							      amrex::EdgeFluxRegister *emf_as_crse, amrex::EdgeFluxRegister *emf_as_fine, int lev,
	...
	template <typename problem_t>
	void QuokkaSimulation<problem_t>::replaceFluxes(std::array<amrex::MultiFab, AMREX_SPACEDIM> &fluxes,
	    ...
	    -> std::tuple<std::array<amrex::MultiFab, AMREX_SPACEDIM>, std::array<amrex::MultiFab, AMREX_SPACEDIM>, std::array<amrex::MultiFab, AMREX_SPACEDIM>>
	{
		...
		AMREX_D_TERM(hydroFluxFunction<FluxDir::X1>(primVar, cc_bfield_perp_comps, leftState[0], rightState[0], leftState_bfield[0],
							    rightState_bfield[0], flux[0], facevel[0], fast_mhd_wavespeeds[0], consVar_fc, flatCoefs[0],
							    flatCoefs[1], flatCoefs[2], reconstructGhost, nvars, nghost_Riemann, dx);
			     , hydroFluxFunction<FluxDir::X2>(primVar, cc_bfield_perp_comps, leftState[1], rightState[1], leftState_bfield[1],
							      rightState_bfield[1], flux[1], facevel[1], fast_mhd_wavespeeds[1], consVar_fc, flatCoefs[0],
							      flatCoefs[1], flatCoefs[2], reconstructGhost, nvars, nghost_Riemann, dx);
			     , hydroFluxFunction<FluxDir::X3>(primVar, cc_bfield_perp_comps, leftState[2], rightState[2], leftState_bfield[2],
	    ...
	    -> std::tuple<std::array<amrex::MultiFab, AMREX_SPACEDIM>, std::array<amrex::MultiFab, AMREX_SPACEDIM>, std::array<amrex::MultiFab, AMREX_SPACEDIM>>
	{
		...
		const auto &dx = geom[lev].CellSizeArray();
		AMREX_D_TERM(
		    hydroFOFluxFunction<FluxDir::X1>(primVar, cc_bfield_perp_comps, leftState[0], rightState[0], leftState_bfield[0], rightState_bfield[0],
	...
	void QuokkaSimulation<problem_t>::hydroFOFluxFunction(amrex::MultiFab &primVar_mf, amrex::MultiFab &cc_bfield_perp_comps_mf, amrex::MultiFab &leftState,
							      amrex::MultiFab &rightState, amrex::MultiFab &leftState_bfield,
		...
		for (int i = 0; i < nsubSteps; ++i) {
			...
			if constexpr (IMEX_Aim_22 > 0.0) {
				...
				for (amrex::MFIter iter(state_tmp1_cc); iter.isValid(); ++iter) {
					...
					RadSystem<problem_t>::SetRadEnergySource(radEnergySource_arr, indexRange, dx, prob_lo, prob_hi,
					...
					if constexpr (Physics_Traits<problem_t>::nGroups <= 1) {
						RadSystem<problem_t>::AddSourceTermsSingleGroup(stateTmp1, radEnergySource_arr, indexRange, dt_stage2_implicit,
												1.0, dustGasInteractionCoeff_, rad_tol, rad_tol_rel, tempFloor,
	...
	void QuokkaSimulation<problem_t>::advanceRadiationForwardEuler(int lev, amrex::Real time, amrex::Real dt_radiation, int const /*iter_count*/,
								       int const /*nsubsteps*/, amrex::FluxRegister *fr_as_crse,
								       amrex::FluxRegister *fr_as_fine, amrex::MultiFab &state_out)
	{
		...
		for (amrex::MFIter iter(state_out); iter.isValid(); ++iter) {
			...
			if (do_reflux) {
				...
				for (int dir = 0; dir < AMREX_SPACEDIM; ++dir) {
					refluxFluxes[dir][iter].copy<amrex::RunOn::Device>(expandedFluxes[dir], expandedFluxes[dir].box(), 0,
											   expandedFluxes[dir].box(), 0, refluxFluxes[dir].nComp());
				}
			}
		}
								      ...
								      amrex::MultiFab &state_inter)
	{
		...
		for (amrex::MFIter iter(state_new_cc_[lev]); iter.isValid(); ++iter) {
			...
			RadSystem<problem_t>::AddFluxesRK2(
			    ...
			    {AMREX_D_DECL(fluxDiffusiveArraysOld[0].const_array(), fluxDiffusiveArraysOld[1].const_array(),
					  fluxDiffusiveArraysOld[2].const_array())},
			    {AMREX_D_DECL(fluxDiffusiveArrays[0].const_array(), fluxDiffusiveArrays[1].const_array(), fluxDiffusiveArrays[2].const_array())},
	...
	template <typename problem_t>
	template <FluxDir DIR>
	void QuokkaSimulation<problem_t>::fluxFunction(amrex::Array4<const amrex::Real> const &consState, amrex::FArrayBox &x1Flux,
						       amrex::FArrayBox &x1FluxDiffusive, const amrex::Box &indexRange, const int nvars,
						       amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> dx,
						       std::array<amrex::Array4<const amrex::Real>, AMREX_SPACEDIM> cons_fc)
	{
		int dir = 0;
		...
		if (radiationReconstructionOrder_ == 5) {
			// cell-centered kernel
			HyperbolicSystem<problem_t>::template ReconstructStatesPPM_EP<DIR>(primVar.array(), x1LeftState.array(), x1RightState.array(),
			...
			HyperbolicSystem<problem_t>::template ReconstructStatesPPM<DIR>(primVar.array(), x1LeftState.array(), x1RightState.array(),
											reconstructRange, x1ReconstructRange, nvars);
		} else if (radiationReconstructionOrder_ == 2) {
			// cell-centered kernel writing face-indexed outputs
			HyperbolicSystem<problem_t>::template ReconstructStatesPLM<DIR, SlopeLimiter::sweby>(
			    primVar.array(), x1LeftState.array(), x1RightState.array(), reconstructRange, x1ReconstructRange, nvars);
		...
		RadSystem<problem_t>::template ComputeFluxes<DIR>(x1Flux.array(), x1FluxDiffusive.array(), x1LeftState.array(), x1RightState.array(),
								  x1FluxRange, consState, dx, use_wavespeed_correction_,
