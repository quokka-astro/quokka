...
template <> void QuokkaSimulation<StreamingProblem>::setInitialConditionsOnGrid(quokka::grid const &grid_elem)
{
	...
	amrex::ParallelFor(indexRange, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
		...
		state_cc(i, j, k, HydroSystem<StreamingProblem>::energy_index) = E;
		state_cc(i, j, k, HydroSystem<StreamingProblem>::internalEnergy_index) = E;
		state_cc(i, j, k, HydroSystem<StreamingProblem>::x1Momentum_index) = rho * u;
		...
		if constexpr (Physics_Traits<StreamingProblem>::is_dust_enabled) {
			...
			state_cc(i, j, k, HydroSystem<StreamingProblem>::x1DustMomentum_index) = rho * u;
...
auto problem_main() -> int
{
	...
	for (int i = 0; i < nx; ++i) {
		double const rho = values.at(HydroSystem<StreamingProblem>::density_index)[i];
		double const mom = values.at(HydroSystem<StreamingProblem>::x1Momentum_index)[i];
		double const rho_d = values.at(HydroSystem<StreamingProblem>::dustDensity_index)[i];
		double const mom_d = values.at(HydroSystem<StreamingProblem>::x1DustMomentum_index)[i];
		rho_g_num[i] = rho;
