...
template <> void QuokkaSimulation<HDGalaxy>::preCalculateInitialConditions()
{
	...
	// Cube size is inferred from the file size (matches MHDDisk convention) rather than
	// being hard-coded, so any NxNxN cube generated by fieldgen_mpi works without editing
	// this file.
	...
	if (!isTurbSamplingDone) {
		...
		amrex::Print() << "HDGalaxy init complete\n"
...
template <> void QuokkaSimulation<HDGalaxy>::setInitialConditionsOnGrid(quokka::grid const &grid_elem)
{
	...
	const amrex::Real *turb_vx_tab = userData_.turb_vx_device.data();
	const amrex::Real *turb_vy_tab = userData_.turb_vy_device.data();
	const amrex::Real *turb_vz_tab = userData_.turb_vz_device.data();
	...
	amrex::ParallelFor(indexRange, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
		...
		if (in_disk) {
			...
			dvx_pert = interpolate_turbulence(turb_vx_tab, turb_nx, turb_ny, turb_nz, tx_c, ty_c, tz_c) * turb_rescale;
			dvy_pert = interpolate_turbulence(turb_vy_tab, turb_nx, turb_ny, turb_nz, tx_c, ty_c, tz_c) * turb_rescale;
			dvz_pert = interpolate_turbulence(turb_vz_tab, turb_nx, turb_ny, turb_nz, tx_c, ty_c, tz_c) * turb_rescale;
		}
		vx += dvx_pert;
		vy += dvy_pert;
...
template <> void QuokkaSimulation<HDGalaxy>::computeAfterTimestep()
{
	...
	// Floor guards against divide-by-zero if sn_mass_per_event_msun is misconfigured
	...
	for (int lev = 0; lev <= finest_level; ++lev) {
		...
		if (lev < finest_level) {
			mask = amrex::makeFineMask(state.boxArray(), state.DistributionMap(), state_new_cc_[lev + 1].boxArray(), refRatio(lev), 1, 0);
		} else {
			mask.setVal(1);
		}
		...
		for (amrex::MFIter mfi(state); mfi.isValid(); ++mfi) {
			...
			amrex::ParallelFor(box, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
				...
				const int N_SN = amrex::max(1, static_cast<int>(std::floor(sfe * M_cell / M_cluster_per_SN)));

				// Total ejecta mass: N_SN clusters each contributing M_cluster_per_SN
				const double Mej = static_cast<double>(N_SN) * M_cluster_per_SN;
				...
				for (int di = -stencil_radius; di <= stencil_radius; ++di) {
					for (int dj = -stencil_radius; dj <= stencil_radius; ++dj) {
						for (int dk = -stencil_radius; dk <= stencil_radius; ++dk) {
							if (di * di + dj * dj + dk * dk > stencil_radius * stencil_radius) {
								continue;
							}
							const int ii = i + di;
							const int jj = j + dj;
							...
							if (ii < dlo[0] || ii > dhi[0] || jj < dlo[1] || jj > dhi[1] || kk < dlo[2] || kk > shi[2]) {
								continue;
							}
				...
				for (int di = -stencil_radius; di <= stencil_radius; ++di) {
					for (int dj = -stencil_radius; dj <= stencil_radius; ++dj) {
						for (int dk = -stencil_radius; dk <= stencil_radius; ++dk) {
							...
							const double rho_nb = s(ii, jj, kk, HydroSystem<HDGalaxy>::density_index);
							const double w_mom =
							    have_mass_weight ? (rho_nb / neighbor_mass_sum) : (1.0 / static_cast<double>(N_kernel - 1));
				...
				for (int di = -stencil_radius; di <= stencil_radius; ++di) {
					for (int dj = -stencil_radius; dj <= stencil_radius; ++dj) {
						for (int dk = -stencil_radius; dk <= stencil_radius; ++dk) {
							...
							const double dE_ijk = (Esn + Ekin_ej) / Vsnr_local + (vCOMx * pradx + vCOMy * prady + vCOMz * pradz);
		...
		for (amrex::MFIter mfi(state); mfi.isValid(); ++mfi) {
			...
			amrex::ParallelFor(box, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
				...
				const double c = (px_old * px_old + py_old * py_old + pz_old * pz_old) - 2.0 * rho_new * (Etot_old + dE - Eint_floor);
