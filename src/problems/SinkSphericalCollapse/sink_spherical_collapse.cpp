...
//==============================================================================
/// \file sink_spherical_collapse.cpp
/// \brief Defines a test problem for pressureless spherical collapse with sink particle formation.
///
#include <limits>
...
#include "sink_spherical_collapse.hpp"
...
template <> void QuokkaSimulation<CollapseProblem>::setInitialConditionsOnGrid(quokka::grid const &grid_elem)
{
	...
	amrex::ParallelFor(indexRange, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
		...
		double rho_min = 6.0e-22;
		double rho_max = 6.0e-16;
		double R_sphere = 3.08568e16;
		double R_smooth = 1.54284e15;
...
template <> void QuokkaSimulation<CollapseProblem>::createInitialParticles()
{
	...
	const quokka_sinkparticle::SinkParticleContainer::ParticleInitData pdata = {{particle_mass, 0, 0, 0, 0, 0, 0}}; // {mass vx vy vz amx amy amz}
	SinkParticles->InitRandom(num_particles, iseed, pdata, generate_on_root_rank);
}
