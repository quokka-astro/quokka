...
template <>
AMREX_GPU_HOST_DEVICE auto DustDrag<DustDampingWithExternalForce>::ComputeReciprocalStoppingTime(amrex::Real /*rho_g*/, amrex::GpuArray<amrex::Real, nDustGroups_> /*rho_d*/,
...
auto v_gas_analytic(double t) -> double {
  return (C_GAS_A * std::exp(LAMBDA1 * t) + C_GAS_B * std::exp(LAMBDA2 * t) + C_GAS_D * t + C_GAS_E) / rho_gas;
}
...
auto E_gas_analytic(double t) -> double
{
  ...
  const double E_gas_initial = P_INITIAL / (quokka::EOS_Traits<DustDampingWithExternalForce>::gamma - 1.0) + 0.5 * rho_gas * std::pow(v_gas_analytic(0), 2);
  return E_gas_initial + integral;
}

// add Strang Split source term for constant external force
template <> void QuokkaSimulation<DustDampingWithExternalForce>::addStrangSplitSources(amrex::MultiFab &mf, int lev, amrex::Real time, amrex::Real dt_lev) // NOLINT
{
...
auto problem_main() -> int
{
  ...
  if (amrex::ParallelDescriptor::IOProcessor()) {
    ...
    matplotlibcpp::save("./dust_damping_gas_velocity_external.pdf");
    ...
    matplotlibcpp::tight_layout();
    matplotlibcpp::save("./dust_damping_dust1_velocity_external.pdf");
    ...
    matplotlibcpp::tight_layout();
    matplotlibcpp::save("./dust_damping_dust2_velocity_external.pdf");
    ...
    matplotlibcpp::tight_layout();
    matplotlibcpp::save("./dust_damping_gas_energy_external.pdf");
    ...
    matplotlibcpp::plot(t, rel_v_d1, {{"label", R"($|v_g - v_{d,1}|$)"}, {"color", "orange"}, {"linestyle", "None"}, {"marker", "o"}, {"markerfacecolor", "none"}, {"markersize", "4"}});
    matplotlibcpp::plot(t, rel_v_d2, {{"label", R"($|v_g - v_{d,2}|$)"}, {"color", "blue"}, {"linestyle", "None"}, {"marker", "o"}, {"markerfacecolor", "none"}, {"markersize", "4"}});
    matplotlibcpp::legend();
    matplotlibcpp::xlabel("t");
