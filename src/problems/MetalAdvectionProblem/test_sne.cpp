...
const amrex::GpuArray<amrex::Real, ARR_SIZE> logphi_data{
...
const amrex::GpuArray<amrex::Real, ARR_SIZE> logg_data{
...
const amrex::GpuArray<amrex::Real, ARR_SIZE> z_data{
...
template <> void QuokkaSimulation<MetalProblem>::setInitialConditionsOnGrid(quokka::grid const &grid_elem)
{
	...
	amrex::ParallelFor(indexRange, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
		...
		const Phidm = (prefac * std::log(1. + std::pow(z / R0_Gal, 2)));
		...
		double double P = rho_disk * std::pow(sigma1, 2.0) + rho_halo * std::pow(sigma2, 2.0);
		...
		amrex::ParallelFor(box, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
			...
			for (int n = 0; n < np; ++n) {
				...
				if (x0 < 0.5 * dx[0] && y0 < 0.5 * dx[1] && z0 < 0.5 * dx[2]) {
					...
					state(i, j, k, HydroSystem<MetalProblem>::energy_index) += rho_eint_blast;
					state(i, j, k, HydroSystem<MetalProblem>::internalEnergy_index) += rho_eint_blast;
					state(i, j, k, Physics_Indices<MetalProblem>::pscalarFirstIndex) += scalar_blast;
				}
			}
		});
