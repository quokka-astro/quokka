...
struct DTypeFrontJaff {
};

constexpr double c_hat = C::c_light / 1000.0;

template <> struct quokka::EOS_Traits<DTypeFrontJaff> {
	static constexpr double mean_molecular_weight = 1.0;
	static constexpr double gamma = 5. / 3.;
};

template <> struct Physics_Traits<DTypeFrontJaff> : DefaultPhysicsTraits {
	// cell-centred
...
template <> struct RadSystem_Traits<DTypeFrontJaff> {
...
template <> struct SimulationData<DTypeFrontJaff> {
...
auto compute_effective_radius(amrex::MultiFab const &state_mf, amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> const &dx) -> amrex::Real
{
	...
	reduce_op.eval(state_mf, amrex::IntVect(0), reduce_data, [=] AMREX_GPU_DEVICE(int box_no, int i, int j, int k) noexcept -> amrex::Real {
		const amrex::Real n_HI =
		    state[box_no](i, j, k, HydroSystem<DTypeFrontJaff>::scalar0_index + static_cast<int>(Species::H)) / spmasses[Species::H];
		const amrex::Real n_HII =
		    state[box_no](i, j, k, HydroSystem<DTypeFrontJaff>::scalar0_index + static_cast<int>(Species::Hp)) / spmasses[Species::Hp];
...
{
#ifdef DTYPEFRONT_USE_ROSENBROCK
	amrex::Print() << "DTypeFrontJaff microphysics integrator: Rosenbrock (Rosenbrock tableau " << integrator_rp::rosenbrock_tableau << ": "
		       << rosenbrock_tableau_name(integrator_rp::rosenbrock_tableau) << ")\n";
#else
	amrex::Print() << "DTypeFrontJaff microphysics integrator: VODE\n";
#endif
}
...
template <>
void RadSystem<DTypeFrontJaff>::SetRadEnergySource(array_t &radEnergy, const amrex::Box &indexRange, amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> const &dx,
						   ...
						   amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> const & /*prob_hi*/, amrex::Real /*time*/)
{
	...
	const amrex::Real L_star = Q * RadSystem<DTypeFrontJaff>::GetChemBandQuanta(0);
...
template <> void QuokkaSimulation<DTypeFrontJaff>::preCalculateInitialConditions()
{
...
template <> AMREX_GPU_HOST_DEVICE auto RadSystem<DTypeFrontJaff>::ComputePlanckOpacity(const double /*rho*/, const double /*Tgas*/) -> amrex::Real
{
	return 0.0_rt;
}

template <> AMREX_GPU_HOST_DEVICE auto RadSystem<DTypeFrontJaff>::ComputeFluxMeanOpacity(const double /*rho*/, const double /*Tgas*/) -> amrex::Real
{
	return 0.0_rt;
}

template <> void QuokkaSimulation<DTypeFrontJaff>::setInitialConditionsOnGrid(quokka::grid const &grid_elem)
{
	...
	amrex::ParallelFor(indexRange, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
		for (int g = 0; g < Physics_Traits<DTypeFrontJaff>::nGroups; ++g) {
			state_cc(i, j, k, RadSystem<DTypeFrontJaff>::radEnergy_index + Physics_NumVars::numRadVarsPerGroup * g) = 1.e-99_rt;
			state_cc(i, j, k, RadSystem<DTypeFrontJaff>::x1RadFlux_index + Physics_NumVars::numRadVarsPerGroup * g) = 0.0_rt;
			state_cc(i, j, k, RadSystem<DTypeFrontJaff>::x2RadFlux_index + Physics_NumVars::numRadVarsPerGroup * g) = 0.0_rt;
			state_cc(i, j, k, RadSystem<DTypeFrontJaff>::x3RadFlux_index + Physics_NumVars::numRadVarsPerGroup * g) = 0.0_rt;
		}
		state_cc(i, j, k, RadSystem<DTypeFrontJaff>::gasEnergy_index) = Egas0;
		state_cc(i, j, k, RadSystem<DTypeFrontJaff>::gasDensity_index) = rhotot;
		state_cc(i, j, k, RadSystem<DTypeFrontJaff>::gasInternalEnergy_index) = Egas0;
		state_cc(i, j, k, RadSystem<DTypeFrontJaff>::x1GasMomentum_index) = 0.0_rt;
		state_cc(i, j, k, RadSystem<DTypeFrontJaff>::x2GasMomentum_index) = 0.0_rt;
		state_cc(i, j, k, RadSystem<DTypeFrontJaff>::x3GasMomentum_index) = 0.0_rt;
		for (int nn = 0; nn < NumSpec; ++nn) {
			state_cc(i, j, k, HydroSystem<DTypeFrontJaff>::scalar0_index + nn) =
			    state.xn[nn] * spmasses[nn]; // scalar indices carry partial densities instead of number densities
		}
	});
}

template <> void QuokkaSimulation<DTypeFrontJaff>::computeAfterTimestep()
{
	const int lev = 0;
	...
	const amrex::Real eps = RadSystem<DTypeFrontJaff>::GetChemBandQuanta(0);
...
auto problem_main() -> int
{
	...
	QuokkaSimulation<DTypeFrontJaff> sim;
	...
	// Check 2: temperature in cavity and neutral region at end of simulation
	{
		...
		for (amrex::MFIter mfi(state_mf); mfi.isValid(); ++mfi) {
			...
			amrex::LoopOnCpu(box, [&](int i, int j, int k) noexcept {
				const amrex::Real rho = state(i, j, k, HydroSystem<DTypeFrontJaff>::density_index);
				const amrex::Real Eint = state(i, j, k, RadSystem<DTypeFrontJaff>::gasInternalEnergy_index);
				const amrex::Real n_HI_cell =
				    state(i, j, k, HydroSystem<DTypeFrontJaff>::scalar0_index + static_cast<int>(Species::H)) / spmasses[Species::H];
				const amrex::Real n_HII_cell =
				    state(i, j, k, HydroSystem<DTypeFrontJaff>::scalar0_index + static_cast<int>(Species::Hp)) / spmasses[Species::Hp];
				...
				burn_t bstate;
				for (int nn = 0; nn < NumSpec; ++nn) {
					bstate.xn[nn] = state(i, j, k, HydroSystem<DTypeFrontJaff>::scalar0_index + nn) / spmasses[nn];
				}
				bstate.rho = rho;
				bstate.e = Eint / rho;
