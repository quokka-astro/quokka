...
constexpr amrex::Real gamma_gas = quokka::EOS_Traits<MHDBitwiseICs>::gamma;
			 ...
			 amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> const &prob_lo, quokka::centering cen, quokka::direction dir, amrex::Real time)
{
	...
	if (cen == quokka::centering::cc) {
		...
		state(i, j, k, HydroSystem<MHDBitwiseICs>::density_index) = density;
		state(i, j, k, HydroSystem<MHDBitwiseICs>::x1Momentum_index) = v_x1_prf * density;
		state(i, j, k, HydroSystem<MHDBitwiseICs>::x2Momentum_index) = v_x2_prf * density;
		state(i, j, k, HydroSystem<MHDBitwiseICs>::x3Momentum_index) = v_x3_prf * density;
		state(i, j, k, HydroSystem<MHDBitwiseICs>::energy_index) = Etot;
		state(i, j, k, HydroSystem<MHDBitwiseICs>::internalEnergy_index) = Eint;
		...
		if (dir == quokka::direction::x) {
			state(i, j, k, MHDSystem<MHDBitwiseICs>::bfield_index) = b_x1_L;
		} else if (dir == quokka::direction::y) {
			state(i, j, k, MHDSystem<MHDBitwiseICs>::bfield_index) = b_x2_L;
		} else if (dir == quokka::direction::z) {
			state(i, j, k, MHDSystem<MHDBitwiseICs>::bfield_index) = b_x3_L;
		}
	}
}
...
auto verifyPeriodicBCs(const amrex::MultiFab &mf, const std::string &label) -> int
{
	...
	for (amrex::MFIter mf_iter(mf, false); mf_iter.isValid(); ++mf_iter) {
		const auto &fab_view = mf[mf_iter].const_array();
		...
		for (int axis_x0 = 0; axis_x0 < 3; ++axis_x0) {
			...
			const int axis_x1 = (axis_x0 + 1) % 3;
			const int axis_x2 = (axis_x0 + 2) % 3;
			// loop over boundary layers in this axis
			for (int layer_index = 1; layer_index <= nghosts[axis_x0]; ++layer_index) {
				// compute valid/ghost indices for the two sides along this axis
				const int lower_ghost_x0_coord = valid_lo[axis_x0] - layer_index;
				const int upper_ghost_x0_coord = valid_hi[axis_x0] + layer_index;
				const int lower_valid_x0_coord = valid_hi[axis_x0] - (layer_index - 1);
				const int upper_valid_x0_coord = valid_lo[axis_x0] + (layer_index - 1);
				// iterate the band for the two remaining axes
				for (int x1_coord = valid_lo[axis_x1]; x1_coord <= valid_hi[axis_x1]; ++x1_coord) {
					for (int x2_coord = valid_lo[axis_x2]; x2_coord <= valid_hi[axis_x2]; ++x2_coord) {
						...
						auto create_coord = [&](int index_axis_x0, int index_axis_x1, int index_axis_x2) -> std::array<int, 3> {
							...
							coords[axis_x2] = index_axis_x2;
							return coords;
						};
						// compare both sides (lower/upper) for all components
						const std::array<int, 3> lower_valid_coords = create_coord(lower_valid_x0_coord, x1_coord, x2_coord);
						const std::array<int, 3> lower_ghost_coords = create_coord(lower_ghost_x0_coord, x1_coord, x2_coord);
						const std::array<int, 3> upper_valid_coords = create_coord(upper_valid_x0_coord, x1_coord, x2_coord);
						const std::array<int, 3> upper_ghost_coords = create_coord(upper_ghost_x0_coord, x1_coord, x2_coord);
	...
	if (num_diffs == 0) {
		amrex::Print() << "[" << label << "] boundary-band ghosts match periodic counterparts (abs diff == 0).\n";
	}
	return num_diffs;
}
...
auto problem_main() -> int
{
	...
	int num_diffs_total = 0;
	num_diffs_total += verifyPeriodicBCs(cc_state[amr_level], "cell-centered");
	num_diffs_total += verifyPeriodicBCs(fc_state[amr_level][0], "FC-x");
	num_diffs_total += verifyPeriodicBCs(fc_state[amr_level][1], "FC-y");
	num_diffs_total += verifyPeriodicBCs(fc_state[amr_level][2], "FC-z");

	if (num_diffs_total == 0) {
		amrex::Print() << "Success: ghost-zones in the ICs are bitwise identical to their valid-domain counterparts.\n";
		return 0;
	}
