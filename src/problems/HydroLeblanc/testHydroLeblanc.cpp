								  ...
								  amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> const &prob_lo)
{
	...
	std::string const filename = "../extern/ppm1d/leblanc.dat";
	...
	if (amrex::ParallelDescriptor::IOProcessor()) {
		...
		for (int i = 0; i < nx; ++i) {
			...
			amrex::Real const specific_Eint = Eint / rho;
			amrex::Real const pressure = (quokka::EOS_Traits<ShocktubeProblem>::gamma - 1.) * Eint;
		...
		matplotlibcpp::clf();
		double const msize = 5.0;
		int const s = 8; // stride
