/// \file test_radparticle_3D.cpp
/// \brief Defines a 3D test problem for radiating particles.
///
...
template <> void QuokkaSimulation<ParticleProblem>::createInitialRadParticles()
{
	...
	RadParticles->InitFromAsciiFile("../inputs/RadParticles3D.txt", nreal_extra, nullptr);
}
...
auto problem_main() -> int
{
	...
	const auto dx = sim.Geom(0).CellSize(0);
	const auto dy = sim.Geom(0).CellSize(1);
	const auto dz = sim.Geom(0).CellSize(2);
	...
	amrex::Print() << "Radiation energy (simulation, exact) = (" << total_Erad << ", " << total_Erad_exact << ")\n";

	int status = 1;
	const double rel_err_tol = 5.0e-4;
