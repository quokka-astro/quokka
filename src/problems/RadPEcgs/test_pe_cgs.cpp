...
constexpr double erad_floor = 1.0e-12 * Erad0;
constexpr double initial_T0 = 10.0; // K
constexpr double c = C::c_light;
constexpr double chat = c;
constexpr double kappa0 = 1.0e-5; // opacity
constexpr double kappa1 = 1.0e5; // opacity
constexpr double rho0 = 100.0 * C::m_p; // g/cm^3
constexpr double mu = C::m_p;
constexpr double k_B = C::k_B;
constexpr double initial_Egas = 3. / 2. * rho0 / mu * k_B * initial_T0;
constexpr double x_max = 6.171355163e+16; // cm, = 0.02 pc
constexpr double t_max = 0.5 * x_max / c;
constexpr double dt_max = x_max / 128 / c * 10;
...
template <> void QuokkaSimulation<Problem>::setInitialConditionsOnGrid(quokka::grid const &grid_elem)
{
	...
	amrex::ParallelFor(indexRange, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
		for (int g = 0; g < Physics_Traits<Problem>::nGroups; ++g) {
			state_cc(i, j, k, RadSystem<Problem>::radEnergy_index + Physics_NumVars::numRadVars * g) = erad_floor;
...
auto problem_main() -> int
{
	...
	for (int i = 0; i < nx; ++i) {
		...
		xs.at(i) = x;
		if (with_opacity) {
			erad_exact.at(i) = (x <= chat * tmax) ? Erad0 * std::exp(- rho0 * kappa1 * x) : erad_floor;
		} else {
			erad_exact.at(i) = (x <= chat * tmax) ? Erad0 : erad_floor;
		}
		// double erad_sim = 0.0;
	...
	const double rel_err_tol = 2.0e-2;
	int status = 1;
	...
	matplotlibcpp::ylabel("Erad_1 (erg cm^-3)");
	...
	if (n_group_ > 1) {
		...
		matplotlibcpp::xlabel("x (cm)");
		matplotlibcpp::ylabel("Erad_2 (erg cm^-3)");
		// matplotlibcpp::ylim(-0.1, 1.1);
		matplotlibcpp::legend();
		matplotlibcpp::title(fmt::format("photoelectric heating at t = {:.1f}", sim.tNew_[0]));
	...
	matplotlibcpp::ylim(0.5e1, 1.5e7);
