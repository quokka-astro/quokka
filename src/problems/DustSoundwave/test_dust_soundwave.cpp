...
auto v_gas_analytic(double t) -> double {
	return real_part_analytic(t, re_u_g, im_u_g);
}

auto rho_gas_analytic(double t) -> double {
	return real_part_analytic(t, re_rho_g, im_rho_g);
}

auto v_dust_analytic(double t) -> double {
	return real_part_analytic(t, re_u_d, im_u_d);
}

auto rho_dust_analytic(double t) -> double {
	return real_part_analytic(t, re_rho_d, im_rho_d);
}
...
auto problem_main() -> int
{
  ...
  if ((rel_err_v_gas > rel_err_tol) || (rel_err_rho_gas > rel_err_tol) ||
	  (rel_err_v_dust > rel_err_tol) || (rel_err_rho_dust > rel_err_tol)) {
	  status = 1;
  }
	...
	matplotlibcpp::plot(t_dense, norm_v_gas_dense,
	                    {{"label", "gas (analytic)"}, {"color", "blue"}, {"linestyle", "--"}});
	matplotlibcpp::plot(t_vec_plot, norm_v_dust_plot,
	                    {{"label", "dust (numerical)"}, {"color", "red"}, {"linestyle", "None"}, {"marker", "o"}, {"markersize", "4"}});
	matplotlibcpp::plot(t_dense, norm_v_dust_dense,
	                    {{"label", "dust (analytic)"}, {"color", "red"}, {"linestyle", "--"}});
	...
	matplotlibcpp::plot(t_vec_plot, norm_rho_gas_plot,
	                    {{"label", "gas (numerical)"}, {"color", "blue"}, {"linestyle", "None"}, {"marker", "o"}, {"markersize", "4"}});
	matplotlibcpp::plot(t_dense, norm_rho_gas_dense,
	                    {{"label", "gas (analytic)"}, {"color", "blue"}, {"linestyle", "--"}});
	matplotlibcpp::plot(t_vec_plot, norm_rho_dust_plot,
	                    {{"label", "dust (numerical)"}, {"color", "red"}, {"linestyle", "None"}, {"marker", "o"}, {"markersize", "4"}});
	matplotlibcpp::plot(t_dense, norm_rho_dust_dense,
	                    {{"label", "dust (analytic)"}, {"color", "red"}, {"linestyle", "--"}});
	matplotlibcpp::legend();
	matplotlibcpp::xlabel("Time");
