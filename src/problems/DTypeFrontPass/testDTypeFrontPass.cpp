...
/// dusty hydrogen slab: photoionize.flux_optical into the OPTICAL band, photoionize.flux_ion into the
/// ionizing band, and photoionize.flux_ir into the IR band. Both the radiation energy source and the
/// companion radiation flux source are set, with flux = c * E, so all three inflows are fully beamed along
/// +x. flux_ir defaults to 0, in which case the IR band receives no direct source at all and is filled purely
/// by dust re-emission, as in the original version of this test.
///
/// The two thermal bands (IR, optical) are scaled differently inside the solver than the chemistry band -- a
/// thermal group's source is multiplied by chat/c and a chemistry band's is not -- so the shipped
/// flux_optical/flux_ir fluxes and flux_ion differ by exactly c/chat = 1000 for equal injected energy.
/// Photochemistry is disabled, so the ionizing band is a pure transport channel here; it is transparent to
/// the dust opacity and nothing absorbs it.
///
...
template <> struct RadSystem_Traits<DTypeFront1D> {
	...
	static constexpr amrex::GpuArray<double, Physics_Traits<DTypeFront1D>::nGroups + 1> radBoundaries{0, 1.0e14, 3.29e15, 1.0e19};
...
template <> struct SimulationData<DTypeFront1D> {
	...
	amrex::Real flux_optical{}; // optical photon flux [photons cm^-2 s^-1] injected at x = 0
	amrex::Real flux_ion{};	    // ionizing photon flux [photons cm^-2 s^-1] injected at x = 0
	amrex::Real flux_ir{};	    // IR photon flux [photons cm^-2 s^-1] injected at x = 0
	amrex::Vector<amrex::Real> t_vec_;
	amrex::Vector<amrex::Real> xfront_vec_;
	amrex::Vector<amrex::Real> E_ir_vec_;
	amrex::Vector<amrex::Real> E_optical_vec_;
					   ...
					   amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> const & /*prob_hi*/, amrex::Real /*time*/)
{
	...
	//
	// The IR, optical and ionizing bands are all fed; on top of whatever the dust reprocesses into it, the IR
	// band also receives its own direct beamed source, photoionize.flux_ir (0 by default).
	//
	// The optical and IR sources share the same thermal-group scaling (chat/c, see below), so with
	// flux_ir == 0 the IR band starts dark exactly as before and is filled purely by the dust's own thermal
	// re-emission of the absorbed optical light.
	//
	// The two thermal bands and the chemistry band take different internal scalings: a thermal group's
	// source is multiplied by chat/c, a chemistry band's is not (see source_terms_multi_group.hpp). The
	// shipped optical/ionizing fluxes differ by exactly that factor of c/chat = 1000, so the two bands
	// receive the same injected energy and the energy budget below would be off by three orders of
	// magnitude if either scaling were wrong.
	...
	amrex::GpuArray<amrex::Real, Physics_Traits<DTypeFront1D>::nGroups> src_by_group{0.0_rt};
	src_by_group[group_ir] = flux_ir * E_photon / dx[0];
	src_by_group[group_optical] = flux_optical * E_photon / dx[0];
	src_by_group[group_ionizing] = flux_ion * E_photon / dx[0];

	amrex::ParallelFor(indexRange, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
		for (int g = 0; g < Physics_Traits<DTypeFront1D>::nGroups; ++g) {
			const amrex::Real src = (i == 0) ? src_by_group[g] : 0.0_rt;
...
template <> void QuokkaSimulation<DTypeFront1D>::computeAfterTimestep()
{
	...
	const amrex::Real Erad_threshold = front_threshold_fraction * compute_plateau_erad(userData_.flux_optical);
	...
	userData_.E_ir_vec_.push_back(compute_group_total_erad(state_new_cc_[lev], dx, group_ir));
	userData_.E_optical_vec_.push_back(compute_group_total_erad(state_new_cc_[lev], dx, group_optical));
...
auto problem_main() -> int
{
	...
	const double F = sim.userData_.flux_optical;
	const double F_ion = sim.userData_.flux_ion;
	const double F_ir = sim.userData_.flux_ir;
	...
	// tolerance absorbs the small amount of heat the gas releases on its way to equilibrium.
	{
		...
		// (see source_terms_multi_group.hpp), so the thermal (IR + optical) and ionizing contributions are
		// counted differently here. Every group starts at the radiation floor.
		const double injected =
		    ((c_hat / C::c_light) * (F + F_ir) + F_ion) * E_photon * t_end + Physics_Traits<DTypeFront1D>::nGroups * Erad_floor_ * Lx;
	...
	// Check 4: dust reprocessing. With the default flux_ir = 0, nothing is injected directly into the IR
	// band, so every erg in it arrived there by being absorbed out of the optical beam and thermally
	// re-emitted by the dust; the "reprocessed fraction" below assumes exactly that; and is only meaningful
	// while flux_ir stays 0. Behind the light front the optical band is in steady state and simply
	// attenuates, E_opt(x) = (F E_photon / c) exp(-kappa_opt rho x), because the ~130 K dust emits nothing
	// back into the optical; integrating that to the front gives a closed form for what should be left
	// unprocessed.
	{
	...
	if (amrex::ParallelDescriptor::IOProcessor()) {
		...
		// Domain-integrated IR/optical band energy vs. time, compared against the zero-opacity
		// expectation. Both bands are thermal groups directly sourced by SetRadSource (flux for optical,
		// flux_ir for IR, 0 by default); with zero opacity and reflecting boundaries none of that injected
		// energy would be absorbed, so each band simply accumulates its own source, (chat/c) * flux_g *
		// E_photon * t. In the real (nonzero-opacity) run the IR band also picks up whatever the dust
		// reprocesses out of the optical beam, so with flux_ir == 0 any excess of the numerical E_IR curve
		// above this zero-opacity reference is exactly the reprocessed energy.
		const int n2 = static_cast<int>(sim.userData_.t_vec_.size());
		std::vector<amrex::Real> t_Myr2(n2);
		std::vector<amrex::Real> E_optical_num(n2);
		std::vector<amrex::Real> E_ir_num(n2);
		std::vector<amrex::Real> E_optical_zero_kappa(n2);
		std::vector<amrex::Real> E_ir_zero_kappa(n2);
		for (int i = 0; i < n2; ++i) {
			t_Myr2[i] = sim.userData_.t_vec_[i] / seconds_per_Myr;
			E_optical_num[i] = sim.userData_.E_optical_vec_[i];
			E_ir_num[i] = sim.userData_.E_ir_vec_[i];
			E_optical_zero_kappa[i] = (c_hat / C::c_light) * F * E_photon * sim.userData_.t_vec_[i] + Erad_floor_ * Lx;
			E_ir_zero_kappa[i] = (c_hat / C::c_light) * F_ir * E_photon * sim.userData_.t_vec_[i] + Erad_floor_ * Lx;
		}
		matplotlibcpp::clf();
		std::map<std::string, std::string> optical_args;
		optical_args["label"] = "E_optical (numerical)";
		optical_args["color"] = "C1";
		std::map<std::string, std::string> optical_zero_kappa_args;
		optical_zero_kappa_args["label"] = "E_optical (kappa = 0)";
		optical_zero_kappa_args["color"] = "C3";
		optical_zero_kappa_args["linestyle"] = "--";
		std::map<std::string, std::string> ir_args;
		ir_args["label"] = "E_IR (numerical)";
		ir_args["color"] = "C0";
		std::map<std::string, std::string> ir_zero_kappa_args;
		ir_zero_kappa_args["label"] = "E_IR (kappa = 0)";
		ir_zero_kappa_args["color"] = "C4";
		ir_zero_kappa_args["linestyle"] = "--";
		matplotlibcpp::plot(t_Myr2, E_optical_num, optical_args);
		matplotlibcpp::plot(t_Myr2, E_optical_zero_kappa, optical_zero_kappa_args);
		matplotlibcpp::plot(t_Myr2, E_ir_num, ir_args);
		matplotlibcpp::plot(t_Myr2, E_ir_zero_kappa, ir_zero_kappa_args);
		matplotlibcpp::xlabel("time (Myr)");
		matplotlibcpp::ylabel("domain-integrated radiation energy (erg cm^-2)");
		matplotlibcpp::legend();
		matplotlibcpp::tight_layout();
		matplotlibcpp::save("./dtype_front_pass_bands.pdf");
	}
#endif
