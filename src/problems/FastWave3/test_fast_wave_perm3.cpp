...
#include "test_fast_wave_perm3.hpp"
...
constexpr double magnetosonic_speed = gcem::sqrt(gcem::pow(alfven_speed, 2) + gcem::pow(sound_speed, 2));
constexpr double bg_mag_x1 = 0.0;
constexpr double bg_mag_x2 = bg_mag_amplitude;
constexpr double bg_mag_x3 = 0.0;
...
{
	// return -bg_mag_x3 * x2;
	return -x1 / 2.0 * (bg_mag_amplitude + delta_b * std::cos(omega * time - k_amplitude * x3));
}
AMREX_GPU_DEVICE auto computeMagneticVectorPotential_x(double /*x1*/, double /*x2*/, double x3, double time) -> double
{
	...
	return bg_mag_amplitude * x3 / 2.0 + ((delta_b)*std::sin(omega * time - k_amplitude * x3) / (-2.0 * k_amplitude));
}
AMREX_GPU_DEVICE auto computeMagneticVectorPotential_y(double /*x1*/, double /*x2*/, double /*x3*/, double /*time*/) -> double { return 0.0; }
					  ...
					  double time)
{
	...
	if (cen == quokka::centering::cc) {
		const double cos_wave_C = std::cos(omega * time - k_amplitude * x3_C);

		// magnetic field at the center of the cell
		const double x1mag = 0.0;
		const double x2mag = bg_mag_amplitude + delta_b * cos_wave_C;
		...
		const double x3vel = magnetosonic_speed * delta_b / bg_mag_amplitude * cos_wave_C;
