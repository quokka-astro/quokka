...
auto runWaveTest(int nx) -> double
{
	...
	auto BCs_cc = quokka::BC<MHDAlfvenWaveLinearConvergence>(quokka::BCType::int_dir);

	const int nvars_fc = Physics_Indices<MHDAlfvenWaveLinearConvergence>::nvarTotal_fc;
	...
	// Run simulation
	QuokkaSimulation<MHDAlfvenWaveLinearConvergence> sim(BCs_cc, BCs_fc);
