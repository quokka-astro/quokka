...
void gridftGen(cplx_t *grid, float kmin, float kmax, float kidx)
{
	...
	kidx -= 2;
	for (i = 0; i < nxloc; i++) {
		if (i + xlocstart <= ngridft)
			ieff = i + xlocstart;
		else
			ieff = ngrid - (i + xlocstart);
		for (j = 0; j < ngrid; j++) {
			if (j <= ngridft)
				jeff = j;
			else
	...
	for (i = 0; i < nxloc; i++) {
		...
		for (j = 0; j < ngrid; j++) {
			...
			for (k = 0; k < ngridft; k++) {
				...
				kappa_mag = sqrt(kappa[0] * kappa[0] + kappa[1] * kappa[1] + kappa[2] * kappa[2]);
				for (l = 0; l < 3; l++)
					kappan[l] = kappa[l] / kappa_mag;
				idx = ((ptrdiff_t)i * ngrid + j) * ngridft + k;
...
void normalize(double *grid, float stddev)
{
	...
	MPI_Allreduce(&locpower, &totpower, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
...
void fftinit(fftw_plan *plan, double **grid, double **work)
{
	...
	*plan = fftw_mpi_plan_dft_c2r_3d(ngrid, ngrid, ngrid, (fftw_complex *)(*grid), *grid, MPI_COMM_WORLD, FFTW_ESTIMATE);
}
	...
	while (xptr < ngrid) {
		...
		if (myrank == 0) {
			...
			if (owner == 0) {
				...
				MPI_Recv(gridtmp, thisslab, rowtype, owner, 1, MPI_COMM_WORLD, &status);
				/* Now write it out */
				for (i = 0; i < thisslab; i++) {
					for (j = 0; j < ngrid; j++) {
						idx = ((ptrdiff_t)i * ngrid + j) * 2 * ngridft;
			...
			/* I'm not the root processor, but I own this data, so send it
			   to root */
			MPI_Send(grid, nxloc_i, rowtype, 0, 1, MPI_COMM_WORLD);
		}
...
int main(int argc, char **argv)
{
...
#ifdef VERBOSE
		if (myrank == 0)
			printf("Generating field %d...\n", n);
#endif
		gridftGen((cplx_t *)(grid + n * locsize), kmin, kmax, kidx);
	}
...
#ifdef VERBOSE
	if (myrank == 0)
		printf("Projecting out non-solenoidal components...\n");
#endif
	...
	for (n = 0; n < 3; n++) {
		...
		if (myrank == 0)
			printf("Doing fft..\n");
#endif
		fftw_mpi_execute_dft_c2r(plan, (fftw_complex *)(grid + n * locsize), grid + n * locsize);
	}
