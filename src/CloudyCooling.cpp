...
void readCloudyData(cloudy_tables &cloudyTables) {
  ...
  cloudyTables.log_nH = std::make_unique<amrex::TableData<double, 1>>(
      copy_1d_table(cloudy.grid_parameters[0]));
  cloudyTables.log_Tgas = std::make_unique<amrex::TableData<double, 1>>(
      copy_1d_table(cloudy.grid_parameters[1]));

  cloudyTables.cooling = std::make_unique<amrex::TableData<double, 2>>(
      extract_2d_table(cloudy.cooling_data));
  cloudyTables.heating = std::make_unique<amrex::TableData<double, 2>>(
      extract_2d_table(cloudy.heating_data));
  cloudyTables.mean_mol_weight = std::make_unique<amrex::TableData<double, 2>>(
      extract_2d_table(cloudy.mmw_data));
}
