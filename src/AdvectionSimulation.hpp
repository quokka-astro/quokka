...
template <typename problem_t>
void AdvectionSimulation<problem_t>::computeAfterTimestep() {
  // do nothing -- user should implement using problem-specific template
  // specialization
}

template <typename problem_t>
void AdvectionSimulation<problem_t>::ComputeDerivedVar(int lev,
...
template <typename problem_t>
void AdvectionSimulation<problem_t>::advanceSingleTimestepAtLevel(
  ...
  // registers: one to store the old timestep, and one to store the intermediate
  // stage and final stage. The intermediate stage and final stage re-use the
  // same register.
...
template <typename problem_t>
auto AdvectionSimulation<problem_t>::computeFluxes(
  ...
  AMREX_D_TERM(
      fluxFunction<FluxDir::X1>(consVar, x1Flux.array(), indexRange, nvars);
...
template <typename problem_t>
template <FluxDir DIR>
void AdvectionSimulation<problem_t>::fluxFunction(
    amrex::Array4<const amrex::Real> const &consState,
  ...
  amrex::Box const &x1ReconstructRange =
      amrex::surroundingNodes(reconstructRange, dim);
  ...
  LinearAdvectionSystem<problem_t>::ConservedToPrimitive(
      consState, primVar.array(), ghostRange, nvars);
  ...
  LinearAdvectionSystem<problem_t>::template ComputeFluxes<DIR>(
      x1Flux, x1LeftState.array(), x1RightState.array(), advectionVel,
      x1FluxRange, nvars);
}
