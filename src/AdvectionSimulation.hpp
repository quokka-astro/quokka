...
template<typename problem_t>
void AdvectionSimulation<problem_t>::stageOneRK2SSP(const double dt,
	array_t &consVar, const amrex::Box& indexRange, const int nvars)
{
    // TODO(ben): convert indexRange to cell_range (std::pair<int,int> along x-direction)
	...
	// Stage 1 of RK2-SSP
    {
        ...
        advectionSystem_.ConservedToPrimitive(consVar, std::make_pair(0, dim1));
        advectionSystem_.ReconstructStatesPPM(primVar, x1LeftState, x1RightState, ppm_range, nvars);
        advectionSystem_.ComputeFluxes(cell_range);
        advectionSystem_.PredictStep(cell_range);
    }

	AMREX_ALWAYS_ASSERT_WITH_MESSAGE(
	    advectionSystem_.CheckStatesValid(consVarPredictStep, cell_range),
	    "[stage 1] Non-realizable states produced. This should not happen!");
}

template<typename problem_t>
void AdvectionSimulation<problem_t>::stageTwoRK2SSP(const double dt,
	array_t &consVar, const amrex::Box& indexRange, const int nvars)
{
    // TODO(ben): convert indexRange to cell_range (std::pair<int,int> along x-direction)
	...
	// Stage 2 of RK2-SSP
    {
        ...
        advectionSystem_.ConservedToPrimitive(consVarPredictStep, std::make_pair(0, dim1));
        advectionSystem_.ReconstructStatesPPM(primVar, x1LeftState, x1RightState, ppm_range, nvars);
        advectionSystem_.ComputeFluxes(cell_range);
        advectionSystem_.AddFluxesRK2(consVar, consVarPredictStep);
    }

	AMREX_ALWAYS_ASSERT_WITH_MESSAGE(
	    advectionSystem_.CheckStatesValid(consVar, cell_range),
