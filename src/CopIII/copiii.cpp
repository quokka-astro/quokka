...
template <> struct HydroSystem_Traits<CopIII> {
	static constexpr bool reconstruct_eint = true;
};

template <> struct Physics_Traits<CopIII> {
	// cell-centred
...
template <> struct SimulationData<CopIII> {
...
template <> void RadhydroSimulation<CopIII>::preCalculateInitialConditions()
{
...
template <> void RadhydroSimulation<CopIII>::setInitialConditionsOnGrid(quokka::grid grid_elem)
{
	...
	amrex::ParallelFor(indexRange, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
		...
		state_cc(i, j, k, HydroSystem<CopIII>::density_index) = state.rho;
		state_cc(i, j, k, HydroSystem<CopIII>::x1Momentum_index) = state.rho * vx;
		state_cc(i, j, k, HydroSystem<CopIII>::x2Momentum_index) = state.rho * vy;
		state_cc(i, j, k, HydroSystem<CopIII>::x3Momentum_index) = state.rho * vz;
		state_cc(i, j, k, HydroSystem<CopIII>::internalEnergy_index) = e;

		Real const Egas = RadSystem<CopIII>::ComputeEgasFromEint(state.rho, state.rho*vx, state.rho*vy, state.rho*vz, e);
		state_cc(i, j, k, HydroSystem<CopIII>::energy_index) = Egas;

		for (int nn = 0; nn < NumSpec; ++nn) {
			state_cc(i, j, k, HydroSystem<CopIII>::scalar0_index + nn) =
...
template <> void RadhydroSimulation<CopIII>::ErrorEst(int lev, amrex::TagBoxArray &tags, amrex::Real /*time*/, int /*ngrow*/)
{
	...
	for (amrex::MFIter mfi(state_new_cc_[lev]); mfi.isValid(); ++mfi) {
		...
		const int nidx = HydroSystem<CopIII>::density_index;

		amrex::ParallelFor(box, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {

			Real const rho = state(i, j, k, nidx);
			Real pressure = HydroSystem<CopIII>::ComputePressure(state, i, j, k);
			amrex::GpuArray<Real, Physics_Traits<CopIII>::numMassScalars> massScalars = RadSystem<CopIII>::ComputeMassScalars(state, i, j, k);

			amrex::Real cs = quokka::EOS<CopIII>::ComputeSoundSpeed(rho, pressure, massScalars);
...
template <> void RadhydroSimulation<CopIII>::ComputeDerivedVar(int lev, std::string const &dname, amrex::MultiFab &mf, const int ncomp_cc_in) const
{
	...
	if (dname == "temperature") {
		...
		amrex::ParallelFor(mf, [=] AMREX_GPU_DEVICE(int bx, int i, int j, int k) noexcept {
			...
			Real const rho = state[bx](i, j, k, HydroSystem<CopIII>::density_index);
			amrex::Real Eint = state[bx](i, j, k, HydroSystem<CopIII>::internalEnergy_index);

			amrex::GpuArray<Real, Physics_Traits<CopIII>::numMassScalars> massScalars = RadSystem<CopIII>::ComputeMassScalars(state[bx], i, j, k);

			output[bx](i, j, k, ncomp) = quokka::EOS<CopIII>::ComputeTgasFromEint(rho, Eint, massScalars);
		});
	}
	...
	if (dname == "pressure") {
		...
		amrex::ParallelFor(mf, [=] AMREX_GPU_DEVICE(int bx, int i, int j, int k) noexcept {
			amrex::Real Pgas = HydroSystem<CopIII>::ComputePressure(state[bx], i, j, k);
	...
	if (dname == "velx") {
		...
		amrex::ParallelFor(mf, [=] AMREX_GPU_DEVICE(int bx, int i, int j, int k) noexcept {
			...
			Real const rho = state[bx](i, j, k, HydroSystem<CopIII>::density_index);
			Real const xmom = state[bx](i, j, k, HydroSystem<CopIII>::x1Momentum_index);
...
auto problem_main() -> int
{
	...
	// boundary conditions
	const int ncomp_cc = Physics_Indices<CopIII>::nvarTotal_cc;
	...
	RadhydroSimulation<CopIII> sim(BCs_cc);
