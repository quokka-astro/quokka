...
public:
	...
	TypedMultifab(const amrex::BoxArray& ba, const amrex::DistributionMapping& dm, int nghost)
	    : ba_(ba), dm_(dm), nghost_(nghost) {
	...
	auto array(const amrex::MFIter& mfi) const {
		static_assert(TypeListContains_v<VarType, TL>, 
			      "Variable type not in TypeList");
	...
	template <class VarType>
	auto array(const amrex::MFIter& mfi) {
		static_assert(TypeListContains_v<VarType, TL>, 
			      "Variable type not in TypeList");
	...
	template <class VarType>
	auto arrays(const amrex::Box& box) const {
		static_assert(TypeListContains_v<VarType, TL>, 
			      "Variable type not in TypeList");
		const auto& info = getComponentInfo<VarType>();
	...
	template <class VarType>
	auto arrays(const amrex::Box& box) {
		static_assert(TypeListContains_v<VarType, TL>, 
			      "Variable type not in TypeList");
		auto& info = getComponentInfo<VarType>();
		return info.mf->arrays()[box][info.component_index];
	...
	bool hasComponent() const {
		return component_map_.find(std::type_index(typeid(VarType))) != component_map_.end();
	}
	
	// Get the underlying MultiFab for a component (for AMReX interop)
	template <class VarType>
	amrex::MultiFab& getMultiFab() {
		auto& info = getComponentInfo<VarType>();
		return *info.mf;
	}
	
	template <class VarType>
	const amrex::MultiFab& getMultiFab() const {
		const auto& info = getComponentInfo<VarType>();
		return *info.mf;
	}
...
private:
	...
	template <class OtherTL>
	void extractComponents(const TypedMultifab<OtherTL>& other) {
	...
	template <class OtherTL>
	void extractComponents(TypedMultifab<OtherTL>&& other) {
...
auto makeTypedMultifab(const amrex::BoxArray& ba, const amrex::DistributionMapping& dm, 
		       int nghost, SourceTMs&&... sources) {
