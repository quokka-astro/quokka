...
DataTable::DataTable(const amrex::Vector<amrex::Real>& x_coords,
...
void DataTable::initialize(const amrex::Vector<amrex::Real>& x_coords,
                           const amrex::Vector<amrex::Real>& y_coords,
                           const amrex::Vector<amrex::Vector<amrex::Real>>& data)
{
	...
	x_coords_ = std::make_unique<amrex::TableData<amrex::Real, 1>>(
		amrex::Array<int, 1>{0}, 
		amrex::Array<int, 1>{x_size_ - 1}, 
	...
	y_coords_ = std::make_unique<amrex::TableData<amrex::Real, 1>>(
		amrex::Array<int, 1>{0}, 
		amrex::Array<int, 1>{y_size_ - 1}, 
		amrex::The_Pinned_Arena()
	);
	...
	data_ = std::make_unique<amrex::TableData<amrex::Real, 2>>(
		amrex::Array<int, 2>{0, 0}, 
		amrex::Array<int, 2>{x_size_ - 1, y_size_ - 1}, 
		amrex::The_Pinned_Arena()
	);
...
auto DataTable::const_tables() const -> DataTableGpuConst
{
	...
	DataTableGpuConst tables{
		x_coords_->const_table(),
		y_coords_->const_table(),
		data_->const_table(),
		x_min_,
		x_max_,
		y_min_,
...
bool DataTable::is_initialized() const
{
	return (x_coords_ != nullptr && y_coords_ != nullptr && data_ != nullptr);
}

int DataTable::x_size() const
{
	return x_size_;
}

int DataTable::y_size() const
{
	return y_size_;
}
