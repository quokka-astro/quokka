...
template <class T, class TL>
inline constexpr bool TypeListContains_v = TypeListContains<T, TL>::value;
...
template <class TL1, class TL2>
using TypeListCat_t = typename TypeListCat<TL1, TL2>::type;

// Extract types from TL1 that are in TL2
template <class TL1, class TL2>
struct TypeListIntersection;

template <class TL2>
struct TypeListIntersection<TypeList<>, TL2> {
	using type = TypeList<>;
};

template <class Head, class... Tail, class TL2>
struct TypeListIntersection<TypeList<Head, Tail...>, TL2> {
	using tail_intersection = typename TypeListIntersection<TypeList<Tail...>, TL2>::type;
	using type = std::conditional_t<TypeListContains_v<Head, TL2>,
					 TypeListCat_t<TypeList<Head>, tail_intersection>,
					 tail_intersection>;
};

template <class TL1, class TL2>
using TypeListIntersection_t = typename TypeListIntersection<TL1, TL2>::type;
