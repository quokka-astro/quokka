...
FewModesFT::FewModesFT(const std::string &prefix, int num_modes, const std::vector<std::vector<amrex::Real>> &k_vec, amrex::Real k_peak, amrex::Real sol_weight,
		       amrex::Real t_corr, uint32_t rseed, const amrex::BoxArray &ba, const amrex::DistributionMapping &dm, bool fill_ghosts)
    : num_modes_(num_modes), prefix_(prefix), k_vec_(k_vec), k_peak_(k_peak), sol_weight_(sol_weight), t_corr_(t_corr), fill_ghosts_(fill_ghosts),
      var_hat_real_d_(3 * num_modes), var_hat_imag_d_(3 * num_modes), k_vec_d_(3 * num_modes)
{
