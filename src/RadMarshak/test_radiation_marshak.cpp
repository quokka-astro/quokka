...
template <>
AMREX_GPU_DEVICE auto RadSystem<SuOlsonProblem>::ComputePlanckOpacity(
    const double /*rho*/, const double /*Tgas*/) -> double {
  return kappa;
}

template <>
AMREX_GPU_DEVICE auto RadSystem<SuOlsonProblem>::ComputeRosselandOpacity(
    const double /*rho*/, const double /*Tgas*/) -> double {
  return kappa;
}
...
RadSystem<SuOlsonProblem>::ComputeTgasFromEgas(const double /*rho*/,
...
template <>
AMREX_GPU_DEVICE auto
RadSystem<SuOlsonProblem>::ComputeEgasFromTgas(const double /*rho*/,
                                               const double Tgas) -> double {
  return (alpha_SuOlson / 4.0) * std::pow(Tgas, 4);
}

template <>
AMREX_GPU_DEVICE auto RadSystem<SuOlsonProblem>::ComputeEgasTempDerivative(
    const double /*rho*/, const double Tgas) -> double {
...
AMRSimulation<SuOlsonProblem>::setCustomBoundaryConditions(
  ...
  if (i < 0) {
    ...
    const double E_0 =
        consVar(0, j, k, RadSystem<SuOlsonProblem>::radEnergy_index);
    const double F_0 =
        consVar(0, j, k, RadSystem<SuOlsonProblem>::x1RadFlux_index);
...
auto problem_main() -> int {
  ...
  if (amrex::ParallelDescriptor::IOProcessor()) {
    ...
    for (int i = 0; i < nx; ++i) {
      ...
      const double Erad_t =
          values.at(RadSystem<SuOlsonProblem>::radEnergy_index).at(i);
      Erad.at(i) = Erad_t;
      Trad.at(i) = std::pow(Erad_t / a_rad, 1. / 4.);

      const double Etot_t =
          values.at(RadSystem<SuOlsonProblem>::gasEnergy_index).at(i);
      const double rho =
          values.at(RadSystem<SuOlsonProblem>::gasDensity_index).at(i);
      const double x1GasMom =
          values.at(RadSystem<SuOlsonProblem>::x1GasMomentum_index).at(i);
    ...
    interpolate_arrays(xs.data(), Trad_exact_interp.data(),
                       static_cast<int>(xs.size()), xs_exact.data(),
                       Trad_exact.data(), static_cast<int>(xs_exact.size()));
