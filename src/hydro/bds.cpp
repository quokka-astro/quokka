...
Real cubic_interp_fast(Real v_m1, Real v_0, Real v_p1, Real v_p2) {
    // Fused multiply-adds are generated better if written this way
    return W0 * (v_m1 + v_p2) + W1 * (v_0 + v_p1);
}

void ComputeBDSReconstructionOptimized(
    const MultiFab& input_mf,
    MultiFab& x_L, MultiFab& x_R,
    MultiFab& y_L, MultiFab& y_R,
    MultiFab& z_L, MultiFab& z_R,
    int num_ghost
) {
...
#pragma omp parallel
#endif
    for (MFIter mfi(input_mf, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
        {
            ...
            for(int dz = 0; dz < 3; ++dz) {
                for(int dy = 0; dy < 3; ++dy) {
                    for(int dx = 0; dx < 3; ++dx) {
                        nbr[dz][dy][dx] = src(i + dx - 1, j + dy - 1, k + dz - 1);
                    }
                }
            }
            ...
            Real c[8];      // The corner values
            Real b_min[8];  // Min bound
            Real b_max[8];  // Max bound
            ...
            for (int corn_idx = 0; corn_idx < 8; ++corn_idx) {
                ...
                for (int z_iter = 0; z_iter < 4; ++z_iter) {
                    ...
                    for (int y_iter = 0; y_iter < 4; ++y_iter) {
                        ...
                        Real v0 = src(bi+0, bj+y_iter, bk+z_iter);
                        Real v1 = src(bi+1, bj+y_iter, bk+z_iter);
                        Real v2 = src(bi+2, bj+y_iter, bk+z_iter);
                        Real v3 = src(bi+3, bj+y_iter, bk+z_iter);
            ...
            for(int k=0; k<8; ++k) sum_corners += c[k];
            ...
            for (int iter = 0; iter < MAX_ITER; ++iter) {
                Real sum_curr = 0.0;
                for(int k=0; k<8; ++k) sum_curr += c[k];
                ...
                Real count = 0.0;
                Real mask[8]; // 1.0 if candidate, 0.0 if not

                for(int k=0; k<8; ++k) {
                    // Logic: if (sign > 0) return val > target; else return val < target;
                    bool is_cand = (diff > 0.0) ? (c[k] > target_limit) 
                                                : (c[k] < target_limit);
            ...
            // X Faces (Stride 1)
            xl(i,j,k) = 0.25 * (c[0] + c[2] + c[4] + c[6]); // x=0
            xr(i,j,k) = 0.25 * (c[1] + c[3] + c[5] + c[7]); // x=1

            // Y Faces (Stride 2)
            yl(i,j,k) = 0.25 * (c[0] + c[1] + c[4] + c[5]); // y=0
            yr(i,j,k) = 0.25 * (c[2] + c[3] + c[6] + c[7]); // y=1

            // Z Faces (Stride 4)
            zl(i,j,k) = 0.25 * (c[0] + c[1] + c[2] + c[3]); // z=0
            zr(i,j,k) = 0.25 * (c[4] + c[5] + c[6] + c[7]); // z=1
        });
    }
}
