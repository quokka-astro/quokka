...
void StochasticForcing::init(int rank, const Real *prob_lo,
                             const Real *prob_hi) {
  ...
  amrex::Vector<Real> domain_length(SpectralRank);
  ...
  if (amrex::ParallelDescriptor::IOProcessor()) {
    std::cout << "\nStochastic forcing field:"
              ...
              << alpha[2] << ")"
              << "\n   band width = (" << BandWidth[0] << " " << BandWidth[1]
              << " " << BandWidth[2] << ")"
              << "\n   integral velocity = (" << IntgrVelocity[0] << " "
              << IntgrVelocity[1] << " " << IntgrVelocity[2] << ")"
              << "\n   integral length = (" << IntgrLength[0] << " "
              << IntgrLength[1] << " " << IntgrLength[2] << ")"
              << "\n   integral time = (" << IntgrTime[0] << " " << IntgrTime[1]
              << " " << IntgrTime[2] << ")"
              << "\n   autocorrelation time = (" << AutoCorrlTime[0] << " "
              << AutoCorrlTime[1] << " " << AutoCorrlTime[2] << ")"
              << std::endl;
  }
  ...
  if (amrex::ParallelDescriptor::IOProcessor()) {
    if (verbose) {
      std::cout << "Total number of stochastic forcing modes = " << NumModes
                << std::endl;
    }
    ...
    if (SpectProfile == Plane) {
      ...
      for (i = 1; i <= i2; i++) {
        ...
        if (verbose > 1) {
          std::cout << "i = " << i << ", n = " << i - 1
                    << ", Amplitude = " << Amplitude[0][i - 1] << std::endl;
        }
      }
      ...
      if (SpectralRank > 1) {
        ...
        for (j = 1; j <= j2; j++) {
          for (i = i1; i <= i2; i++) {
            ...
            if (verbose > 1) {
              std::cout << "i = " << i << ", j = " << j << ", n = " << n
                        << ", Amplitude = " << Amplitude[0][n] << std::endl;
            }
            ++n;
          }
        }
      ...
      if (SpectralRank > 1) {
        ...
        for (j = 1; j <= j2; j++) {
          ...
          for (i = i1; i <= i2; i++) {
            ...
            if (verbose > 1) {
              std::cout << "i = " << i << ", a = " << a << ", j = " << j
                        << ", b = " << b << ", x1 = " << x1 << ", x2 = " << x2
                        << ", n = " << n << ", Amplitude = " << Amplitude[0][n]
                        << std::endl;
            }
    ...
    Real norm = 0.0;
    for (n = 0; n < NumModes; n++) {
      norm += 2 * Amplitude[0][n] * Amplitude[0][n];
    }
    norm = 1 / sqrt(norm);
    ...
    for (n = 0; n < NumModes; n++) {
      ...
      if (SpectralRank > 1) {
        Amplitude[1][n] = Amplitude[0][n];
      }
      if (SpectralRank > 2) {
        Amplitude[2][n] = Amplitude[1][n];
      }
    }
    if (verbose) {
      std::cout << "Number of non-zero stochastic forcing modes = "
                << NumNonZeroModes << std::endl;
    }
    ...
    for (int dim = 0; dim < SpectralRank; dim++) {
      ...
      // normalization such that RMS acceleration is V/T for isotropic
      // characteristic scales
      norm = (IntgrVelocity[dim] / IntgrTime[dim]) /
             sqrt(1.0 - 2.0 * SolenoidalWeight +
                  3.0 * SolenoidalWeight * SolenoidalWeight);

      if (verbose) {
        std::cout << "Normalization factor[" << dim << "] = " << norm
                  << std::endl;
      }
    ...
    mt_init(static_cast<unsigned int>(seed));
  ...
  amrex::ParallelDescriptor::Bcast(
      &NumNonZeroModes, 1, amrex::ParallelDescriptor::IOProcessorNumber());
  amrex::ParallelDescriptor::Bcast(
      mask, NumModes, amrex::ParallelDescriptor::IOProcessorNumber());
  ...
  amrex::Vector<int> kvect(SpectralRank);
...
void StochasticForcing::read_params() {
  ...
  if (!done) {
    ...
    case 3:
      ...
      break;
    default:
      if (amrex::ParallelDescriptor::IOProcessorNumber()) {
    ...
    amrex::Vector<int> input_int(SpectralRank);
    ...
    amrex::Vector<Real> input_real(SpectralRank);
    ...
    for (int i = 0; i < SpectralRank; i++) {
      ...
      IntgrTime[i] = (IntgrVelocity[i] > 0.0)
                         ? (IntgrLength[i] / IntgrVelocity[i])
                         : FLT_MAX;
    }
