...
AMRSimulation<ShockCloud>::setCustomBoundaryConditions(const amrex::IntVect &iv, amrex::Array4<Real> const &consVar,
...
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE auto user_rhs(Real /*t*/, quokka::valarray<Real, 1> &y_data,
...
void computeCooling(amrex::MultiFab &mf, const Real dt_in, cloudy_tables &cloudyTables)
{
	...
	amrex::ParallelFor(mf, [=] AMREX_GPU_DEVICE(int bx, int i, int j, int k) noexcept {
		...
		const Real x3Mom = state[bx](i, j, k, HydroSystem<ShockCloud>::x3Momentum_index);
		const Real Egas = state[bx](i, j, k, HydroSystem<ShockCloud>::energy_index);
		const Real Eint_aux = state[bx](i, j, k, HydroSystem<ShockCloud>::internalEnergy_index);

		Real Eint = RadSystem<ShockCloud>::ComputeEintFromEgas(rho, x1Mom, x2Mom, x3Mom, Egas);

		if (!(Eint > 0.)) {
			// this should be impossible, due to the SyncDualEnergy step
			printf("[cooling] rho = %.17e, Eint = %.17e, Eint_aux = %.17e, dt = %.17e\n", rho, Eint,
			       Eint_aux, dt);
			// replace with Eint_aux
			if (Eint_aux > 0.) {
				Eint = Eint_aux;
			}
		}
		...
		quokka::valarray<Real, 1> abstol = {
		    reltol_floor * ComputeEgasFromTgas(rho, T_floor, HydroSystem<ShockCloud>::gamma_, tables)};
		...
		if (nsteps >= maxStepsODEIntegrate) {
			...
			printf("[cooling] max substeps exceeded! rho = %.17e, Eint = %.17e, T = %.17e, cooling "
			       "time = %.17e, dt = %.17e\n",
...
template <>
void RadhydroSimulation<ShockCloud>::addStrangSplitSources(amrex::MultiFab &state, int const lev, amrex::Real time,
							   amrex::Real const dt)
{
...
template <>
void RadhydroSimulation<ShockCloud>::ComputeDerivedVar(int lev, std::string const &dname, amrex::MultiFab &mf,
...
AMREX_GPU_DEVICE AMREX_FORCE_INLINE auto ComputeCellTemp(int i, int j, int k, amrex::Array4<const Real> const &state,
...
template <>
void RadhydroSimulation<ShockCloud>::ErrorEst(int lev, amrex::TagBoxArray &tags, Real /*time*/, int /*ngrow*/)
{
...
template <>
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE auto RadSystem<ShockCloud>::ComputePlanckOpacity(const double /*rho*/,
											  const double /*Tgas*/)
    -> double
{
...
template <>
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE auto RadSystem<ShockCloud>::ComputeRosselandOpacity(const double /*rho*/,
											     const double /*Tgas*/)
    -> double
{
	return 1000.; // cm^2 g^-1
...
template <>
void RadSystem<ShockCloud>::AddSourceTerms(array_t &consVar, arrayconst_t & /*radEnergySource*/,
					   ...
					   amrex::Real dt)
{
	...
	amrex::ParallelFor(indexRange, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
		...
		const double Egas0 = ComputeEintFromEgas(rho, x1GasMom0, x2GasMom0, x3GasMom0, Egastot0);
		...
		if constexpr (gamma_ != 1.0) {
			...
			amrex::Real const Egastot1 = ComputeEgasFromEint(rho, x1GasMom1, x2GasMom1, x3GasMom1, Egas0);
