...
//==============================================================================
/// \file DataTableCooling.cpp
/// \brief Implements methods for interpolating cooling rates from data tables using DataTable wrapper.
///
...
{
	amrex::Print() << "Initializing DataTable cooling.\n";
	amrex::Print() << fmt::format("data_table_file: {}.\n", hdf5_file);
	...
	AMREX_ALWAYS_ASSERT_WITH_MESSAGE(file_id != h5_error, "Failed to open DataTable cooling data file!");
	...
	status = H5Aread(attr_id, H5T_NATIVE_DOUBLE, &dataTableTables.rho_min);
	...
	status = H5Aread(attr_id, H5T_NATIVE_DOUBLE, &dataTableTables.rho_max);
	...
	status = H5Aread(attr_id, H5T_NATIVE_DOUBLE, &dataTableTables.eint_min);
	...
	status = H5Aread(attr_id, H5T_NATIVE_DOUBLE, &dataTableTables.eint_max);
	...
	status = H5Aread(attr_id, H5T_NATIVE_DOUBLE, &dataTableTables.cloudy_H_mass_fraction);
	...
	read2DDataset("/data/cooling_rates", dataTableTables.cooling_rates);
	read2DDataset("/data/temperatures", dataTableTables.temperatures);
	read2DDataset("/data/sound_speeds", dataTableTables.sound_speeds);
	read2DDataset("/data/pressures", dataTableTables.pressures);
	read2DDataset("/data/entropies", dataTableTables.entropies);

	H5Fclose(file_id);

	amrex::Print() << fmt::format("\tDensity range: {} to {} g/cm^3 ({} steps).\n", dataTableTables.rho_min, dataTableTables.rho_max, n_rho);
	amrex::Print() << fmt::format("\tSpecific energy range: {} to {} erg/g ({} steps).\n", dataTableTables.eint_min, dataTableTables.eint_max, n_eint);
}
