...
template <typename T> void Gravity<T>::read_params()
{
	...
	if (!done) {
		static_assert(gravity::gravity_type == GravityMode::Constant || gravity::gravity_type == GravityMode::Poisson);
		...
		if (n_abs_tol <= 1) {
			...
			for (int lev = 1; lev < nlevs; ++lev) {
				abs_tol[lev] = abs_tol[lev - 1] * std::pow(sim->refRatio(lev - 1)[0], 2);
			}
			...
			amrex::Abort("If you are providing multiple values for abs_tol, you must provide "
		...
		if (n_rel_tol <= 1) {
			...
			amrex::Abort("If you are providing multiple values for rel_tol, you must provide "
				     "at least one value for every level up to amr.max_level.");
		}

		Ggravity = 4.0 * M_PI * C::Gconst;
		if (gravity::verbose > 1) {
			amrex::Print() << "Getting Gconst from constants: " << C::Gconst << std::endl;
			amrex::Print() << "Using " << Ggravity << " for 4 pi G in Gravity.cpp " << std::endl;
		}
...
template <typename T> void Gravity<T>::install_level(int level)
{
	...
	if (gravity::gravity_type == GravityMode::Poisson) {
		...
		for (int n = 0; n < AMREX_SPACEDIM; ++n) {
			grad_phi_prev[level][n] = std::make_unique<MultiFab>(amrex::convert(sim->boxArray(level), IntVect::TheDimensionVector(n)), dm, 1, 0);
			grad_phi_prev[level][n]->setVal(0.);
		}

		grad_phi_curr[level].resize(AMREX_SPACEDIM);
		for (int n = 0; n < AMREX_SPACEDIM; ++n) {
			grad_phi_curr[level][n] = std::make_unique<MultiFab>(amrex::convert(sim->boxArray(level), IntVect::TheDimensionVector(n)), dm, 1, 0);
...
template <typename T> auto Gravity<T>::get_gravity_type() -> gravity::GravityMode { return gravity::gravity_type; }

template <typename T> auto Gravity<T>::get_max_solve_level() -> int { return gravity::max_solve_level; }

template <typename T> auto Gravity<T>::NoSync() -> int { return gravity::no_sync; }

template <typename T> auto Gravity<T>::DoCompositeCorrection() -> int { return gravity::do_composite_phi_correction; }
...
template <typename T> void Gravity<T>::solve_for_phi(int level, MultiFab &phi, const amrex::Vector<MultiFab *> &grad_phi, int is_new)
{
	...
	if (gravity::verbose != 0) {
		...
		amrex::Print() << "Gravity<T>::solve_for_phi() time = " << end << std::endl << std::endl;
	}
}

// This is used to obtain boundary conditions at coarse-fine interfaces for
// level solves
template <typename T> void Gravity<T>::GetCrsePhi(int level, MultiFab &phi_crse, Real time)
{
	...
	phi_crse.clear();
	phi_crse.define(sim->boxArray(level - 1), sim->DistributionMap(level - 1), 1, 1);
...
template <typename T> void Gravity<T>::multilevel_solve_for_new_phi(int level, int finest_level_in)
{
	...
	for (int lev = level; lev <= finest_level_in; lev++) {
		...
		for (int n = 0; n < AMREX_SPACEDIM; ++n) {
			grad_phi_curr[lev][n] =
			    std::make_unique<MultiFab>(amrex::convert(sim->boxArray(lev), IntVect::TheDimensionVector(n)), sim->DistributionMap(lev), 1, 0);
		}
	}
...
void Gravity<T>::actual_multilevel_solve(int crse_level, int finest_level_in, const amrex::Vector<amrex::Vector<amrex::MultiFab *>> &grad_phi, int is_new)
{
	...
	if (fine_level >= crse_level) {
		...
		for (int amr_lev = fine_level; amr_lev > crse_level; amr_lev--) {
			...
			if (is_new == 1) {
				amrex::average_down(phi_new_[amr_lev], phi_new_[amr_lev - 1], 0, 1, ratio);
			} else if (is_new == 0) {
				amrex::average_down(phi_old_[amr_lev], phi_old_[amr_lev - 1], 0, 1, ratio);
			}
		}
	...
	for (int amr_lev = gravity::max_solve_level + 1; amr_lev <= finest_level_in; amr_lev++) {
		...
		for (int n = 0; n < AMREX_SPACEDIM; ++n) {
			amrex::InterpFromCoarseLevel(*grad_phi[amr_lev][n], time, *grad_phi[amr_lev - 1][n], 0, 0, 1, sim->Geom(amr_lev - 1),
						     sim->Geom(amr_lev), gp_phys_bc, 0, gp_phys_bc, 0, sim->refRatio(amr_lev - 1), gp_interp, gp_bcs, 0);
		}
	}
}

template <typename T> void Gravity<T>::average_fine_ec_onto_crse_ec(int level, int is_new)
{
	...
	for (int n = 0; n < AMREX_SPACEDIM; ++n) {
		...
		crse_gphi_fine[n] = std::make_unique<MultiFab>(eba, sim->DistributionMap(level + 1), 1, 0);
	}

	auto &grad_phi = (is_new) != 0 ? grad_phi_curr : grad_phi_prev;

	amrex::average_down_faces(amrex::GetVecOfConstPtrs(grad_phi[level + 1]), amrex::GetVecOfPtrs(crse_gphi_fine), fine_ratio);
...
template <typename T> auto Gravity<T>::get_rhs(int crse_level, int nlevs, int is_new) -> amrex::Vector<std::unique_ptr<MultiFab>>
{
	...
	for (int ilev = 0; ilev < nlevs; ++ilev) {
		...
		rhs[ilev] = std::make_unique<MultiFab>(sim->boxArray(amr_lev), sim->DistributionMap(amr_lev), 1, 0);

		MultiFab &state = (is_new == 1) ? *(sim->getStateNew(amr_lev)) : *(sim->getStateOld(amr_lev));
    ...
    -> Real
{
	...
	return actual_solve_with_mlmg(crse_level, fine_level, phi, crhs, gp, res, crse_bcdata, rel_eps, abs_eps);
}

template <typename T>
auto Gravity<T>::actual_solve_with_mlmg(int crse_level, int fine_level, const amrex::Vector<amrex::MultiFab *> &phi,
					const amrex::Vector<const amrex::MultiFab *> &rhs,
					const amrex::Vector<std::array<amrex::MultiFab *, AMREX_SPACEDIM>> &grad_phi,
					const amrex::Vector<amrex::MultiFab *> &res, const amrex::MultiFab *const crse_bcdata, amrex::Real rel_eps,
