...
template <typename problem_t> class RadSystem : public HyperbolicSystem<problem_t>
{
	...
	template <FluxDir DIR>
	static void ReconstructStatesPPM(arrayconst_t &q, array_t &leftState, array_t &rightState, amrex::Box const &cellRange, const int iReadFrom,
					 const int iWriteFrom, amrex::Box const &interfaceRange, int nvars);
...
void RadSystem<problem_t>::ReconstructStatesPPM(arrayconst_t &q_in, array_t &leftState_in, array_t &rightState_in, amrex::Box const &cellRange,
						const int iReadFrom, const int iWriteFrom, amrex::Box const &interfaceRange, const int nvars)
{
	...
	amrex::ParallelFor(indexRange, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
		...
		if constexpr (gamma_ != 1.0) {
			...
			for (n = 0; n < maxIter; ++n) {
				...
				// check if converged
				if ((std::abs(F_G / Etot0) < resid_tol) && (std::abs(((c / chat) * F_R) / Etot0) < resid_tol)) {
					break;
				}
