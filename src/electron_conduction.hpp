...
struct ElectronConductionParams {
	amrex::Real conductivity_prefactor = 0.0; // kappa = prefactor * T^(5/2)
	amrex::Real flux_limiter_phi = 1.0;
...
template <typename problem_t> class ElectronConduction
{
				    ...
				    amrex::Real dt, ElectronConductionParams const &params)
	{
		...
		amrex::ParallelFor(state, [=] AMREX_GPU_DEVICE(int bx, int i, int j, int k) noexcept {
			...
			const amrex::Real Eint_old = Egas_old - Ekin - Emag;
			...
			if (t_min > 0.0) {
				if constexpr (HydroSystem<problem_t>::nmscalars_ > 0) {
					amrex::GpuArray<amrex::Real, HydroSystem<problem_t>::nmscalars_> massScalars =
					    RadSystem<problem_t>::ComputeMassScalars(state_out[bx], i, j, k);
					Eint_new = amrex::max(Eint_new, quokka::EOS<problem_t>::ComputeEintFromTgas(rho, t_min, massScalars));
				} else {
					Eint_new = amrex::max(Eint_new, quokka::EOS<problem_t>::ComputeEintFromTgas(rho, t_min));
				}
			}
