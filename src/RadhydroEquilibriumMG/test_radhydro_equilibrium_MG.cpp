...
#include "test_radhydro_equilibrium_MG.hpp"
...
constexpr double kappa0 = 1.0e20;	    // cm^2 g^-1
...
template <> void RadhydroSimulation<MGProblem>::setInitialConditionsOnGrid(quokka::grid grid_elem)
{
	...
	amrex::ParallelFor(indexRange, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
		...
		const double Trad = compute_initial_Tgas(r - x0);
		const double rho = compute_exact_rho(r - x0);
...
auto problem_main() -> int
{
	...
	double max_time = 4.8e-5; // max_time = 2 * width / v1;
	// read max_time from input file
	amrex::ParmParse const pp0;
	pp0.query("max_time", max_time);
