...
template<>
struct SimulationData<NewProblem>
{
...
RadhydroSimulation<NewProblem>::GetGradFixedPotential(amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> posvec)
...
template <>
void HydroSystem<NewProblem>::EnforceLimits(
    amrex::Real const densityFloor, 
    amrex::Real const pressureFloor, 
    amrex::Real const speedCeiling, 
    amrex::Real const tempCeiling,
  ...
  amrex::ParallelFor(
      state_mf, [=] AMREX_GPU_DEVICE(int bx, int i, int j, int k) noexcept {
        ...
        amrex::Real const Temp = (gamma_ - 1.) * Eint /
       ...
       if (std::abs(vx1)>speedCeiling){
        ...
        if(Temp>tempCeiling){
          ...
          state[bx](i, j, k, internalEnergy_index) = tempCeiling * 
                                                 (kb * (state[bx](i, j, k, density_index)  / Const_mH/ 0.6)) / (gamma_-1.0);
          state[bx](i, j, k, energy_index) = dummy + state[bx](i, j, k, internalEnergy_index);
        }
        
        if(Temp<tempFloor){
          amrex::Real dummy = Etot - state[bx](i, j, k, internalEnergy_index);
          state[bx](i, j, k, internalEnergy_index) = tempFloor * 
                                                 (kb * (state[bx](i, j, k, density_index)  / Const_mH/ 0.6)) / (gamma_-1.0);
          state[bx](i, j, k, energy_index) = dummy + state[bx](i, j, k, internalEnergy_index);
        }
