...
template <> void RadhydroSimulation<CollapseProblem>::computeAfterTimestep(std::optional<int> global_step)
{
  ...
  if ((plotfileInterval_ > 0) && (step % plotfileInterval_ == 0)) {
    ...
    for (int i = 0; i < nx; ++i) {
      ...
      xs.at(i) = x;
      const auto rho_t = values.at(HydroSystem<CollapseProblem>::density_index)[i];
      const auto v_t = values.at(HydroSystem<CollapseProblem>::x2Momentum_index)[i] / rho_t; // v_y
      const auto Egas_t = values.at(HydroSystem<CollapseProblem>::internalEnergy_index)[i];
      const auto pressure_t = quokka::EOS<CollapseProblem>::ComputePressure(rho_t, Egas_t);
      const auto T_t = quokka::EOS<CollapseProblem>::ComputeTgasFromEint(rho_t, Egas_t);
      rhogas.at(i) = rho_t;
      Tgas.at(i) = T_t;
      Egas.at(i) = Egas_t;
      Vgas.at(i) = v_t;
...
auto problem_main() -> int
{
  ...
  sim.computeAfterTimestep(1);
	...
	for (int i = 0; i < nx; ++i) {
		double rho = values.at(HydroSystem<CollapseProblem>::density_index)[i];
        amrex::Print() << "x " << position[i] << ",," << rho << std::endl;
	}
