...
template <typename T> void Gravity<T>::init_multipole_grav()
{
	...
	for (int l = 0; l <= multipole::lnum_max; ++l) {
		...
		for (int m = 1; m <= l; ++m) {
			...
			multipole::factArray(l, m) = 2.0 * factorial(l - m) / factorial(l + m) * multipole::volumeFactor;
		}
	}
	...
	multipole::rmax = 0.5 * maxWidth * std::sqrt(static_cast<Real>(AMREX_SPACEDIM));
}

template <typename T> void Gravity<T>::fill_multipole_BCs(int crse_level, int fine_level, const amrex::Vector<amrex::MultiFab *> &Rhs, amrex::MultiFab &phi)
{
	...
	amrex::Box boxq0(amrex::IntVect(D_DECL(0, 0, 0)), amrex::IntVect(D_DECL(gravity::lnum, 0, npts - 1)));
	amrex::Box boxqC(amrex::IntVect(D_DECL(0, 0, 0)), amrex::IntVect(D_DECL(gravity::lnum, gravity::lnum, npts - 1)));
	amrex::Box boxqS(amrex::IntVect(D_DECL(0, 0, 0)), amrex::IntVect(D_DECL(gravity::lnum, gravity::lnum, npts - 1)));
	...
	for (int lev = crse_level; lev <= fine_level; ++lev) {
		...
		amrex::MultiFab source(Rhs[lev - crse_level]->boxArray(), Rhs[lev - crse_level]->DistributionMap(), 1, 0);

		amrex::MultiFab::Copy(source, *Rhs[lev - crse_level], 0, 0, 1, 0);

		if (lev < fine_level) {
			auto mask = amrex::makeFineMask(sim->boxArray(lev), sim->DistributionMap(lev), sim->boxArray(lev + 1), sim->refRatio(lev),
							1.0,  // coarse
							0.0); // fine
		...
		for (amrex::MFIter mfi(source); mfi.isValid(); ++mfi) {
			...
			amrex::ParallelFor(amrex::Gpu::KernelInfo().setReduction(true), bx,
					   [=] AMREX_GPU_DEVICE(int i, int j, int k, amrex::Gpu::Handler const &handler) {
						   ...
						   Real rmax_cubed_inv = 1.0 / (multipole::rmax * multipole::rmax * multipole::rmax);

						   Real x = (problo[0] + (static_cast<Real>(i) + 0.5) * dx[0] - coord_center[0]) / multipole::rmax;

						   Real y = (problo[1] + (static_cast<Real>(j) + 0.5) * dx[1] - coord_center[1]) / multipole::rmax;

						   Real z = (problo[2] + (static_cast<Real>(k) + 0.5) * dx[2] - coord_center[2]) / multipole::rmax;
						   ...
						   multipole_add(cosTheta, phiAngle, r, rho(i, j, k), vol * rmax_cubed_inv, qL0_arr, qLC_arr, qLS_arr, qU0_arr,
								 qUC_arr, qUS_arr, npts, nlo, index, handler, true);
					   });
		}
	...
	amrex::ParallelDescriptor::ReduceRealSum(qL0.dataPtr(), static_cast<int>(boxq0.numPts()));
	amrex::ParallelDescriptor::ReduceRealSum(qLC.dataPtr(), static_cast<int>(boxqC.numPts()));
	amrex::ParallelDescriptor::ReduceRealSum(qLS.dataPtr(), static_cast<int>(boxqS.numPts()));
	...
	for (amrex::MFIter mfi(phi); mfi.isValid(); ++mfi) {
		...
		amrex::ParallelFor(bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j, int k) {
			...
			if (i < domlo[0] || i > domhi[0] || j < domlo[1] || j > domhi[1] || k < domlo[2] || k > domhi[2]) {
				...
				for (int n = nlo; n <= npts - 1; ++n) {
					...
					for (int m = 1; m <= gravity::lnum; ++m) {
						for (int l = 1; l <= gravity::lnum; ++l) {
							...
							phi_arr(i, j, k) +=
							    (qLC_arr(l, m, n) * std::cos(m * phiAngle) + qLS_arr(l, m, n) * std::sin(m * phiAngle)) *
	...
	if (gravity::verbose != 0) {
		...
		if (amrex::ParallelDescriptor::IOProcessor()) {
			std::cout << "Gravity<T>::fill_multipole_BCs() time = " << end << std::endl << std::endl;
		}
	}
}
	...
	for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
		if (geom.isPeriodic(idim)) {
			...
			if (phys_bc->lo(idim) == Symmetry) {
				amrex::Abort("Reflecting boundaries not supported for Poisson gravity!");
			} else {
