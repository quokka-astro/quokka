// New implementation of computeHydroFluxes using MFIter loops to avoid
// temporary MultiFab allocations
template <typename problem_t>
auto QuokkaSimulation<problem_t>::computeHydroFluxes(
    amrex::MultiFab const &consVar_cc,
    std::array<amrex::MultiFab, AMREX_SPACEDIM> const &consVar_fc,
    const int nvars, const int lev)
    -> std::tuple<std::array<amrex::MultiFab, AMREX_SPACEDIM>,
  ...
  const int reconstructGhost =
      3; // reconstruct *two* additional cells outside valid region
  // we need two additional ghost cells in order to compute two ghost face
  // velocities
  ...
  for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
    ...
    if constexpr (Physics_Traits<problem_t>::is_mhd_enabled) {
      fast_mhd_wavespeeds[idim] =
          amrex::MultiFab(ba_face, dm, 2, reconstructGhost - 1);
    }
  }

  // Use MFIter loop to process each box
  for (amrex::MFIter mfi(consVar_cc, amrex::TilingIfNotGPU()); mfi.isValid();
       ++mfi) {
    ...
    amrex::ParallelFor(
        primBox, nvars, [=] AMREX_GPU_DEVICE(int i, int j, int k, int n) {
          ...
          if (n == HydroSystem<problem_t>::density_index) {
            ...
            primVar(i, j, k, n) =
                quokka::EOS<problem_t>::ComputePressure(rho, Eint);
    ...
    AMREX_D_TERM(
        {
          ...
          HydroSystem<problem_t>::template ComputeFlatteningCoefficients<
              FluxDir::X1>(primVar_fab, flatCoef, flatCoefs_fab[0].box());
        },
        {
          auto flatCoef = flatCoefs_fab[1].array();
          HydroSystem<problem_t>::template ComputeFlatteningCoefficients<
              FluxDir::X2>(primVar_fab, flatCoef, flatCoefs_fab[1].box());
        },
        {
          auto flatCoef = flatCoefs_fab[2].array();
          HydroSystem<problem_t>::template ComputeFlatteningCoefficients<
              FluxDir::X3>(primVar_fab, flatCoef, flatCoefs_fab[2].box());
        })
    ...
    AMREX_D_TERM(
        {
          ...
          auto fSpdsArray = (Physics_Traits<problem_t>::is_mhd_enabled)
                                ? fast_mhd_wavespeeds[0].array(mfi)
                                : amrex::Array4<amrex::Real>{};

          hydroFluxFunctionMFIter<FluxDir::X1>(
              primVar_fab, leftState_fab[0], rightState_fab[0], flux[0][mfi],
              facevel[0][mfi],
              (Physics_Traits<problem_t>::is_mhd_enabled)
                  ? &fast_mhd_wavespeeds[0][mfi]
                  : nullptr,
        ...
        },
        {
          ...
          auto fSpdsArray = (Physics_Traits<problem_t>::is_mhd_enabled)
                                ? fast_mhd_wavespeeds[1].array(mfi)
                                : amrex::Array4<amrex::Real>{};

          hydroFluxFunctionMFIter<FluxDir::X2>(
              primVar_fab, leftState_fab[1], rightState_fab[1], flux[1][mfi],
              facevel[1][mfi],
              (Physics_Traits<problem_t>::is_mhd_enabled)
                  ? &fast_mhd_wavespeeds[1][mfi]
                  : nullptr,
              consVar_fc, flatCoefs_fab[0], flatCoefs_fab[1], flatCoefs_fab[2],
              mfi, reconstructGhost, nvars);
        },
        {
          ...
          auto fSpdsArray = (Physics_Traits<problem_t>::is_mhd_enabled)
                                ? fast_mhd_wavespeeds[2].array(mfi)
                                : amrex::Array4<amrex::Real>{};

          hydroFluxFunctionMFIter<FluxDir::X3>(
              primVar_fab, leftState_fab[2], rightState_fab[2], flux[2][mfi],
              facevel[2][mfi],
              (Physics_Traits<problem_t>::is_mhd_enabled)
                  ? &fast_mhd_wavespeeds[2][mfi]
                  : nullptr,
              consVar_fc, flatCoefs_fab[0], flatCoefs_fab[1], flatCoefs_fab[2],
              mfi, reconstructGhost, nvars);
        })
  ...
  if (lowLevelDebuggingOutput_ == 1) {
    // Note: We can't output primVar and flatCoefs here since they're not
    // MultiFabs anymore We'd need to restructure the debugging output if needed
    amrex::Print() << "WARNING: Low-level debugging output not fully supported "
                      "with MFIter implementation\n";
  }

  // return flux and face-centered velocities
  return std::make_tuple(std::move(flux), std::move(facevel),
                         std::move(fast_mhd_wavespeeds));
}
...
void QuokkaSimulation<problem_t>::hydroFluxFunctionMFIter(
    amrex::FArrayBox &primVar_fab, amrex::FArrayBox &leftState_fab,
    amrex::FArrayBox &rightState_fab, amrex::FArrayBox &flux_fab,
    amrex::FArrayBox &faceVel_fab, amrex::FArrayBox *x1FSpds_fab,
    std::array<amrex::MultiFab, AMREX_SPACEDIM> const &consVar_fc,
    amrex::FArrayBox const &x1Flat_fab, amrex::FArrayBox const &x2Flat_fab,
    amrex::FArrayBox const &x3Flat_fab, amrex::MFIter const &mfi,
  ...
  if constexpr (Physics_Traits<problem_t>::is_mhd_enabled) {
    ...
    amrex::ParallelFor(primBox, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
      const double bx2_m =
          x2State_fc(i, j, k, Physics_Indices<problem_t>::mhdFirstIndex);
      const double bx2_p =
          x2State_fc(i + delta_x2[0], j + delta_x2[1], k + delta_x2[2],
                     Physics_Indices<problem_t>::mhdFirstIndex);
      primVar_arr(i, j, k, HydroSystem<problem_t>::x2Magnetic_index) =
          0.5 * (bx2_m + bx2_p);

      const double bx3_m =
          x3State_fc(i, j, k, Physics_Indices<problem_t>::mhdFirstIndex);
      const double bx3_p =
          x3State_fc(i + delta_x3[0], j + delta_x3[1], k + delta_x3[2],
                     Physics_Indices<problem_t>::mhdFirstIndex);
      primVar_arr(i, j, k, HydroSystem<problem_t>::x3Magnetic_index) =
          0.5 * (bx3_m + bx3_p);
    });
  }
  ...
  if (reconstructionOrder_ == 5) {
    ...
    HyperbolicSystem<problem_t>::template ReconstructStatesPLM<
        DIR, SlopeLimiter::minmod>(primVar_fab, leftState_fab, rightState_fab,
                                   ng_reconstruct, nvars);
  ...
  HydroSystem<problem_t>::template FlattenShocks<DIR>(
      primVar_fab, x1Flat_fab, x2Flat_fab, x3Flat_fab, leftState_fab,
      rightState_fab, ng_reconstruct, nvars);
