...
## Testing Guidelines
Add a unit test registered with CTest or extend `regression/quokka-tests.ini` with a matching `inputs/` file for new features. Write test problems in CGS (`Physics_Traits::unit_system = UnitSystem::CGS`) with physical values; do not use `UnitSystem::CONSTANTS` or `CUSTOM` unless it is absolutely necessary and the user has said so, since dimensionless units cannot exercise any empirically calibrated constant. Preserve baseline outputs in `tests/` when regression scripts consume them, and document any GPU-only runs by recording the exact commands. Keep numerical changes reproducible with notes on key diagnostics.
