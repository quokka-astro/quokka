...
Prefer the `quokka` CLI in `scripts/bash/quokka` for routine workflows. Configure with `quokka config <preset>` (`1d`, `3d`, `1d-debug`, `3d-debug`), adding `--delete` when reconfiguring an existing preset build directory. Build one or more problems with `quokka build <preset> <problem> [<problem> ...]` or by glob with `quokka build <preset> --filter <glob>`. Run problem executables with `quokka run <preset> <problem>` (optionally `--input <file>` and `--fpe`), run CTest suites with `quokka run <preset> -j <N>` or `quokka run <preset> --filter <regex>`, list problems with `quokka list`, inspect targets with `quokka target <preset>`, and clean generated test outputs with `quokka clean`. Use `--source <file>` on `config/build/buildrun/run/target` when environment setup is required per command.
