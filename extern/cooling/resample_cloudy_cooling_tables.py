#!/usr/bin/env python3
# ABOUTME: Resample Cloudy format cooling tables as a function of specific
...
def find_eint_range(tables):
    ...
    Args:
        tables: the Cloudy cooling tables
    ...
    T_max = 10.0 ** tables.log_T[-1]
    mu_min = np.nanmin(tables.mmw)
    mu_max = np.nanmax(tables.mmw)
...
def resample_cloudy_cooling_tables(cloudy_file, n_rho=100, n_eint=100, 
                                   output_file='resampled_cloudy_cooling_tables.h5'):
    """Resample Cloudy format cooling tables on a not-quite-logarithmic grid of density and specific internal energy.    
    ...
    Args:
        cloudy_file: path to Cloudy HDF5 cooling tables
    ...
    tables = read_tables(cloudy_file)
    ...
    print(f"Resampling Cloudy cooling tables on {n_rho} x {n_eint} grid using not-quite-logarithmic spacing...")
    ...
    with h5py.File(output_file, 'w') as f:
        ...
        metadata_group.attrs['description'] = 'Cloudy cooling rates resampled on (rho, e_int) grid using not-quite-logarithmic spacing'
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Resample Cloudy cooling tables on (rho, e_int) grid')
    
    parser.add_argument('cloudy_file', type=str, nargs='?',
                        help='Path to Cloudy HDF5 cooling table file')
    ...
    parser.add_argument('--output', type=str, default='resampled_cloudy_cooling_tables.h5',
                        help='Output HDF5 file name (default: resampled_cloudy_cooling_tables.h5)')
    ...
    # For other modes, cloudy_file is required
    if not args.cloudy_file:
        parser.error("cloudy_file is required unless using --test mode")    
    else:
        # Resampling mode
        resample_cloudy_cooling_tables(
            args.cloudy_file,
            n_rho=args.n_rho,
            n_eint=args.n_eint,
