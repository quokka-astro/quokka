...
def resample_cooling_tables(grackle_file, n_rho=100, n_eint=100, zmet=1.0,
    ...
    # Save resampled tables to HDF5 file (DataTable PRD format)
    print(f"\nSaving resampled tables to {output_file}")

    # Stack the 5 outputs into a single (5, n_rho, n_eint) array
    combined = np.stack([cooling_rates, temperatures, sound_speeds, pressures, entropies], axis=0)

    with h5py.File(output_file, 'w') as f:
        # File-level attribute
        f.attrs['include_pe'] = 1 if include_pe else 0

        # DataTable group with standard attributes (see prd-HDF5-datatable-for-cooling.md)
        g = f.create_group('data')
        g.attrs.create('Ndim', 2, dtype='i4')
        g.attrs.create('Nx', np.array([n_rho, n_eint], dtype=np.int32))
        g.attrs.create('Nout', 5, dtype='i4')
        g.attrs.create('input_names', np.array(['rho', 'eint'], dtype='S'))
        g.attrs.create('output_names',
                       np.array(['cooling_rate', 'temperature', 'sound_speed', 'pressure', 'entropy'], dtype='S'))
        g.attrs.create('input_units', np.array(['g/cm^3', 'erg/g'], dtype='S'))
        g.attrs.create('output_units',
                       np.array(['erg/cm^3/s/(g/cm^3)^2', 'K', 'cm/s', 'dyne/cm^2', 'erg*cm^2'], dtype='S'))
        g.attrs.create('xlo', np.array([rho_min, eint_min], dtype=np.float64))
        g.attrs.create('xhi', np.array([rho_max, eint_max], dtype=np.float64))
        g.attrs.create('spacing', np.array(['fast_log', 'fast_log'], dtype='S'))
        g.create_dataset('data', data=combined)

        # Optional grids group for irregular-grid reference (regularly-sampled here)
