  ...
  void sync_to_gpu() {
    {
      ...
      amrex::Gpu::copy(amrex::Gpu::hostToDevice, pinnedAmpl.begin(),
                       pinnedAmpl.end(), ampl_gpu.begin());
    }

    amrex::Gpu::copy(amrex::Gpu::hostToDevice, ampl_factor, ampl_factor + 3,
                     ampl_factor_gpu.begin());

    for (int dim = 0; dim < 3; dim++) {
      {
        amrex::Gpu::PinnedVector<double> pinnedAka(aka[dim].size());
        std::copy(aka[dim].begin(), aka[dim].end(), pinnedAka.begin());
        amrex::Gpu::copy(amrex::Gpu::hostToDevice, pinnedAka.begin(),
                         pinnedAka.end(), aka_gpu[dim].begin());
      }
      {
        amrex::Gpu::PinnedVector<double> pinnedakb(akb[dim].size());
        std::copy(akb[dim].begin(), akb[dim].end(), pinnedakb.begin());
        amrex::Gpu::copy(amrex::Gpu::hostToDevice, pinnedakb.begin(),
                         pinnedakb.end(), akb_gpu[dim].begin());
      }
    }
  } // sync_to_gpu
...
public:
  ...
  TurbGenEx() : TurbGen(amrex::ParallelDescriptor::MyProc()) {}
  ...
  void get_turb_vector_unigrid(
    ...
    amrex::Box xSpace(amrex::IntVect(AMREX_D_DECL(box.smallEnd()[0], 0, 0)),
                      amrex::IntVect(AMREX_D_DECL(box.bigEnd()[0], 0, 0)));
    amrex::Box ySpace(amrex::IntVect(AMREX_D_DECL(0, box.smallEnd()[1], 0)),
                      amrex::IntVect(AMREX_D_DECL(0, box.bigEnd()[0], 0)));
    amrex::Box zSpace(amrex::IntVect(AMREX_D_DECL(0, 0, box.smallEnd()[2])),
                      amrex::IntVect(AMREX_D_DECL(0, 0, box.bigEnd()[2])));
    ...
    amrex::ParallelFor(
        xSpace, nmodes,
        [=, modesPointers = this->modes_pointers,
    ...
    amrex::ParallelFor(
        box,
        [=, akaPointers = this->aka_pointers, akbPointers = this->akb_pointers,
         modesPointers = this->modes_pointers,
         nmodes = this->nmodes] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
          ...
          for (int m = 0; m < nmodes; m++) {
            ...
            real =
                (xPrecomp(i, 0, 0, COS_INDEX) * yPrecomp(0, j, 0, COS_INDEX) -
            ...
            imag =
                (yPrecomp(0, j, 0, COS_INDEX) * zPrecomp(0, 0, k, SIN_INDEX) +
            ...
            v[X] += ampGPU_prt[m] *
                    (akaPointers[X][m] * real - akbPointers[X][m] * imag);
            if constexpr (AMREX_SPACEDIM > 1)
              v[Y] += ampGPU_prt[m] *
                      (akaPointers[Y][m] * real - akbPointers[Y][m] * imag);
            if constexpr (AMREX_SPACEDIM > 2)
              v[Z] += ampGPU_prt[m] *
                      (akaPointers[Z][m] * real - akbPointers[Z][m] * imag);
          }
