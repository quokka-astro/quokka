...
struct TurbGenData {
  ...
  double mode[3][tgd_max_n_modes], aka[3][tgd_max_n_modes],
      akb[3][tgd_max_n_modes];          // modes arrays
  double OUphases[6 * tgd_max_n_modes]; // phases
  double ampl[tgd_max_n_modes];         // amplitudes
  int ndim;                             // number of spatial dimensions
  int random_seed, seed; // 'random seed' is the original starting seed, then
                         // 'seed' gets updated by call to RNG
  ...
  double
      angles_exp; // for power-law spectrum: angles exponent for sparse sampling
};
...
int TurbGen_init_single_realisation(const double L[3], const double k_min,
                                    const double k_max, const int spect_form,
                                    const double power_law_exp,
                                    const double angles_exp,
                                    const double sol_weight,
                                    const int random_seed, const int PE);
...
void TurbGen_get_turb_vector_unigrid(const double pos_beg[3],
                                     const double pos_end[3], const int n[3],
                                     float *return_grid[3]);
...
int TurbGen_init_single_realisation(const double L[3], const double k_min,
                                    ...
                                    const int random_seed, const int PE) {
  // ******************************************************
  // Initialise the tubrulence generator for a single turbulent realisation,
  // e.g., for producing turbulent initial conditions, with parameters specified
  // as inputs to the function; see descriptions below.
  // ******************************************************
  ...
  tgd.stir_min = (k_min - DBL_EPSILON) * 2 * M_PI /
                 tgd.L[X]; // Minimum driving wavenumber <~  k_min * 2pi / Lx
  tgd.stir_max = (k_max + DBL_EPSILON) * 2 * M_PI /
                 tgd.L[X]; // Maximum driving wavenumber >~  k_max * 2pi / Lx
  tgd.spect_form =
      spect_form; // (0: band/rectangle/constant, 1: paraboloid, 2: power law)
  tgd.power_law_exp = power_law_exp; // power-law amplitude exponent (only if
                                     // spect_form = 2, i.e., power law)
  tgd.angles_exp =
      angles_exp; // angles exponent (only if spect_form = 2, i.e., power law)
  tgd.sol_weight = sol_weight;   // solenoidal weight (0: compressive, 0.5:
  ...
  tgd.OUvar = 1.0; // Ornstein-Uhlenbeck variance (the field needs to be
                   // re-normalised outside this call)
  tgd.ndim = 3;
  TurbGen_printf("============================================================="
                 "==================\n");
  ...
  // weight
  tgd.sol_weight_norm =
      sqrt(3.0 / tgd.ndim) * sqrt(3.0) * 1.0 /
      sqrt(1.0 - 2.0 * tgd.sol_weight + tgd.ndim * pow(tgd.sol_weight, 2.0));
  // initialise modes
  TurbGen_init_modes();
...
int TurbGen_init_driving(char *parameter_file, const int PE) {
  ...
  // If called from an MPI-parallelised code, supply its MPI rank in PE,
  // otherwise provide PE=0
  // ******************************************************
  // set parameter file
  ...
  TurbGen_read_from_parameter_file("nsteps_per_turnover_time", 'i',
                                   &tgd.nsteps_per_turnover_time);
  // define derived physical quantities
  tgd.stir_min = (k_min - DBL_EPSILON) * 2 * M_PI /
                 tgd.L[X]; // Minimum driving wavenumber <~  k_min * 2pi / Lx
  tgd.stir_max = (k_max + DBL_EPSILON) * 2 * M_PI /
                 tgd.L[X]; // Maximum driving wavenumber >~  k_max * 2pi / Lx
  tgd.decay = tgd.L[X] / k_driv /
              tgd.velocity; // Auto-correlation time, t_turb = Lx / k_driv /
                            // velocity; i.e., turbulent turnover (crossing)
                            // time; with k_driv in units of 2pi/Lx
  tgd.energy =
      pow(ampl_coeff * tgd.velocity, 3.0) /
      tgd.L[X]; // Energy input rate => driving amplitude ~ sqrt(energy/decay)
  ...
  tgd.dt = tgd.decay /
           tgd.nsteps_per_turnover_time; // time step in OU process and for
                                         // creating new driving pattern
  tgd.step = -1; // set internal OU step to 0 for start-up
  tgd.seed =
      tgd.random_seed; // copy original seed into local seed;
...
bool TurbGen_check_for_update(double time) {
  ...
  // ******************************************************
  int step_requested = floor(
      time / tgd.dt); // requested OU step number based on input current 'time'
  if (tgd.verbose)
    TurbGen_printf("step_requested = %i\n", step_requested);
  if (step_requested <= tgd.step) {
    if (tgd.verbose)
      TurbGen_printf("no update of pattern...returning.\n");
    return false; // no update (yet) -> return false, i.e., no change to driving
                  // pattern
  }
  // update OU vector
  for (int is = tgd.step; is < step_requested; is++) {
    TurbGen_OU_noise_update(); // this seeks to the requested OU state (updates
                               // OUphases)
    if (tgd.verbose)
      TurbGen_printf("step = %i, time = %f\n", tgd.step,
                     tgd.step * tgd.dt); // print some info
  }
  TurbGen_get_decomposition_coeffs(); // calculate solenoidal and compressive
                                      // coefficients (aka, akb) from OUphases
  double time_gen = tgd.step * tgd.dt;
  TurbGen_printf("Generated new turbulence driving pattern: #%6i, time = %e, "
                 "time/t_turb = %-7.2f\n",
...
void TurbGen_get_turb_vector_unigrid(const double pos_beg[3],
  ...
  for (int dim = 0; dim < 3; dim++)
    d[dim] = (pos_end[dim] - pos_beg[dim]) / (n[dim] - 1);
  // pre-compute grid position geometry, and trigonometry, for quicker access in
  // loop over modes below
  for (int m = 0; m < tgd.n_modes; m++) { // loop over modes
    ampl[m] =
        2.0 * tgd.sol_weight_norm *
        tgd.ampl[m]; // pre-compute amplitude including normalisation factors
    for (int i = 0; i < n[X]; i++) {
...
void TurbGen_get_turb_vector(const double pos[3], double v[3]) {
  // ******************************************************
  // Compute physical turbulent vector v[3]=(vx,vy,vz) at position
  // pos[3]=(x,y,z) from loop over all turbulent modes; return into double *
  // v[3]
  // ******************************************************
  // containers for speeding-up calculations below
  double ampl[tgd.n_modes];
...
void TurbGen_print_info(char *print_mode) {
  ...
  if (strcmp("single_realisation", print_mode) == 0) {
    TurbGen_printf(
        "Initialized %i modes for generating a single turbulent realisation.\n",
        tgd.n_modes);
  }
  if (tgd.spect_form == 0)
    TurbGen_printf(
        ...
        tgd.spect_form);
  if (tgd.spect_form == 1)
    TurbGen_printf(" spectral form                                       = %i "
                   "(Parabola)\n",
                   tgd.spect_form);
  if (tgd.spect_form == 2)
    TurbGen_printf(" spectral form                                       = %i "
...
int TurbGen_read_from_parameter_file(char *search, char type, void *ret) {
  ...
  // 'search' at the beginning of each line; if 'search' is found return double
  // of value after '=' type: 'i' for int, 'd' for double return type void *ret
  // ******************************************************
  FILE *fp;
  char *line = NULL;
  size_t len = 0;
  ...
  if (fp == NULL) {
    printf("TurbGen: ERROR: could not open parameter file '%s'\n",
           tgd.parameter_file);
    exit(-1);
  }
  bool found = false;
...
int TurbGen_init_modes(void) {
  ...
  kc = tgd.stir_min;
  if (tgd.spect_form == 1)
    kc = 0.5 * (tgd.stir_min + tgd.stir_max);
  ...
  if (tgd.spect_form != 2) { // for Band and Parabola
    if (tot_n_modes + pow(2.0, tgd.ndim - 1) > tgd_max_n_modes) {
      TurbGen_printf("init_stir:  n_modes = %i, maxmodes = %i", tgd.n_modes,
                     tgd_max_n_modes);
      ...
      for (ik[Y] = ikmin[Y]; ik[Y] <= ikmax[Y]; ik[Y]++) {
        ...
        for (ik[Z] = ikmin[Z]; ik[Z] <= ikmax[Z]; ik[Z]++) {
          ...
          if ((ka >= tgd.stir_min) && (ka <= tgd.stir_max)) {
            ...
            if (tgd.verbose)
              TurbGen_printf("init_stir:  ampl[%i] = %f\n", tgd.n_modes,
                             tgd.ampl[tgd.n_modes]);
  ...
  // ===============================================================================
  // === for power law, generate modes that are distributed randomly on the
  // k-sphere
  // === with the number of angles growing ~ k^angles_exp
  if (tgd.spect_form == 2) {

    TurbGen_printf("There would be %i turbulent modes, if k-space were fully "
                   "sampled (angles_exp = 2.0)...\n",
                   tot_n_modes);
    TurbGen_printf("Here we are using angles_exp = %f\n", tgd.angles_exp);

    // initialize additional random numbers (uniformly distributed) to randomise
    // angles
    int seed = -tgd.seed; // initialise Numerical Recipes rand gen (call with
                          // negative integer)
    ...
    for (ik[0] = ikmin[0]; ik[0] <= ikmax[0]; ik[0]++) {
      ...
      for (iang = 1; iang <= nang; iang++) {
        ...
        phi = 2 * M_PI *
              TurbGen_ran2(&tgd.seed); // phi = [0,2pi] sample the whole sphere
        if (tgd.ndim == 1) {
          if (phi < M_PI)
            phi = 0.0;
          if (phi >= M_PI)
        ...
        if (tgd.ndim > 2)
          theta = acos(
              1.0 -
              2.0 * TurbGen_ran2(
                        &tgd.seed)); // theta = [0,pi] sample the whole sphere

        if (tgd.verbose)
          TurbGen_printf("entering: theta = %f, phi = %f\n", theta, phi);
        ...
        if ((ka >= tgd.stir_min) && (ka <= tgd.stir_max)) {
          ...
          if (tgd.n_modes + pow(2.0, tgd.ndim - 1) > tgd_max_n_modes) {
            TurbGen_printf("init_stir:  n_modes = %i, maxmodes = %i",
                           tgd.n_modes, tgd_max_n_modes);
            TurbGen_printf("Too many stirring modes");
            exit(-1);
          }
          ...
          // note: power spectrum ~ amplitude^2 (1D), amplitude^2 * 2pi k (2D),
          // amplitude^2 * 4pi k^2 (3D)
          // ...and correct for the number of angles sampled relative to the
          // full sampling (k^2 per k-shell in 3D)
          amplitude = sqrt(amplitude * pow((double)ik[0], tgd.ndim - 1) /
                           (double)(nang) * 4.0 * sqrt(3.0)) *
                      pow(kc / ka, (tgd.ndim - 1) / 2.0);
...
void TurbGen_OU_noise_init(void) {
  ...
  for (int i = 0; i < 6 * tgd.n_modes; i++)
    tgd.OUphases[i] = tgd.OUvar * TurbGen_grn();
} // TurbGen_OU_noise_init
