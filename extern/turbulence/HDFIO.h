...
// The detault is to use this class with MPI support (to switch off MPI, the
// user needs to #define NO_MPI). Also note that this class makes use of
// H5_HAVE_PARALLEL defined in hdf5.h, if hdf5 has parallel support.
#ifdef NO_MPI
#ifndef MPI_Comm
#define MPI_Comm int
#endif
...
static const unsigned int flash_str_len =
    79; // string length for FLASH scalars and runtime parameters I/O
template <typename>
struct FlashScalarsParametersStruct; // structure for FLASH scalars and runtime
                                     // parameters I/O
template <> struct FlashScalarsParametersStruct<int> {
  char name[flash_str_len];
  int value;
};
template <> struct FlashScalarsParametersStruct<double> {
  char name[flash_str_len];
  double value;
};
template <> struct FlashScalarsParametersStruct<bool> {
  char name[flash_str_len];
  bool value;
};
template <> struct FlashScalarsParametersStruct<std::string> {
  char name[flash_str_len];
  char value[flash_str_len];
};
// FLASHPropAssign template function; applies to string case (so we do a
// string-copy from string to c_str)
template <typename T, typename Td>
inline void FLASHPropAssign(T val, Td *data) {
  strcpy(*data, val.c_str());
};
// FLASHPropAssign template functions that apply to int, double, bool cases
template <> inline void FLASHPropAssign<int>(int val, int *data) {
  *data = val;
}; // simply assign
template <> inline void FLASHPropAssign<double>(double val, double *data) {
  *data = val;
}; // simply assign
template <> inline void FLASHPropAssign<bool>(bool val, bool *data) {
  *data = val;
}; // simply assign
} // namespace NameSpaceHDFIO
 ...
 * HDFIO class
 *
 * This class represents an HDF5 object/file and provides basic HDF operations
 * like opening a file, reading data form a dataset of that file, creating new
 * HDF files and datasets and writing datasets into a file. There are also some
 * functions for getting and setting different attributes, like the
 * dimensionaltity of the HDF dataset to be written or the datasetnames inside
 * an HDF5 file.
 *
...
private:
  ...
  hsize_t HDFSize, HDFDims[4]; // HDF5 stuff; can maximally handle 4D datasets
                               // (but who would ever want more?)
...
public:
  HDFIO(void) {
    // empty constructor, so we can define a global HDFIO object in application
    // code
    Constructor("", 'r', MPI_COMM_NULL, 0);
  };

public:
  HDFIO(const int verbose) { Constructor("", 'r', MPI_COMM_NULL, verbose); };

public:
  HDFIO(const std::string filename) {
    Constructor(filename, 'r', MPI_COMM_NULL, 1);
  };

public:
  HDFIO(const std::string filename, const int verbose) {
    Constructor(filename, 'r', MPI_COMM_NULL, verbose);
  };

public:
  HDFIO(const std::string filename, const char read_write_char) {
...
public:
  ~HDFIO() {
    if (Verbose > 1)
      std::cout << "HDFIO: destructor called." << std::endl;
  };

private:
  void Constructor(const std::string filename, const char read_write_char,
                   MPI_Comm comm, const int verbose) {
    Filename = filename; // HDF5 filename
    Verbose = verbose;   // verbose (can be 0: no stdout, 1: default output, 2:
                         // more output)
    ClassSignature =
        "HDFIO: ";    // class signature, when this class is printing to stdout
    ...
    if (Filename != "")
      this->open(Filename, read_write_char,
                 comm); // open file if provided in constructor
  };

  // get function signature for printing to stdout
private:
  std::string FuncSig(const std::string func_name) {
...
public:
   ...
   * @param *DataBuffer pointer to double/float/int array to which data is to be
   * written
   * @param Datasetname datasetname
   * @param DataType (i.e. H5T_STD_I32LE)
   */
public:
  void read(void *const DataBuffer, const std::string Datasetname,
...
public:
  void read(void *const DataBuffer, const std::string Datasetname,
    ...
    HDF5_status =
        H5Dread(Dataset_id, DataType, H5S_ALL, H5S_ALL, plist_id, DataBuffer);
    assert(HDF5_status != HDF5_error);
#ifdef H5_HAVE_PARALLEL
    if (comm != MPI_COMM_NULL) {
   ...
   */
public:
  void read_attribute(void *const DataBuffer, const std::string Datasetname,
                      const std::string Attributename, const hid_t DataType) {
    this->read_attribute(DataBuffer, Datasetname, Attributename, DataType,
                         MPI_COMM_NULL); // serial mode
  };
...
public:
   ...
   * @param out_count (output count array, i.e. the number of cells[dim] in
   * output)
   */
public:
  void read_slab(void *const DataBuffer, const std::string Datasetname,
                 const hid_t DataType, const hsize_t offset[],
                 const hsize_t count[], const hsize_t out_rank,
                 const hsize_t out_offset[], const hsize_t out_count[]) {
    /// simply set total_out_count = out_count, in which case we selected a
    /// hyperslab that fits completely into the total size of the output array.
    /// Use the overloaded function below, if output goes into an offset
    /// hyperslab
    this->read_slab(DataBuffer, Datasetname, DataType, offset, count, out_rank,
                    out_offset, out_count, out_count);
  };
...
public:
  void read_slab(void *const DataBuffer, const std::string Datasetname,
    ...
    /// hyperslab
    this->read_slab(DataBuffer, Datasetname, DataType, offset, count, out_rank,
                    out_offset, out_count, out_count, comm);
  };
   ...
   * @param out_count (output count array, i.e. the number of cells[dim] in
   * output)
   * @param total_out_count (total output count array, i.e. the total number of
   * cells[dim] in output)
   */
public:
  void read_slab(void *const DataBuffer, const std::string Datasetname,
    ...
    this->read_slab(DataBuffer, Datasetname, DataType, offset, count, out_rank,
                    out_offset, out_count, total_out_count,
                    MPI_COMM_NULL); // serial mode
  };
...
public:
  void read_slab(void *const DataBuffer, const std::string Datasetname,
    ...
    // select hyperslab
    HDF5_status = H5Sselect_hyperslab(Dataspace_id, H5S_SELECT_SET, offset,
                                      NULL, count, NULL);
    assert(HDF5_status != HDF5_error);

    // create memspace
    hid_t Memspace_id = H5Screate_simple(out_rank, total_out_count, NULL);
    HDF5_status = H5Sselect_hyperslab(Memspace_id, H5S_SELECT_SET, out_offset,
                                      NULL, out_count, NULL);
    assert(HDF5_status != HDF5_error);
...
#endif
    // read buffer
    HDF5_status = H5Dread(Dataset_id, DataType, Memspace_id, Dataspace_id,
                          plist_id, DataBuffer);
    assert(HDF5_status != HDF5_error);
#ifdef H5_HAVE_PARALLEL
    if (comm != MPI_COMM_NULL) {
   ...
   */
public:
  void overwrite_slab(void *const DataBuffer, const std::string Datasetname,
                      const hid_t DataType, const hsize_t offset[],
                      const hsize_t count[], const hsize_t out_rank,
                      const hsize_t out_offset[], const hsize_t out_count[]) {
    this->overwrite_slab(DataBuffer, Datasetname, DataType, offset, count,
                         out_rank, out_offset, out_count,
                         MPI_COMM_NULL); // serial mode
  };
...
public:
...
#endif
    // overwrite dataset
    HDF5_status = H5Dwrite(Dataset_id, DataType, Memspace_id, Dataspace_id,
                           plist_id, DataBuffer);
    assert(HDF5_status != HDF5_error);
#ifdef H5_HAVE_PARALLEL
    if (comm != MPI_COMM_NULL) {
   ...
   * @param *DataBuffer pointer to double/float/int array containing data to be
   * written
   * @param Datasetname datasetname
   * @param DataType (i.e. H5T_STD_I32LE)
   */
public:
  void overwrite(const void *const DataBuffer, const std::string Datasetname,
                 const hid_t DataType) {
...
public:
  void overwrite(const void *const DataBuffer, const std::string Datasetname,
    ...
    // overwrite dataset
    HDF5_status =
        H5Dwrite(Dataset_id, DataType, H5S_ALL, H5S_ALL, plist_id, DataBuffer);
    assert(HDF5_status != HDF5_error);
#ifdef H5_HAVE_PARALLEL
    if (comm != MPI_COMM_NULL) {
      HDF5_status = H5Pclose(plist_id);
...
public:
  void write(const void *const DataBuffer, const std::string Datasetname,
    ...
    // -------------- create dataset
    Dataset_id = H5Dcreate(File_id, Datasetname.c_str(), DataType, Dataspace_id,
...
public:
   ...
   */
public:
  bool dataset_exists(const std::string Datasetname) {
    ...
    for (unsigned int i = 0; i < dsets_in_file.size(); i++) {
      if (dsets_in_file[i] == Datasetname) {
        dset_in_file = true;
        break;
      }
    }
...
public:
  void delete_dataset(const std::string Datasetname) {
    if (this->dataset_exists(
            Datasetname)) { // if the Datasetname is in the file, delete it
      HDF5_status = H5Ldelete(File_id, Datasetname.c_str(),
                              H5P_DEFAULT); // delete dataset
      assert(HDF5_status != HDF5_error);
    }
  };
...
public:
   ...
   */
public:
  void setRank(const int Rank) { this->Rank = Rank; };
   ...
   */
public:
  void setDimX(const int dim_x) { HDFDims[0] = static_cast<hsize_t>(dim_x); };

public:
  void setDimY(const int dim_y) { HDFDims[1] = static_cast<hsize_t>(dim_y); };

public:
  void setDimZ(const int dim_z) { HDFDims[2] = static_cast<hsize_t>(dim_z); };
...
public:
   ...
   */
public:
  int getDimX(void) const { return static_cast<int>(HDFDims[0]); };

public:
  int getDimY(void) const { return static_cast<int>(HDFDims[1]); };

public:
  int getDimZ(void) const { return static_cast<int>(HDFDims[2]); };
...
public:
  int getSize(const std::string Datasetname) {
    ...
    if (Dataset_id == HDF5_error) {
      std::cout << "HDFIO:  getSize():  CAUTION: Datasetname '" << Datasetname
                << "' does not exists in file '" << this->getFilename()
                << "'. Continuing..." << std::endl;
      return 0;
    }
    assert(Dataset_id != HDF5_error);
...
public:
   ...
   */
public:
  hid_t getFileID(void) { return File_id; };
...
public:
  std::vector<std::string> getDatasetnames(void) {
    ...
    for (int i = 0; i < nsets; i++) {
      ...
      if (Verbose > 2)
        std::cout << "dataset in file: " << ret[i] << std::endl;
    }
    return ret;
  };
  ...
  // *************************************************************************
  // //
  /// returns FLASH integer, real, logical, string scalars or runtime parameters
  /// as maps
public:
...
public:
  std::map<std::string, int> ReadFlashIntegerParameters() {
    return GetFLASHProps<int>("integer runtime parameters");
  };

public:
  std::map<std::string, double> ReadFlashRealScalars() {
...
public:
  std::map<std::string, bool> ReadFlashLogicalScalars() {
    return GetFLASHProps<bool>("logical scalars");
  };

public:
  std::map<std::string, bool> ReadFlashLogicalParameters() {
    return GetFLASHProps<bool>("logical runtime parameters");
  };

public:
  std::map<std::string, std::string> ReadFlashStringScalars() {
    return GetFLASHProps<std::string>("string scalars");
  };

public:
  std::map<std::string, std::string> ReadFlashStringParameters() {
    return GetFLASHProps<std::string>("string runtime parameters");
  };
  // returns map of FLASH integer, real, logical, string scalars or runtime
  // parameters
private:
  template <typename T>
  std::map<std::string, T> GetFLASHProps(std::string dsetname) {
    std::map<std::string, T> ret; // return object
    int nProps = this->getDims(dsetname)[0];
    hid_t h5string = H5Tcopy(H5T_C_S1);
    H5Tset_size(h5string, NameSpaceHDFIO::flash_str_len);
    hid_t dtype;
    ...
    if (typeid(T) == typeid(bool))
      dtype = H5T_NATIVE_HBOOL;
    if (typeid(T) == typeid(std::string))
      dtype = H5Tcopy(h5string);
    hid_t datatype = H5Tcreate(
        H5T_COMPOUND, sizeof(NameSpaceHDFIO::FlashScalarsParametersStruct<T>));
    H5Tinsert(datatype, "name",
              HOFFSET(NameSpaceHDFIO::FlashScalarsParametersStruct<T>, name),
              h5string);
    H5Tinsert(datatype, "value",
              HOFFSET(NameSpaceHDFIO::FlashScalarsParametersStruct<T>, value),
              dtype);
    NameSpaceHDFIO::FlashScalarsParametersStruct<T> *data =
        new NameSpaceHDFIO::FlashScalarsParametersStruct<T>[nProps];
    this->read(data, dsetname, datatype);
    for (int i = 0; i < nProps; i++) {
      std::string pname = Trim((std::string)data[i].name);
      ret[pname] = (T)data[i].value;
      if (Verbose > 1)
        std::cout << FuncSig(__func__) << "name, value = '" << pname << "', '"
                  << ret[pname] << "'" << std::endl;
    }
    delete[] data;
    return ret;
...
private:
  ...
  // //
public:
  void OverwriteFlashIntegerScalars(std::map<std::string, int> props) {
    for (std::map<std::string, int>::iterator it = props.begin();
         it != props.end(); it++)
      OverwriteFLASHProp("integer scalars", it->first, it->second);
  };

public:
  void OverwriteFlashIntegerParameters(std::map<std::string, int> props) {
    for (std::map<std::string, int>::iterator it = props.begin();
         it != props.end(); it++)
...
public:
  void OverwriteFlashRealScalars(std::map<std::string, double> props) {
    for (std::map<std::string, double>::iterator it = props.begin();
         it != props.end(); it++)
      OverwriteFLASHProp("real scalars", it->first, it->second);
  };

public:
  void OverwriteFlashRealParameters(std::map<std::string, double> props) {
    for (std::map<std::string, double>::iterator it = props.begin();
         it != props.end(); it++)
...
public:
  void OverwriteFlashLogicalScalars(std::map<std::string, bool> props) {
    for (std::map<std::string, bool>::iterator it = props.begin();
         it != props.end(); it++)
      OverwriteFLASHProp("logical scalars", it->first, it->second);
  };

public:
  void OverwriteFlashLogicalParameters(std::map<std::string, bool> props) {
    for (std::map<std::string, bool>::iterator it = props.begin();
         it != props.end(); it++)
...
public:
  void OverwriteFlashStringScalars(std::map<std::string, std::string> props) {
...
public:
  void
  OverwriteFlashStringParameters(std::map<std::string, std::string> props) {
    for (std::map<std::string, std::string>::iterator it = props.begin();
         it != props.end(); it++)
      OverwriteFLASHProp("string runtime parameters", it->first, it->second);
  };
  // overwrite FLASH integer, real, logical, string scalars or runtime
  // parameters
private:
  template <typename T>
  void OverwriteFLASHProp(std::string datasetname, std::string fieldname,
                          T fieldvalue) {
    hid_t dsetId = H5Dopen(File_id, datasetname.c_str(), H5P_DEFAULT);
    hid_t spaceId = H5Dget_space(dsetId);
    hsize_t locDims[1];
    H5Sget_simple_extent_dims(spaceId, locDims, NULL);
    int num = locDims[0];
    hid_t h5string = H5Tcopy(H5T_C_S1);
    H5Tset_size(h5string, NameSpaceHDFIO::flash_str_len);
    hid_t dtype;
    ...
    H5Tinsert(datatype, "value",
              ...
              dtype);
    NameSpaceHDFIO::FlashScalarsParametersStruct<T> *data =
        new NameSpaceHDFIO::FlashScalarsParametersStruct<T>[num];
    H5Dread(dsetId, datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT, data);
    for (int i = 0; i < num; i++)
      if (strncmp(data[i].name, fieldname.c_str(), fieldname.size()) == 0) {
        if (Verbose > 1)
          std::cout << FuncSig(__func__) << ": Overwriting " << fieldname
                    << " of " << data[i].value << " with " << fieldvalue
                    << " in '" << datasetname << "'" << std::endl;
        NameSpaceHDFIO::FLASHPropAssign<T>(fieldvalue, &data[i].value);
      }
    H5Dwrite(dsetId, datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT, data);
    delete[] data;
    H5Tclose(datatype);
    H5Tclose(h5string);
    H5Sclose(spaceId);
