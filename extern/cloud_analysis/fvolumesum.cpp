...
AMREX_FORCE_INLINE Real weight_function(
    amrex::Array4<const Real> const &w_arr, int i, int j, int k) {
  // return a weight value for this cell
  amrex::Real weight = 0.0;

  if (w_arr(i, j, k) > 1.0e-5) {
    weight = 1.0;
  }
...
void main_main()
{
    std::string weightvar_name = "scalar";
    ...
    for (int ilev = 0; ilev <= fine_level; ++ilev) {
        ...
        if (ilev < fine_level) {
            ...
            const MultiFab& mf_w = pf.get(ilev, weightvar_name);
            for (MFIter mfi(mf); mfi.isValid(); ++mfi) {
                const Box& bx = mfi.validbox();
                if (bx.ok()) {
                    const auto& m = mask.array(mfi);
                    const auto& fab = mf.array(mfi);
                    const auto& w_arr = mf_w.const_array(mfi);
                        ...
                        for (int j = lo.y; j <= hi.y; ++j) {
                            for (int i = lo.x; i <= hi.x; ++i) {
                                if (m(i,j,k) == 0) { // not covered by fine
                                    ...
                                    lsum += weight_function(w_arr, i, j, k) * fab(i,j,k) * vol;
                                }
                            }
                        }
                    }
    ...
    if (ParallelDescriptor::IOProcessor()) {
        std::cout << "USING WEIGHT FUNCTION scalar > 1.0e-5" << std::endl;
