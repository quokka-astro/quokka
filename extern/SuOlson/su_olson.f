...
c..reproduces the tables in the su-olson 1996 paper

c..declare
      integer          i,j
      double precision epsilon,usolution,vsolution

      integer          nxmax,ntmax
      parameter        (nxmax = 12, ntmax = 11)
      double precision xaray(nxmax),taray(ntmax),
     &                 uans(ntmax,nxmax),vans(ntmax,nxmax)


c..load the position and tau arrays
c..table 1 from
      data  xaray/0.0d0, 0.1d0, 0.25d0, 0.5d0, 0.75d0, 1.0d0, 2.5d0,
     &            5.0d0, 7.5d0, 10.0d0, 15.0d0, 20.0d0/

      data  taray/1.0d-3, 3.0d-3, 1.0d-2, 3.0d-2, 0.1d0, 0.3d0, 1.0d0,
     &            3.0d0, 10.0d0, 30.0d0, 100.0d0/


c..popuar formats
 10    format(1x,1p13e12.4)
 11    format(1x,a,1pe12.4)
 12    format(1x,t3,a,t14,1p13e12.4)



c..define the parameter epsilon, equation 13
      epsilon = 0.1d0


c..compute table 1 on page 346
      write(6,*) 'working on u solution'
      do i=1,ntmax
       write(6,11) 'working on tau ',taray(i)
       do j=1,nxmax
        uans(i,j) = usolution(xaray(j),taray(i),epsilon)
       enddo
      enddo
...
c..write 'em out
      write(6,*)
      write(6,*)
      write(6,12) 'tau\\pos',xaray
      do i=1,ntmax
       write(6,10) taray(i),(uans(i,j), j=1,nxmax)
      enddo


c..compute table 2 on page 346
      write(6,*)
      write(6,*) 'working on v solution'
      do i=1,ntmax
       write(6,11) 'working on tau ',taray(i)
       do j=1,nxmax
        vans(i,j) = vsolution(xaray(j),taray(i),epsilon,uans(i,j))
       enddo
      enddo
...
c..write it out
      ...
      write(6,12) 'tau\\pos',xaray
      do i=1,ntmax
       write(6,10) taray(i),(vans(i,j), j=1,nxmax)
      enddo
