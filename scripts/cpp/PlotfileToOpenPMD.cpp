...
void PrintUsage() {
  amrex::Print()
      << "\n"
      << "Convert an AMReX plotfile to an openPMD ADIOS2 file with lossy "
         "compression.\n"
      << "Usage:\n"
      << "  PlotfileToOpenPMD [--infile PLOTFILE] [--outfile OUTPUT] [--engine "
         "BP5]\n"
...
void SetupMesh(openPMD::Mesh &mesh, const amrex::Geometry &geom,
  ...
  auto const dataset =
      openPMD::Dataset(openPMD::determineDatatype<amrex::Real>(), global_size);
...
void WritePlotfileToOpenPMD(const Options &opts) {
  ...
  int const max_level =
      opts.maxLevel >= 0 ? std::min(opts.maxLevel, finest_level) : finest_level;
  ...
  amrex::RealBox const real_box(plotfile.probLo().data(),
                                plotfile.probHi().data());

  amrex::Vector<amrex::Geometry> geom_levels;
  geom_levels.reserve(nlevels);
  for (int lev = 0; lev < nlevels; ++lev) {
    geom_levels.emplace_back(plotfile.probDomain(lev), real_box,
                             plotfile.coordSys(), is_per);
  }

  int const iteration_index =
      opts.iteration >= 0 ? opts.iteration : std::max(plotfile.levelStep(0), 0);
  std::string const config = BuildAdios2Config(opts);

  openPMD::Series series(opts.outfile, openPMD::Access::CREATE,
                         amrex::ParallelDescriptor::Communicator(), config);
  ...
  for (int lev = 0; lev < nlevels; ++lev) {
    ...
    for (auto const &comp : components) {
      ...
      openPMD::MeshRecordComponent mesh =
          iteration.meshes[mesh_name][openPMD::MeshRecordComponent::SCALAR];
      amrex::Box const global_box = geom.Domain();
      ...
      for (amrex::MFIter mfi(mf); mfi.isValid(); ++mfi) {
        ...
        auto const chunk_offset =
            ToReversedVector(local_box.smallEnd() - global_box.smallEnd());
