...
digits (default: 5), and a 3-digit suffix is appended (e.g., plt00001 → plt00001000.png, plt00001001.png for ndigits=5).
...
def plot_one(pltdir, args, suffix_idx=None):
    ...
    if kind == "slc":
        ...
        #slc = yt.SlicePlot(ds, view_dir, field)
        #if center is not None:
        #    #slc.set_center(((center[0], 'cm'), (center[1], 'cm')))
        #    slc.set_center(center)
    elif kind == "prj":
        slc = yt.ProjectionPlot(ds, view_dir, field, weight_field=args.weight_field, center=center)
    else:
...
def get_index_from_pltdir(pltdir):
    idx = 0
    try:
        idx = int(os.path.basename(pltdir)[-8:])
    except ValueError:
        try:
            idx = int(os.path.basename(pltdir)[-7:])
        except ValueError:
            try:
                idx = int(os.path.basename(pltdir)[-6:])
            except ValueError:
                try:
                    idx = int(os.path.basename(pltdir)[-5:])
                except ValueError:
                    try:
                        idx = int(os.path.basename(pltdir)[-4:])
                    except ValueError:
...
def main(args):
    ...
    # sort pltdir
    valid_pltdirs = sorted(valid_pltdirs, key=get_index_from_pltdir)[:args.max_snapshots]
    ...
    if args.time_interval is not None:
        ...
        print(f"Plot jobs (pltdir, suffix, time): {[(os.path.basename(p), s, t) for p, s, t in plot_jobs[:10]]}{'...' if len(plot_jobs) > 10 else ''}")
    else:
    ...
    if args.n_processes == 1 or args.first_only:
        ...
        plot_args = [(pltdir, args, suffix_idx) for pltdir, suffix_idx, target_time in plot_jobs]
...
def parse_args():
    ...
    parser.add_argument("--max_snapshots", type=int, default=1000000, help="Maximum number of snapshots to plot. Default: None (no filtering)")
    # field to weight with in projection plot
    parser.add_argument("--weight_field", type=str, default=None, help="Field to weight with in projection plot. Default: None (no weighting)")
    # number of digits for basename index padding
    parser.add_argument("--ndigits", type=int, default=7, help="Number of digits to pad the basename index to when using --time_interval. Default: 7")
