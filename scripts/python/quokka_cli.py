...
def cmd_list(context: CliContext, args: argparse.Namespace) -> CommandResult:
    ...
    if args.list_kind == "problems":
        problems = discover_problems(profile.build_dir, "list", context.profile_name())
        data = {"problems": problems}
        text = "\n".join(problems)
        resource = {"kind": "problem", "name": "*"}
    elif args.list_kind == "tests":
        tests = [test.name for test in discover_tests(profile.build_dir, "list", context.profile_name())]
        data = {"tests": tests}
        text = "\n".join(tests)
