...
-   `AMREX_ASSERT`: Works when either `-DAMREX_DEBUG` or `-DAMREX_USE_ASSERTION` is defined. When enabled, the assertion will abort unless both `-DNDEBUG` is defined AND `-DAMREX_USE_ASSERTION` is NOT defined.
