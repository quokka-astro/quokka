...
- **User source injection:** `RadSystem<problem_t>::SetRadSource` is called before each implicit solve and lets a problem add its own radiation source. It fills two buffers: `radEnergySource`, a luminosity volume density per group, and `radFluxSource`, the companion flux source in component \\(3g + n\\) for group \\(g\\) along direction \\(n\\). Both are scaled the same way internally — by \\(\hat c / c\\) for a thermal group and not at all for a chemistry band — so setting `radFluxSource = c * radEnergySource` injects fully beamed, free-streaming radiation (\\(|F| = c E\\)) for either kind of band, and leaving `radFluxSource` at zero injects isotropically.
