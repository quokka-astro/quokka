...
- Custom physics: problem-specific traits determine which subsystems `QuokkaSimulation` activates (e.g., MHD, radiation groups, chemistry), and each problem supplies initial conditions (cells and face-centered fields) before calling `sim.evolve()` ([QuokkaSimulation.hpp](https://github.com/quokka-astro/quokka/blob/development/src/QuokkaSimulation.hpp#L183-L199), [`test_orszag_tang.cpp`](https://github.com/quokka-astro/quokka/blob/development/src/problems/OrszagTang/test_orszag_tang.cpp#L30-L138)).
...
2. **Problem setups.** Study more drivers in `src/problems/` and the matching documentation in the Tests section to see how diagnostic comparisons are scripted ([`test_orszag_tang.cpp`](https://github.com/quokka-astro/quokka/blob/development/src/problems/OrszagTang/test_orszag_tang.cpp#L27-L138), [test index](tests/index.md)).
