...
- **Initial setup.** After cloning, run `./scripts/bash/bootstrap.sh` once to install the `quokka` CLI and `pre-commit.sh` into `~/.local/bin/`.
...
- **Pre-commit checks.** Run `./scripts/bash/pre-commit.sh` to execute the same pre-commit hooks (YAML validation, clang-format, merge conflict detection, etc.) that the CI enforces. The script prefers `uv run --with pre-commit`, falls back to `pre-commit` directly, and auto-installs via `pip install --user` if neither is available — no manual setup required.
