...
- **Quick GPU CI on a PR.** Drop the comments `/azp run quick` and `/azp run romc-quick` on a PR to trigger quick GPU CI workflows. These build and run, on GPUs, only the test problems whose sources under `src/problems` have changed in the PR. If the PR doesn't touch `src/problems`, the quick workflows have nothing to build; make a dummy change to a test problem to trigger them, and revert that dummy change before merging if it isn't otherwise needed.
