...
- **Start with a smoke test.** For first-time local verification, prefer the fast local validation path in the Quokka CLI guide before running longer problem tests or GPU regression suites ([Quokka CLI](quokka_cli.md#fast-local-validation)).
