...
| dust.picard_alpha_rtol             | Float         | `1e-6`         | Relative convergence tolerance for the reciprocal stopping time \\(\alpha=1/T_{\mathrm{s}}\\) at each GIRK stage.                                |
| dust.picard_charge_atol            | Float         | `1e-12`        | Absolute convergence tolerance for \\(\xi\\) at each GIRK stage; used only when the magnetic field is nonzero.                                  |
| dust.picard_charge_rtol            | Float         | `1e-6`         | Relative convergence tolerance for \\(\xi\\), scaled by its value from the current Picard iterate.                                               |
