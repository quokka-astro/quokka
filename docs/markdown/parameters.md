...
| particles.stromgren_max_nonlinear_iters       | Integer       | `20`                                                                                                          | Maximum Newton iterations per attempt for the composite AMR backward-Euler photon solve.                                                                       |
| particles.stromgren_residual_tol              | Float         | `1.0e-3`                                                                                                      | Relative tolerance for the volume-weighted composite backward-Euler residual.                                                                                   |
| particles.stromgren_abort_on_max_iters        | Boolean (0/1) | `0` (Disabled)                                                                                                | If set to 1, aborts only after adaptive photon-timestep bisection cannot produce converged Newton--GMRES solves; otherwise the old photon field is retained.      |
