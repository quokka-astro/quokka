...
| particles.min_mass_individual_stars           | Float         | `9.0`                                                                                                         | Minimum initial mass in solar masses above which stars are sampled individually from the IMF. Stars below it are lumped into the LowMassComposite particle, which neither explodes nor radiates. Must lie in `[1, 120)`. The IMF mass fraction and mean stellar mass above the threshold are recomputed from the Chabrier IMF whenever this changes. Lowering it is expensive: the number of individual star particles per unit stellar mass goes as $f_\star(m)/\langle m\rangle_{>m}$, which is $0.0096\ M_\odot^{-1}$ at $9\ M_\odot$ but $0.0435\ M_\odot^{-1}$ at $3\ M_\odot$, a factor of 4.5. Note also that the luminosity table is clamped at its lowest tabulated mass (2.1 $M_\odot$ for the default grid of `scripts/python/slug_luminosity_table_for_quokka.py`), so a threshold below that assigns stars in the un-tabulated range the luminosity of the lowest tabulated star.                                              |
