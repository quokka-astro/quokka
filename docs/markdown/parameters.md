...
| temperature_floor           | Float         | `5.0` (CGS), `0.0` (otherwise) | The minimum temperature value allowed in the simulation, in K. Enforced through EnforceLimits. The default is 5 K when `Physics_Traits<problem_t>::unit_system` is `UnitSystem::CGS`, and 0 for the `CONSTANTS` and `CUSTOM` unit systems, which have no physical temperature scale. Idealized CGS test problems that use non-physical values should set this to 0 explicitly. |
