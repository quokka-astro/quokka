...
| projection_interval         | Integer       | `-1` (Disabled)| The number of coarse timesteps between 2D projection outputs.                                                                                                                                                   |
...
| do_reflux                   | Integer       | `1` (Enabled) | This turns on refluxing at coarse-fine boundaries (1) or turns it off (0). Except for debugging, this should always be on when AMR is used.                                                                     |
| do_tracers                  | Integer       | `0` (Disabled)| This turns on tracer particles. They are initialized one-per-cell and they follow the fluid velocity.                                                                                         |
| suppress_output             | Integer       | `0` (Disabled)| If set to 1, this disables output to stdout while the simulation is running.                                                                                                                                    |
...
| show_performance_hints      | Boolean (0/1) | `1` (Enabled) | If set to 1, prints performance performance hints.                                                                 |
...
| do_subcycle                 | Integer       | `1` (Enabled) | This turns on subcycling at coarse-fine boundaries (1) or turns it off (0).                                                                                                                    |
...
| print_cycle_timing          | Integer       | `0` (Disabled)| If set to 1, prints per-cycle timing information.                                                                                                                                        |
...
| Parameter Name                         | Type    | Default        | Description                                                                                                                                                                               |
| -------------------------------------- | ------- | -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| hydro.low_level_debugging_output       | Integer | `0` (Disabled) | If set to 1, turns on low-level debugging output for each RK stage. Warning: this writes an enormous volume of data to disk! This should only be used for debugging.          |
...
| hydro.use_dual_energy                  | Integer | `1` (Enabled)  | If set to 1, the code evolves an auxiliary internal energy variable in order to correctly evolve high-mach flows. This should only be disabled (0) for debugging.             |
| hydro.abort_on_fofc_failure            | Integer | `1` (Enabled)  | If set to 1, the code aborts when first-order flux correction fails to yield a physical state (positive density and pressure). This should only be disabled (0) for debugging.            |
...
| radiation.print_iteration_counts     | Integer | `0` (Disabled)  | If set to 1, prints radiation iteration counts for debugging.                                                                                               |
...
| mhd.emf_averaging_scheme      | String      | `"LondrilloDelZanna2004"`  | Determines the method used to average EMF at edges. Can be set to `BalsaraSpicer2004`, `LondrilloDelZanna2004`, or `Balsara2025`. |
...
| Parameter Name                       | Type    | Default         | Description                                                                                                                                       |
| ------------------------------------ | ------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| cooling.enabled                      | Integer | `0` (Disabled)  | If set to 1, turns on optically-thin radiative cooling as a Strang-split source term.                                      |
...
| cooling.read_tables_even_if_disabled | Integer | `0` (Disabled)  | If set to 1, reads the cooling tables even if the cooling module is disabled.                                                                     |
...
| Parameter Name                | Type    | Default         | Description                                                                                                                                     |
| ----------------------------- | ------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| chemistry.enabled             | Integer | `0` (Disabled)  | If set to 1, turns on chemistry as a Strang-split source term.                                                           |
...
| Parameter Name | Type        | Default        | Description                                                                                                                                  |
| -------------- | ----------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| dust.enable_iter_stoptime    | Integer | `0` (Disabled) | If set to 1, enables iterative dust stopping time calculation.                                                                                                 |
...
| dust.print_iteration_counts     | Integer | `0` (Disabled) | If set to 1, prints dust drag iteration counts for debugging.                                                                                               |
...
| Parameter Name                             | Type          | Default         | Description                                                                                                                                                                                             |
| ------------------------------------------ | ------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| particles.disable_SN_feedback              | Integer       | `0`   | If set to 1, disables SN feedback when a particle evolves from SNProgenitor to SNRemnant.                                                                                         |
| particles.sink_particle_use_uniform_kernel | Integer       | `0` (Disabled)  | If set to 1, uses uniform accretion kernel in a (7 dx)^3 box for sink particles.                                                                                                 |
...
| particles.verbose                          | Integer       | `0`             | Verbosity level for particle operations. Higher values provide more detailed output.                                                                                                        |
...
| particles.rad_table                        | String        | **Required** if `particles.use_luminosity_table=1` and `Physics_Traits<problem_t>::is_radiation_enabled=true`    | Path to the radiation luminosity table. Required                                                                                                                                                                 |
...
| Parameter Name               | Type    | Default        | Description                                                                                                                                                  |
| ---------------------------- | ------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| turbulence.enabled           | Integer | `0` (Disabled) | If set to 1, enables turbulence driving using [chfeder's turbulence driving module](https://github.com/chfeder/turbulence_generator). |
| turbulence.length            | Float   | **Required**   | Length of turbulent driving box. Can have 1 value or a comma separated list for each component. Required if `turbulence.enabled=1`.                                                               |
| turbulence.target_vdisp      | Float   | **Required**   | Target turbulent velocity dispersion. Required if `turbulence.enabled=1`.                                                                                                                        |
| turbulence.ampl_factor       | Float   | **Required**   | Amplitude adjust factor for forcing field. Can have 1 value or a comma separated list for each component. Required if `turbulence.enabled=1`.                                                    |
| turbulence.ampl_auto_adjust  | Integer | `0` (Disabled) | If set to 1, enables automatic amplitude adjustment.                                                                                  |
| turbulence.k_driv            | Float   | **Required**   | Characteristic driving scale in units of $2\pi/length$. Required if `turbulence.enabled=1`.                                                                                                      |
| turbulence.k_min             | Float   | **Required**   | Minimum driving wavenumber in units of $2\pi/length$. Required if `turbulence.enabled=1`.                                                                                                        |
| turbulence.k_max             | Float   | **Required**   | Maximum driving wavenumber in units of $2\pi/length$. Required if `turbulence.enabled=1`.                                                                                                        |
| turbulence.sol_weight        | Float   | **Required**   | Solenoidal weight. Can be 0.0 (compressive driving), 1.0 (solenoidal driving) or 0.5 (natural mixture). Required if `turbulence.enabled=1`.                                                      |
| turbulence.spect_form        | Integer | **Required**   | Spectral form of the driving amplitude. Can be 0 (band, rectangle, constant), 1 (paraboloid) or 2 (power law). Required if `turbulence.enabled=1`.                                               |
| turbulence.power_law_exp     | Float   | **Required**   | If spect_form = 2, this sets the spectral power-law exponent (e.g., -5/3: Kolmogorov; -2: Burgers). Required if `turbulence.enabled=1`.                                                          |
| turbulence.angles_exp        | Float   | **Required**   | If spect_form = 2, this sets the number of modes (angles) in k-shell such that it increases as $k^angles_exp$. Required if `turbulence.enabled=1`.                                               |
| turbulence.random_seed       | Integer | **Required**   | Random number seed for driving sequence. Required if `turbulence.enabled=1`.                                                                                                                     |
| turbulence.nsteps_per_t_turb | Integer | **Required**   | Number of turbulence driving pattern updates per turnover time. Required if `turbulence.enabled=1`.                                                                                              |
...
| sfh_to_pe_heating_table        | String        | **Required**   | Path to the table converting star formation history to photoelectric heating. Required if `use_sfh_based_pe_heating = 1` and `const_sfr_Msun_per_year_per_kpc2 < 0`.                                           |
| sf_area_kpc2                   | Float         | **Required**   | Area of the star formation region in kpc^2. Required if `use_sfh_based_pe_heating = 1` and `const_sfr_Msun_per_year_per_kpc2 < 0`.                                                                              |
