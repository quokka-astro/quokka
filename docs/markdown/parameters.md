...
| turbulence.remove_mean_flow  | Boolean (0/1) | `0` (Disabled)                         | If set to 1, subtracts the mass-weighted mean of the forcing's own momentum injection each step, preventing density-weighted driving from injecting a net bulk flow. Pre-existing bulk motion (from initial conditions, gravity, feedback) is left untouched. |
