...
// State
let levels = [
  { id: 0, nx: 128, ny: 128, nz: 128, filling: 100 }
];
...
function addEventListeners() {
    ...
    // Export button
    document.getElementById('export-btn').addEventListener('click', exportCalculation);
}
    ...
    levels.forEach((level, index) => {
        ...
        div.innerHTML = `
            <div class="level-header">
                ...
                ${index > 0 ? `<button class="remove-level-btn" onclick="removeLevel(${index})">×</button>` : ''}
            </div>
            <div class="level-inputs">
                <div>
                    <label>Nx</label>
                    <input type="number" value="${level.nx}" step="64" oninput="updateLevel(${index}, 'nx', this.value)">
                </div>
                <div>
                    <label>Ny</label>
                    <input type="number" value="${level.ny}" step="64" oninput="updateLevel(${index}, 'ny', this.value)">
                </div>
                <div>
                    <label>Nz</label>
                    <input type="number" value="${level.nz}" step="64" oninput="updateLevel(${index}, 'nz', this.value)">
                </div>
                <div>
                    <label>Fill %
                    ...
                    <input type="number" value="${level.filling}" placeholder="100" oninput="updateLevel(${index}, 'filling', this.value)">
                </div>
            </div>
        `;
...
function calculate() {
  ...
  // Derived: min dx
    ...
    if (isManualSteps) {
        ...
        if (unit === 'Myr') tStopS = tStopVal * MYR_TO_S;
        else tStopS = tStopVal; // seconds
  ...
  // Runtime (s)
    ...
    outputs.minDx.textContent = minDxPc > 0 ? `${minDxPc.toExponential(2)} pc` : '-';
  ...
  outputs.rtHours.textContent = formatNumber(totalHours);
    outputs.rtSeconds.textContent = runtimeS > 0 ? runtimeS.toExponential(2) : '0';
...
function formatNumber(num) {
    if (num === 0) return '0';
    if (num >= 1e6 || (num < 0.01 && num > 0)) return num.toExponential(2);
...
function exportCalculation() {
    ...
    // Actually, let's just grab the text content for simplicity and "what you see is what you get", 
    // but for the specific fields requested (runtimeHours, etc) we might need to derive them if they aren't explicitly on screen in that format.
    // Wait, the user requested specific fields: runtimeHours, runtimeDays. 
    // These are calculated in `calculate()` but not all stored in global state.
    // I should probably refactor `calculate` to return values or store them, OR just re-implement the calculation logic inside export (duplication bad), 
    // OR just parse the display values where possible and do simple math.
    
    // Let's try to extract the raw values from the DOM elements where possible, or re-derive simple ones.
    ...
    const perfUnit = inputs.perfUnit.value;
    let gpuNs = 0;
    if (perfUnit === 'ns') gpuNs = perfValue;
    else if (perfUnit === 'mupdates') gpuNs = perfValue > 0 ? 1000 / perfValue : 0;

    // Derived
    let minDxPc = 0;
    if (boxSizePc > 0 && maxNx > 0) minDxPc = boxSizePc / maxNx;
    const minDxKm = minDxPc * PC_TO_KM;

    let dt = 0;
    if (vmax > 0 && minDxKm > 0 && cfl > 0) dt = cfl * minDxKm / vmax;
    const dtYears = dt * SEC_TO_YR;
    ...
    if (manualStepsToggle.checked) {
        ...
        if (dt > 0) totalSteps = tStopS / dt;
    }

    const totalUpdates = totalCells * totalSteps;
    let runtimeS = 0;
    if (gpuNs > 0 && numGpus > 0) runtimeS = totalUpdates * (gpuNs * 1e-9) / numGpus;
