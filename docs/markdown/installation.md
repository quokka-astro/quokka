...
> *Requires ROCm 6.3.0 or newer. The directory containing the HIP and other related binaries must be added to the `PATH` environment variable after the ROCm installation. For a nonstandard installation prefix, set `CMAKE_PREFIX_PATH`, `ROCM_PATH`, `HIP_PATH`, or `hip_DIR` so CMake can find the HIP package.*

Build with `-DAMReX_GPU_BACKEND=HIP`. Your MPI library **must** support GPU-aware MPI for AMD GPUs. The typical AMD GPU compilers are `amdclang++` or `hipcc`. In case your GPU-aware compiler is not being used by default during the build, use the `CMAKE_CXX_COMPILER` and `CMAKE_C_COMPILER` options to specify the C++ and C compilers respectively. Additionally, the AMD GPU architecture may have to be specified. This can be done using the `AMReX_AMD_ARCH` option. The GPU architecture can be found using
