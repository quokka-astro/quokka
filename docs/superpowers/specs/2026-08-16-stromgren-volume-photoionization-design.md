...
with $c_0 = 34.3149$, $c_1 = 15.8931$, $c_2 = -4.0825$. The fit spans $16.46$–$58.34\,M_\odot$ (O9.5V to O3V) with rms residual $0.036$ dex and maximum residual $0.064$ dex.

**Why a quadratic and not a cubic.** The degree was chosen from the data, not assumed. Fitting a cubic to the same twelve points lowers the rms only from $0.036$ to $0.035$ dex — a 3 per cent gain for a 33 per cent larger parameter set — while its leading coefficient comes out negative. The cubic therefore peaks at $71\,M_\odot$ and declines: it would assign a $150\,M_\odot$ star ($\log_{10} Q_0 = 48.91$) less ionizing flux than a $37\,M_\odot$ one ($49.26$), which is a non-physical inversion in a feedback module. The quadratic shares the flaw only beyond $88\,M_\odot$, outside the tabulated range, and the clamp below removes it entirely.

**Outside the tabulated range** the fit is not extrapolated. Below $16.46\,M_\odot$ the rate is zero: extrapolating gives $10^{43.4}$ photons per second at $5\,M_\odot$ against a true value near $10^{38}$, and low-mass stars vastly outnumber O stars in a sampled IMF, so the spurious contribution would dominate the budget. The cutoff also keeps $\log_{10}$ away from zero mass, where it would raise a floating-point trap. Above $58.34\,M_\odot$ the mass is clamped, so $Q$ saturates at the O3V value; this under-predicts very massive stars by roughly a factor of two at $100\,M_\odot$, the conservative direction, and avoids the turnover.

Two earlier iterations are worth recording. The module first used the Vacca, Garmany & Shull (1996) power law $Q \propto m^{4.91}$, which is steeper than the Martins calibration. It was then refitted to three approximate anchor values ($\log_{10} Q_0 = 48.5$, $49.0$, $49.5$ at $20$, $30$, $50\,M_\odot$) before the source table was available; against the real Table 1 those anchors are high by $0.43$ dex at $20\,M_\odot$, $0.12$ dex at $30\,M_\odot$ and low by $0.04$ dex at $50\,M_\odot$. The present fit supersedes both, being derived directly from the tabulated values.

`make_qm_figure.sh` in the PR artefacts folder re-derives this fit from the LaTeX source table and plots $Q(M)$, so the figure and the coefficients in the code cannot drift apart.
...
The test problem needs an exact $Q$ to compare against the analytic radius. Inverting the fit for a mass that yields a chosen rate is awkward and would tie the test to the calibration, so instead the test fixes a $30\,M_\odot$ star and takes $Q = 7.65\times10^{48}\ \mathrm{s}^{-1}$ from the model. It separately validates the calibration itself against five rows of Martins et al. Table 1, plus the zero-below and clamped-above behaviour at the range edges. It additionally supports an override via `stromgren.Q_ion` (a non-positive value, the default, means "use the stellar model"), which the subgrid variant uses to reach a rate far below anything a real star produces.
