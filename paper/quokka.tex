...
We solve the equations of radiation hydrodynamics \citep{Pomraning_1973,Mihalas_1984,Castor_2004} for an inviscid, ideal, nonrelativistic fluid in the so-called `mixed frame,' where the radiation variables are in the laboratory frame (or simulation coordinates) and the radiation-matter interaction terms are written as an expansion in the ratio of the fluid velocity to the speed of light $\beta = v/c$. We write the equations as follows:
\begin{align}
    \frac{\partial \rho}{\partial t} = \dots \, \\
    \frac{\partial (\rho \vec{v})}{\partial t} = \dots \, \\
    \frac{\partial E_g}{\partial t} = \dots \, \\
    \frac{\partial E_r}{\partial t} = \dots \, \\\
    \frac{\partial F_r}{\partial t} = \dots \,
\end{align}
where $G$ is the radiation four-force (see e.g., \citealt{Mihalas_1984}). Expanding in $v/c$, we obtain:
\begin{align}
G^0 = \dots \,
\end{align}
