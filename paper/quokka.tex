...
In this paper we describe \quokka\footnote{Quadrilateral, Umbra-producing, Orthogonal, Kangaroo-conserving Kode for Astrophysics!}, a new code for RHD. In terms of the taxonomy introduced above, \quokka~is a moment-method based code that uses the RSLA to allow an explicit treatment of radiation transport. \quokka~is an adaptive mesh refinement (AMR) code, which begins with a base grid at uniform spatial resolution, but then dynamically adds higher-resolution grids as needed to achieve user-specified accuracy goals \citep{Berger:1984, Berger:1989}. However, these features do not make \quokka~unique: both \textsc{Orion} and \textsc{Ramses} offer moment-based AMR RMHD (as does \textsc{Athena++}, though not in its public release), while \texttt{Fornax} uses RSLA on a dendritic (though not adaptive) grid.

The unique feature of \quokka~is that it has been built from the ground up to run efficiently on graphical processing units (GPUs). While \quokka~is not the first GPU code in astrophysics (others include \textsc{Gamer}, \citealt{Schive10a, Schive18a}, and \textsc{Cholla}, \citealt{Schneider15a}), nor even the first AMR GPU code, it is the first to offer RHD on GPU. Bringing RHD to GPU creates some unique challenges. Implicit methods generally offer poor scalability on GPU, due to the need for global communications during an implicit solve. 
