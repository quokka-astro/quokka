...
In this paper we describe \quokka\footnote{Quadrilateral, Umbra-producing, Orthogonal, Kangaroo-conserving Kode for Astrophysics!}, a new code for RHD. In terms of the taxonomy introduced above, \quokka~is a two moment code that uses the RSLA to allow an explicit treatment of radiation transport. \quokka~is also an adaptive mesh refinement (AMR) code, so it begins with a base grid at uniform spatial resolution, but then dynamically adds higher-resolution grids as needed to achieve user-specified accuracy goals \citep{Berger:1984, Berger:1989}. However, these features do not make \quokka~unique: both \textsc{Orion} and \textsc{Ramses} offer moment-based AMR RMHD (as does \textsc{Athena++}, though not in its public release), while \textsc{Fornax} uses RSLA on a dendritic (though not adaptive) grid.

The unique feature of \quokka~is that it has been built from the ground up to run efficiently on graphical processing units (GPUs). While \quokka~is not the first GPU code in astrophysics (others include \textsc{Gamer}, \citealt{Schive10a, Schive18a}, and \textsc{Cholla}, \citealt{Schneider15a}), nor even the first AMR GPU code, it is the first to offer RHD on GPU. Bringing RHD to GPU creates some unique challenges. GPU computations are often limited by data bandwidth, both in terms of moving data between main memory and the GPU, and in terms of moving data between GPUs. For this reason, implicit methods generally offer poor scalability on GPU, due to the need for global communications during an implicit solve. This motivates our choice of an RSLA method. It also motivates our choice of update strategy, which as we detail below has been designed to minimize communication and maximize data residence time on the GPU. We show that, with this strategy, we are able to achieve update computation rates of approximately 70 million zone updates per second per GPU for pure HD, and almost 10 million for RHD. We also achieve $>93\%$ parallel efficiency out to 256 GPUs. This combination of performance and scaling makes \quokka~orders of magnitude faster than any other public RHD code.
...
We show a stationary shock tube in \autoref{fig:shocktube}\dots
\begin{figure}
...
We show the `LeBlanc' shock tube test in \autoref{fig:leblanc}\dots
\begin{figure}
...
We show the Shu-Osher test in \autoref{fig:shu_osher}\dots
\begin{figure}
...
We show a slow-moving shock in \autoref{fig:sms}\dots
\begin{figure}
...
We show a strong rarefaction in \autoref{fig:vacuum}\dots
\begin{figure}
...
We show a Marshak wave in \autoref{fig:marshak}\dots
\begin{figure}
