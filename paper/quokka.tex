...
In order to test the radiation-gas momentum coupling in the optically-thin limit, we next simulate a radiation-driven wind in the limit of very low optical depth. We consider an isothermal gas with sound speed of $c_T = 0.2 \km \s^{-1}$, with constant opacities $\kappa_P = 0$ and $\kappa_R = 5 \cm^2 \gm^{-1}$. The optical depth across the length of the domain is $10^{-6}$. In dimensionless variables, the wind is defined solely by an initial Mach number $\mathcal{M}_0$, chosen here as $\mathcal{M}_0 = 1.1$. The radiation flux $F_{r,0}$, radiative acceleration $a_0$, and characteristic acceleration length $L$ are then
\begin{align}
F_{r,0} = \rho_0 c_T c / \tau \, , \\
a_0 = \kappa_R F_{r,0} / c \, , \\
L = c_T^2 / a_0 \, .
\end{align}
where $\rho_0$ is the density at the base of the wind. We choose $\rho_0 = 3.897212 \times 10^{-19} \gm \cm^{-3}$. The length of the computational domain is one characteristic acceleration length $L$. The steady-state solution for the Mach number $M$ is given by integrating the steady-state plane-parallel gas momentum equation with respect to position $x$ above the base of the wind, obtaining a Bernoulli equation for $\mathcal{M}$ as a function of height $x$:
\begin{align}
\frac{1}{2} \mathcal{M}_0^2 = \frac{1}{2} \mathcal{M}^2 + \log \left({\frac{\mathcal{M}_0}{\mathcal{M}}}\right) - \left( \frac{x}{L} \right) \, .
\label{eq:wind}
\end{align}
In steady state, mass conservation requires that $\rho = (\mathcal{M}_0 / \mathcal{M}) \rho_0$ throughout the wind. We neglect gravitational forces in this problem, but we note that our solution is formally equivalent to the Eddington ratio $\eta_{\text{Edd}} = 2$ case of the plane-parallel radiation-inhibited Bondi accretion problem of \cite{Skinner_2013}. The Mach number and density profile obtained from \autoref{eq:wind} are used for the initial conditions, and Dirichlet boundary conditions are adopted using the values of the exact solution at the boundaries. An exact isothermal Riemann solver is used for this simulation. Using a CFL number of 0.4, we evolve until $t = 10 \, (L/c_T)$.
In \autoref{fig:wind}, we show the exact solution for the Mach number (circles) as well as the solution produced by \textsc{Quokka} (solid line), finding excellent agreement.
\begin{figure}
...
Shown in \autoref{table:strong_scaling}\dots
