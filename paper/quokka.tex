...
%     \caption{Results for a simulation of a stationary contact discontinuity. Black points show the simulation result, while the blue line shows the exact solution.}
...
\textcolor{red}{There isn't enough detail on the problem setup here. What do you mean an internal radiation source? Implemented how? With what properties? I know the answer's are in Su \& Olson, but you should give enough description here that readers can follow how the problem setup works.}
...
Our first radiation-hydrodynamic test is the the radiation pressure tube problem of \cite{Krumholz_2007}. This problem is designed to show that the radiation pressure can stably balance the gas pressure both in the regime where radiation pressure dominates and the in the regime where gas pressure dominates for a problem where the optical depth is very large and the radiation is in the diffusion regime. \textcolor{red}{DESCRIBE THE PROBLEM SETUP, BOTH IN TERMS OF THE INITIAL CONDITION AND THE NUMERICS (RESOLUTION, RUN TIME, CHOICE OF RSLA PARAMETER, ETC.). ASLO, IT SEEMS ODD THAT FOR THIS PROBLEM YOU'RE PLOTTING THE ERROR, WHEREAS EVERYWHERE ELSE YOU PLOTTED THE SOLUTION ITSELF.}

Our numerical solution agrees with the exact diffusion solution to better than $0.2$ per cent. 

in \autoref{fig:radiation_pressure_tube}
\begin{figure}
