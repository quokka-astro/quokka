...
%DIF ADD quokka.tex      Thu Nov 18 15:33:13 2021
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\DIFadd{We write the timestep $\Delta t$ used for the RK2-SSP integration as
}\begin{align}
\DIFadd{\Delta t = C_0 \, \frac{\Delta x}{|\lambda|} \, ,
}\end{align}
\DIFadd{where $\Delta x$ is the minimum grid spacing, $|\lambda|$ is the maximum signal speed, and $C_0$ is the stability coefficient.
...
.}} \DIFadd{For both forward Euler and RK2-SSP, the maximum stable coefficient $C_0$ for a system of constant-coefficient, linear equations is $C_0 = 1$. The maximum stable CFL number in 3D for the RK2-SSP integrator is therefore $1/3$.
...
\DIFaddend Somewhat surprisingly, we find that such a method-of-lines scheme is not stable when combining higher-order spatial reconstruction with forward Euler time integration\DIFdelbegin \DIFdel{, but }\DIFdelend \DIFaddbegin \DIFadd{. However, we find it }\DIFaddend is stable for timesteps \DIFdelbegin \DIFdel{$\mathcal{O}(\Delta x / (c_s + |v|))$ }\DIFdelend \DIFaddbegin \DIFadd{satisfying the above timestep criterion }\DIFaddend when used with higher-order \DIFdelbegin \DIFdel{Runge-Kutta methods. This is consistent with the findings of \mbox{%DIFAUXCMD
\cite{Stone_2020}}\hspace{0pt}%DIFAUXCMD
, who also find that the maximum stable timestep corresponding to the linear stability region of a given (second order }\DIFdelend \DIFaddbegin \DIFadd{(second-order }\DIFaddend or higher) Runge-Kutta \DIFdelbegin \DIFdel{integrator divided by the number of spatial dimensions.
}\DIFdelend \DIFaddbegin \DIFadd{methods. We note that this stability problem with forward Euler is also found by \mbox{%DIFAUXCMD
\cite{Stone_2020} }\hspace{0pt}%DIFAUXCMD
in the method-of-lines implementation of }\textsc{\DIFadd{Athena++}}\DIFadd{.
}
...
We do not perform the slope-limiting and contact steepening steps of \citetalias{Colella_1984}. We instead \DIFdelbegin \DIFdel{enforce monotonicity of the reconstructed state by re-setting the interface state to lie between the values of the cells adjacent to the interface}\DIFdelend \DIFaddbegin \DIFadd{prevent new extrema in the reconstructed states by limiting the interface states at the faces of each cell to the minimum and maximum of the cell-average values of all 27 cells surrounding the cell under consideration (9 cells in 2D)}\DIFaddend , following \cite{Mignone_2005}. This is followed by the extrema detection and overshoot correction step within each cell as described by \citetalias{Colella_1984}. In this step, the parabola assumed to exist across each cell \DIFaddbegin \DIFadd{along each coordinate dimension }\DIFaddend is examined. If an `overshoot' (as defined by \citetalias{Colella_1984}) of the parabola is detected, we follow the original \citetalias{Colella_1984} prescription of performing linear reconstruction on the side of the cell affected by the overshoot. If an extremum is instead detected, rather than forcing the reconstruction to a constant value across the cell as done by \citetalias{Colella_1984}, we revert to performing a linear reconstruction within the affected cell, following \cite{Balsara_2017}. Either of these steps may make the interface states discontinuous, with distinct states associated with each of the two cells adjacent to an interface\DIFdelbegin \DIFdel{, which reduces the order of accuracy of the stencil. Since the overall algorithm is a combination of second order and (usually) fourth order steps, the overall order of accuracy of PPM is often referred to as `third order' (e.g., \mbox{%DIFAUXCMD
