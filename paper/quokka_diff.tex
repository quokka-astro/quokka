...
%DIF ADD quokka.tex      Mon Nov 15 15:17:51 2021
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Analysis of the stability polynomial of the linearized hydrodynamics equations \mbox{%DIFAUXCMD
\citep{Colella_2011} }\hspace{0pt}%DIFAUXCMD
yields a value for the maximum signal speed $|\tilde \lambda|$ of
}\begin{align}
\DIFadd{|\tilde \lambda| = \max \sum_{d=1}^{D} }[\DIFadd{(\vc{v} \cdot \vc{\hat{e}_d}) + c_s}] \DIFadd{\, ,
\label{eq:colella_stability}
}\end{align}
\DIFadd{where $\vc{v}$ is the fluid velocity, $\vc{\hat{e}}$ is the unit vector in coordinate direction $d$, $c_s$ is the adiabatic sound speed, and the maximum is taken over all cells. This signal speed does not take nonlinear effects into account. Because of this, we conservatively estimate the maximum signal speed as 
}\begin{align}
...
\DIFadd{where we define the dimensionless factor $C_0 / D$ to be the CFL number. For both forward Euler and RK2-SSP, the maximum stable coefficient $C_0$ for a system of constant-coefficient, linear equations is $C_0 = 1$. The maximum stable CFL number in 3D for the RK2-SSP integrator is therefore $1/3$.
We note that it is not sufficient to estimate $|\lambda|$ as $\max (|\vc{v}| + c_s)$, since the component-wise sum of the velocities may exceed the vector magnitude $|\vc{v}|$ and therefore violate the lower bound implied by }\autoref{eq:colella_stability}\DIFadd{.}\footnote{\DIFadd{As an example, consider the velocity vector with unit magnitude $|\vc{v}|$ and equal components $v_x = v_y = v_z$. Then each component $v_i = \sqrt{3}/3$ and the sum of components $\sum_{d=1}^{3} \vc{v} \cdot \vc{\hat{e}_d} = \sqrt{3} \approx 1.732$.}}
