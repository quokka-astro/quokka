...
# Additional arguments can be passed via command line thanks to "$@" (e.g. ./target_ctest.sh -R ParticleSF$)
ctest -j"${SLURM_CPUS_PER_TASK:-30}" -V -R "ParticleSF$" "$@"  # -R regular expression: name of the test
